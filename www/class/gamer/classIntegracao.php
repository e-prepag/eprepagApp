...
$partner_list = array(
	...
	*/
	'Bigpoint' => array(
		...
		'partner_ip_defined_list' => '170.33.13.202,62.146.190.0,178.132.240.0,62.146.89.64,47.254.156.231',
		...
		'sonda_url' => 'https://payment-spi.bigpoint.com/buildpay_eprepag/checktransaction',
...
*/
	...
	*/
	...
	'Xsolla' => array(
		...
		'partner_testing_email' => 0,
		'partner_email' => "v.donet@webteam.md, psdepartment@xsolla.com",	//	stats@xsolla.com
		'partner_do_notify' => 1,
		'partner_do_renotify_automatico' => 0,
		'partner_bypass_ip_check' => 1,
	...
	'Xsolla_TESTE' => array(
		'partner_name' => 'Xsolla_TESTE',
		'partner_id' => '10455',
		'partner_opr_codigo' => '167',
		'partner_active' => '1',
		'partner_currency_code' => 'BRL',
		'partner_url' => 'https://os-secure.srv.local',

		'partner_ip' => '185.30.20.18',

		// OLD 'partner_ip' => '94.103.26.178',
		// OLD	'partner_ip_defined_list' => '94.103.26.178,94.103.26.179,94.103.26.180,94.103.26.181,94.103.26.182,178.248.236.17,178.161.146.107',
		// TESTE 'notify_url' => 'https://test-ps.xsolla.com/eprepag/result',
		'notify_url' => 'https://ps.xsolla.dev/universal/eprepag',
		// TESTE 'return_url' => 'https://test-secure.xsolla.com/status/eprepag/return',
		'return_url' => 'https://os-secure.srv.local/status/universal/return',
		// TESTE 'sonda_url' => 'https://test-ps.xsolla.com/eprepag/sonda',
		//'sonda_url' => 'https://ps.xsolla.com/eprepag/sonda',

		'partner_produto_id' => 565, //$INTEGRACAO_STORE_TREINAMENTO_PRODUTO_ID_STR,	// Xsola - ??, Treinamento - $INTEGRACAO_STORE_TREINAMENTO_PRODUTO_ID_STR
		'partner_testing_email' => 1,
		'partner_email' => "v.donet@webteam.md, psdepartment@xsolla.com",	//	stats@xsolla.com
		'partner_do_notify' => 1,
		'partner_do_renotify_automatico' => 0,
	...
	'Garena_TESTE' => array(
		'partner_name' => 'Garena_TESTE',
		'partner_id' => '10456',
		'partner_opr_codigo' => '137',
		'partner_active' => '1',
		'partner_currency_code' => 'BRL',
		'partner_url' => 'http://testpay.garena.sg',
...
*/
	...
	*/
	...
	if (!is_Partner_valido($params["store_id"])) {
		$msg .= "Partner invalid (" . get_param_value($params["store_id"]) . ")\n";
	}
	//	if(!is_numeric($params["order_id"]))		{ $msg .= "order_id invalid (".get_param_value($params["order_id"]).")\n"; }
	if (!($params["order_id"])) {
		$msg .= "order_id invalid - missing value ('" . get_param_value($params["order_id"]) . "')\n";
	}
	if (!is_numeric($params["amount"])) {
		$msg .= "amount invalid (" . get_param_value($params["amount"]) . ")\n";
	}
	if (($params["amount"] / 100) > $GLOBALS['RISCO_GAMERS_FREE_MAXIMO_POR_PEDIDO']) {
		$msg .= "amount too high: " . ($params["amount"] / 100) . " > " . $GLOBALS['RISCO_GAMERS_FREE_MAXIMO_POR_PEDIDO'] . "\n";
	}
	...
	if (!($params["currency_code"] == 'BRL')) {
		$msg .= "currency_code invalid - missing value (" . get_param_value($params["currency_code"]) . ")\n";
	}
	/*
...
function get_Integracao_params_from_POST()
{
	...
	if (isset($GLOBALS['_POST']['order_id']))
		$params['order_id'] = $GLOBALS['_POST']['order_id'];
	if (isset($GLOBALS['_POST']['order_description']))
		$params['order_description'] = $GLOBALS['_POST']['order_description'];
	if (isset($GLOBALS['_POST']['amount']))
		$params['amount'] = $GLOBALS['_POST']['amount'];
	if (isset($GLOBALS['_POST']['product_id']))
		$params['product_id'] = $GLOBALS['_POST']['product_id'];
	if (isset($GLOBALS['_POST']['transaction_id']))
		$params['transaction_id'] = $GLOBALS['_POST']['transaction_id'];

	if (isset($GLOBALS['_POST']['client_email']))
		$params['client_email'] = $GLOBALS['_POST']['client_email'];
	if (isset($GLOBALS['_POST']['client_id']))
		$params['client_id'] = $GLOBALS['_POST']['client_id'];
	if (isset($GLOBALS['_POST']['client_name']))
		$params['client_name'] = $GLOBALS['_POST']['client_name'];
	if (isset($GLOBALS['_POST']['client_zip_code']))
		$params['client_zip_code'] = $GLOBALS['_POST']['client_zip_code'];
	if (isset($GLOBALS['_POST']['client_street']))
		$params['client_street'] = $GLOBALS['_POST']['client_street'];
	if (isset($GLOBALS['_POST']['client_suburb']))
		$params['client_suburb'] = $GLOBALS['_POST']['client_suburb'];
	if (isset($GLOBALS['_POST']['client_number']))
		$params['client_number'] = $GLOBALS['_POST']['client_number'];
	if (isset($GLOBALS['_POST']['client_city']))
		$params['client_city'] = $GLOBALS['_POST']['client_city'];
	if (isset($GLOBALS['_POST']['client_state']))
		$params['client_state'] = $GLOBALS['_POST']['client_state'];
	if (isset($GLOBALS['_POST']['client_country']))
		$params['client_country'] = $GLOBALS['_POST']['client_country'];
	if (isset($GLOBALS['_POST']['client_telephone']))
		$params['client_telephone'] = $GLOBALS['_POST']['client_telephone'];
	if (isset($GLOBALS['_POST']['language']))
		$params['language'] = $GLOBALS['_POST']['language'];
	if (isset($GLOBALS['_POST']['cmd']))
...
function sanitize_date($dateval, &$err_cod)
{
	if (strlen($dateval) == 19) {
		...
		if ((substr($dateval, 4, 1) == "-") && (substr($dateval, 7, 1) == "-") && (substr($dateval, 10, 1) == " ") && (substr($dateval, 13, 1) == ":") && (substr($dateval, 16, 1) == ":")) {
			...
			if (
				(is_numeric(substr($dateval, 0, 4))) && (is_numeric(substr($dateval, 5, 2))) &&
...
function send_debug_info_by_email($subject, $body, $partner_id_tmp)
{
	...
	if ($b_testing_email) {
		$GLOBALS['_SERVER']['integration_debug_info'] .= htmlspecialchars($body) . "\n" . str_repeat("-", 80) . "\n";
		send_debug_email(
...
function cpf_page_gamer()
{
	...
	$logDir = '/www/arquivos_gerados/txts';
	$logFilePath = $logDir . '/cpf_log.txt';
	...
	if (pathinfo($logFilePath, PATHINFO_EXTENSION) !== 'txt') {
		throw new Exception("Extensão inválida para arquivo.");
	}
	...
	// Formatando os dados em segurança
	$logData  = "CPF: " . ($user->ug_cpf ?: "NENHUM") . PHP_EOL;
	$logData .= "Data nascimento: " . ($user->ug_data_nascimento ?: "NENHUM") . PHP_EOL;

	// Escrevendo no log com LOCK_EX
	if (file_put_contents($logFilePath, $logData, FILE_APPEND | LOCK_EX) === false) {
		throw new Exception("Falha ao escrever no arquivo de log: $logFilePath");
	}
