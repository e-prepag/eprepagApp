...
/**
		'time_waiting_for_sonda' - in seconds, for the last processed Sonda
		'pagto_blocked' - true/false
		'last_numcompra' => - use this ID to test sonda timeout
*/
	private $_bank_sonda;
	...
	public function unblock_bank_if_normal($iforma) {
		...
		echo "Resultado de unblock_bank_if_normal('$iforma_mod') [".$this->_bank_sonda[$iforma_mod]['time_waiting_for_sonda']."?".$this->CONST_delay_max."] - ".(($bret)?"[Desbloqueia]":"[deixa como está]")."\n";
		return $bret;
	}
	...
	public function list_registers($bHTML) {
		...
		$sret = "";
		if($bHTML) {
			$sret .= "<table border='1' cellpadding='0' cellspacing='1' bordercolor='#cccccc' style='border-collapse:collapse;'>\n";
			...
			$sret .= str_repeat("-", 80)."\nkey	pagto_blocked	last_numcompra	time_waiting_for_sonda\n";
		}
		foreach($this->_bank_sonda as $key => $val) {
			if($bHTML) {
				$sret .= "<tr align='center'><td><font color='#666666' size='1' face='Arial, Helvetica, sans-serif'>$key</font></td> <td><font color='#666666' size='1' face='Arial, Helvetica, sans-serif'>".(($val['pagto_blocked'])?"YES":"nope")."</font></td> <td><font color='#666666' size='1' face='Arial, Helvetica, sans-serif'>".$val['last_numcompra']."</font></td> <td><font color='#666666' size='1' face='Arial, Helvetica, sans-serif'>".number_format($val['time_waiting_for_sonda'], 2, '.', '.')."</font></td> </tr>\n";
			} else {
				$sret .= "$key	".(($val['pagto_blocked'])?"YES":"nope")."	".$val['last_numcompra']."	".number_format($val['time_waiting_for_sonda'], 2, '.', '.')."\n";
			}
		}
		if($bHTML) {
			$sret .= "</table>";
