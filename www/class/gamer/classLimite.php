...
class Limite {
	...
	// Limites pagamentos com cart�o atrav�s da Cielo
	...
	// Fim Limites pagamentos com cart�o atrav�s da Cielo
	...
	function __construct($iforma = null,$idusuario = null,$valor = null, $carrinho = null, $periodo_considerado = null) {
		/*
		================ Instru��es dos par�metros do construtor:
		$iforma :...............Indica a forma de pagamento escolhida indicado por caracter
		$idusuario:.............Trata-se do id do usu�rio que esta executando a compra
		$valor:.................� o valor total da compra
		$carrinho:..............� um vetor contendo os opr_codigo dos produtos presentes na venda. 
		$periodo_considerado:...� o periodo utilizado na totaliza��o para se chegar no volume a ser comprado com constantes de risco cadastrado no in�cio da classe.
...
//echo "carrinho_operadoras: <pre>".print_r($carrinho_operadoras,true)."</pre>";
	...
	// Metodo que carrega as operadoras para os meios de pagamentos - Primeira vers�o
...
//		echo "M�s passado: ".date("Y-m-d H:i:s", mktime($ih, $ii, $is, $im-1, $id, $iy))."<br>";
//		echo "trimestre passado: ".date("Y-m-d H:i:s", mktime($ih, $ii, $is, $im-3, $id, $iy))."<br>";
	...
	//Conforme esta habilitado abaixo se um unico item do carrinho for de uma operadora que n�o esta habilitada para a forma de pagamento a compra inteira ser� abortada. 
...
//echo "aux: <pre>".print_r($aux, true)."</pre>";
				...
				if (array_key_exists($this->getIForma(), $aux)) {
					...
					if($aux[$this->getIForma()] >= $this->getValor()) {
						...
						$msg .= "Valor [".$this->getValor()."] N�o Permitido para este Publisher nesta Forma de Pagamento\n";
						$resposta = false;
						break;
					}
				} else {
					$msg .= "Tipo de Pagamento [".$this->getIForma()."] N�o Permitido\n";
					$resposta = false;
					break;
				}
			} else {
				$msg .= "Operadora [".$opr_codigo."] N�o Permitida\n";
				$resposta = false;
				break;
			}
		}//end foreach
	...
	function aplicaRegrasCielo(&$msg, $params){
		...
		if($resposta && !$msg) {
			// 0 - Testa se Cielo est� habilitado
			if(!($this->getCieloHabilitado()==1)) {
				$msg = "Pagamentos Cielo n�o est�o habilitados\n";
				$resposta = false;
			}
		}
		...
		if($resposta && !$msg) {
			// 2 - Testa se usu�rio est� habilitado para Cielo
...
//gravaLog_DebugTMP("==== ug_use_cielo: \n    usuarioGames->getUseCielo(): ".$usuarioGames->getUseCielo()."\n   ".print_r($usuarioGames, true)." \n");
				...
				if($usuarioGames->getUseCielo()!=1) {
					$msg = "Usu�rio n�o est� habilitado para usar pagamentos Cielo (1) \n	Usu�rio: ID: ".$usuarioGames->getId().", Nome: ".$usuarioGames->getNome().", Email: ".$usuarioGames->getEmail()."\n";
					$resposta = false;
				}
			} else {
...
/*
 * ************************* TEMPOR�RIO PARA NOSSOS USUARIOS
 */
...
//FIM do TEMPOR�RIO PARA NOSSOS USUARIOS
...
//gravaLog_DebugTMP("==== ug_use_cielo: \n    usuarioGames->getUseCielo(): ".$usuarioGames->getUseCielo()."\n   ".print_r($usuarioGames, true)." \n");
				...
				if($usuarioGames->getUseCielo()!=1) {
					$msg = "Usu�rio n�o est� habilitado para usar pagamentos Cielo (2)\n	Usu�rio: ID: ".$usuarioGames->getId().", Nome: ".$usuarioGames->getNome().", Email: ".$usuarioGames->getEmail()."\n";
					$resposta = false;
				}
			} else {
				$msg = "Usu�rio n�o est� logado\n";
...
*/
		...
		/*
		// Se for usu�rio de testes -> sem restri��es
...
//		$this->setValor(300);
		...
		if (!($this->getValor() >= $this->valor_min && $this->getValor() <= $this->valor_max)) {
			$msg .= "Valor da compra fora do valor M�nimo e M�ximo permitido";
			return false;
		}
...
//			if($this->usuarioGames->b_IsLogin_Reinaldo()) 
			{
				...
				$msg_debug = "In getVendasTotalDiario(): \n".
							"total n�o incluido valor da compra abaixo: ".$total." (periodo: ".$this->periodo_considerado.")\n".
		...
		if(($total+$this->getValor())<= $this->riscototaldiario) {
			if ($this->getNVendas($msg)) {
				...
				$msg .= "Ultrapassou a quantidade di�ria de compras.\n";
			}
		}
		else {
			$msg .= "Ultrapassou o limite di�rio de compras.\n";
		}
...
//			if($this->usuarioGames->b_IsLogin_Reinaldo()) 
			{
				...
				$msg_debug = "In getNVendas() 3: \n".
					...
					"	qtde n�o incluindo esta compra: ".$rs_row['qtde']."\n".
					"	idusuario: ".$this->getIdUsuario()."\n".		// , Nome: ".$usuarioGames->getNome().", Email: ".$usuarioGames->getEmail()."
					"	sql: ".$sql."\n";
				gravaLog_LimitePagtoOnline($msg_debug);
			}
		...
		if($qtde<$risco_cielo_90mins_npags) {
			$msg .= "N Tentativas nos �ltimos ".$this->intervalo_para_vendas_em_aberto." minutos [$qtde <= $risco_cielo_90mins_npags] => Permitido\n";
			$resposta = true;
		} else {
			$msg .= "N Tentativas nos �ltimos ".$this->intervalo_para_vendas_em_aberto." minutos [$qtde > $risco_cielo_90mins_npags] => n�o Permitido\n";
			$resposta = false;
		}
...
//			if($this->usuarioGames->b_IsLogin_Reinaldo()) 
	...
	function get_debug_table() {
		...
		echo "<tr><td align='left'>CieloHabilitado</td> <td align='left'>".(($this->getCieloHabilitado()==1)?"<font color='blue'>SIM</font>":"<font color='red'>n�o</font>")."</td></tr>\n";
	...
	function get_debug_table_str() {
		$str  = "";
		$str .= "	CieloHabilitado :".(($this->getCieloHabilitado()==1)?"SIM":"n�o")."\n";
...
/*
	Primeira vers�o alterada em fun��o da reuni�o com o Reynaldo
...
//para funcionar este exemplo deve-se fazer uma compra na loja em outra aba at� o momento de sele��o do meio de pagamento.
//entaun somente ap�s dever� executar este.
