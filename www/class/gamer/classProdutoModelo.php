...
*/
	...
	function obter($filtro, $orderBy, &$rs)
	{
		...
		foreach ($filtro as $key => $value) {
			if (is_string($value)) {
				$filtro[$key] = strtoupper($value);
			}
		}

		$getFiltro = function ($key) use (&$filtro) {
			return array_key_exists($key, $filtro) ? $filtro[$key] : null;
		};

		$hasFiltro = function ($key) use ($getFiltro) {
			$value = $getFiltro($key);
		...
		$addInCondition = function ($column, $values) use (&$params, $addParam) {
			$ids = is_array($values) ? $values : preg_split("/\s*,\s*/", (string)$values, -1, PREG_SPLIT_NO_EMPTY);
			$placeholders = [];

			foreach ($ids as $id) {
				if (is_numeric($id)) {
					$placeholders[] = $addParam((int)$id);
				}
			}

			if (empty($placeholders)) {
				$placeholders[] = $addParam(-1);
			}
		...
		if ($hasFiltro("com_produto") || b_isIntegracao()) {
		...
		if ($hasFiltro("ogpm_data_inclusaoMin") && $hasFiltro("ogpm_data_inclusaoMax")) {
			$filtro["ogpm_data_inclusaoMin"] = formata_data_ts($getFiltro("ogpm_data_inclusaoMin") . " 00:00:00", 1, true, true);
			$filtro["ogpm_data_inclusaoMax"] = formata_data_ts($getFiltro("ogpm_data_inclusaoMax") . " 23:59:59", 1, true, true);
		}

		if ($hasFiltro("ogpm_id")) {
			$where[] = "ogpm.ogpm_id = " . $addParam($getFiltro("ogpm_id"));
		}

		if ($hasFiltro("ogpm_ogp_id")) {
			$where[] = "ogpm.ogpm_ogp_id = " . $addParam($getFiltro("ogpm_ogp_id"));
		}

		if ($hasFiltro("ogpm_nome")) {
			$where[] = "upper(ogpm.ogpm_nome) = " . $addParam($getFiltro("ogpm_nome"));
		}

		if ($hasFiltro("ogpm_nomeLike")) {
			$where[] = "upper(ogpm.ogpm_nome) like " . $addParam("%" . $getFiltro("ogpm_nomeLike") . "%");
		}

		if ($hasFiltro("ogpm_descricao")) {
			$where[] = "upper(ogpm.ogpm_descricao) = " . $addParam($getFiltro("ogpm_descricao"));
		}

		if ($hasFiltro("ogpm_descricaoLike")) {
			$where[] = "upper(ogpm.ogpm_descricao) like " . $addParam("%" . $getFiltro("ogpm_descricaoLike") . "%");
		}

		if ($hasFiltro("ogpm_valorMin") && $hasFiltro("ogpm_valorMax")) {
			$where[] = "ogpm.ogpm_valor between " . $addParam($getFiltro("ogpm_valorMin")) . " and " . $addParam($getFiltro("ogpm_valorMax"));
		}

		if ($getFiltro("ogp_drupal") == 1 && $hasFiltro("ogpm_id_list")) {
			$where[] = $addInCondition("ogpm.ogpm_id", $getFiltro("ogpm_id_list"));
		}

		if ($hasFiltro("ogpm_ativo")) {
			$ativoWhere = ["ogpm.ogpm_ativo = " . $addParam($getFiltro("ogpm_ativo"))];

			if (b_isIntegracao()) {
				$ativoWhere[] = "ogpm.ogpm_id = " . $addParam($hasFiltro("ogpm_id") ? $getFiltro("ogpm_id") : -1);
				$ativoWhere[] = "ogp.ogp_integracao > 0";
				$ativoWhere[] = "ogpm.ogpm_integracao > 0";
			}

			if ($getFiltro("show_treinamento") == 1 && in_array((int)$getFiltro("ogpm_id"), [282, 283, 284, 285, 286])) {
				$ativoWhere[] = "ogpm.ogpm_id in (282, 283, 284, 285, 286)";
			}

			$where[] = "(" . implode(" or ", $ativoWhere) . ")";
		}

		if ($hasFiltro("ogpm_nome_imagem")) {
			$where[] = "upper(ogpm.ogpm_nome_imagem) = " . $addParam($getFiltro("ogpm_nome_imagem"));
		}

		if ($hasFiltro("ogpm_nome_imagemLike")) {
			$where[] = "upper(ogpm.ogpm_nome_imagem) like " . $addParam("%" . $getFiltro("ogpm_nome_imagemLike") . "%");
		}

		if ($hasFiltro("ogpm_data_inclusaoMin") && $hasFiltro("ogpm_data_inclusaoMax")) {
			$where[] = "ogpm.ogpm_data_inclusao between " . $addParam($getFiltro("ogpm_data_inclusaoMin")) . " and " . $addParam($getFiltro("ogpm_data_inclusaoMax"));
		}

		if ($hasFiltro("ogpm_pin_valor")) {
			$where[] = "ogpm.ogpm_pin_valor = " . $addParam($getFiltro("ogpm_pin_valor"));
		}

		if ($hasFiltro("ogpm_valor_eppcash")) {
			$where[] = "ogpm.ogpm_valor_eppcash = " . $addParam($getFiltro("ogpm_valor_eppcash"));
		}

		if ($getFiltro("show_treinamento") == 1 && $getFiltro("ogpm_ogp_id") == 63) {
			$orWhere[] = "ogpm.ogpm_ogp_id = " . $addParam($getFiltro("ogpm_ogp_id"));
		}

		if (!empty($where) || !empty($orWhere)) {
			if (!empty($where) && !empty($orWhere)) {
				$sql .= "where ((" . implode(") and (", $where) . ") or (" . implode(") or (", $orWhere) . "))\n";
			} elseif (!empty($where)) {
				$sql .= "where " . implode(" and ", array_map(function ($item) {
					return "(" . $item . ")";
				}, $where)) . "\n";
			} else {
				$sql .= "where " . implode(" or ", array_map(function ($item) {
					return "(" . $item . ")";
				}, $orWhere)) . "\n";
			}
		}
