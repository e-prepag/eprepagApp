...
class UsuarioGames {
    ...
    // Categoria de Fideliza��o
    var $ug_categoria_fidelizacao;

    // Observa��es
    var $ug_obs;

    // Nome da m�e do usu�rio
    var $ug_nome_da_mae;
	...
	/*
		19090 - "ELIANA AP.ANDREOTTI PETTA", 9845 - "REINALDO P�REZ S�NCHEZ KAIZEN", 16265 - "SILVIA CRISTINA PARISOTTO BALDIN" 
		...
		1335408 => DIEGO BARBOSA SOARES DA SILVA 391.404.938-30 Adicionado em 20/02/2024 como solu��o para o caso do ticket #946
	...
	*/
    ...
    function setNewsLetter($ugNewsLetter) {
        // Se n�o for H - HTML ou N - N�o ent�o cadastra T - Text
    ...
    //Metodos de SET e GET para o Campo ug_saldo_fidelizacao - Contem o Saldo de Fideliza��o
    ...
    //Metodos de SET e GET para o Campo ug_categoria_fidelizacao - Contem a Categoria de Fideliza��o
    ...
    function inserir_simple($store_id_novo, $email_novo) {
        ...
        if ($ret == "") {
            ...
            $dataInclusao = "CURRENT_TIMESTAMP";
            $dataUltimoAcesso = "CURRENT_TIMESTAMP";
            $qtdeAcessos = 1;  // Este � o primeiro acesso
            $news_letter = "h";  // usu�rio recebe news
...
//SQL
            ...
            $sql .= ");";
//echo "$sql<br>";
//grava_log_integracao("Integra��o Debug 4: ".date("Y-m-d H:i:s")."\n  $sql \n");
            $ret = SQLexecuteQuery($sql);
            if (!$ret) $ret = "Erro ao inserir usuário.\n";
            else { 
                $ret = "";
			...
			if(!$ret) $ret = "Erro ao atualizar usuário.\n";
                ...
                if (is_object($usuarioGames)) {
                    ...
                    /*
                      ...
                      <tr valign='middle' bgcolor='#FFFFFF'>
                      <td align='left' class='texto'>
                      Voc� acessou nosso site e alterou seu cadastro.<br><br>
                      Utilize seu email " . $objGamesUsuario->getEmail() . " para acessar sua conta e realizar compras em nosso site.
                      </td>
                      </tr>
                      <tr><td>&nbsp;</td></tr>
...
//						enviaEmail($usuarioGames->getEmail(), null, null, "E-Prepag - Atualiza��o de Cadastro", $msgEmail);
    ...
    function validarCamposLogin($senha, $senhaConf, $login) {
        ...
        //SenhaConf 		
        if ($senha != $senhaConf) {
            $ret .= "A confirma��o da senha deve ser igual a senha.";
        }

        //login
 		if(is_null($login) || $login == "") $ret .= "O Email deve ser preenchido.\n";
		elseif(strlen($login) > 100) 		$ret .= "O Email deve ter at� 100 caracteres.\n";
		elseif(!verifica_email($login)) 	$ret .= "O Email � inv�lido.\n";
...
// 			elseif(verificaCPFEx($CPF) == 0) 		$ret .= "O CPF inv�lido. Utilize o formato xxx.xxx.xxx-xx\n";
// 		}
...
// 			elseif(verificaRG($RG) == 0) 	$ret .= "O RG inv�lido.\n";
// 		}
        ...
        //	else{
        //		if(!is_null($CPF) && $CPF != ""){
        //			if(verificaCPFEx($CPF) == 0) $ret .= "O CPF � inv�lido. Utilize o formato xxx.xxx.xxx-xx\n";
        //		}
        //		if(!is_null($RG) && $RG != ""){
//					if(!eregi("^[A-Z0-9\.-/]{7,13}$", $RG)) $ret .= "O RG � inv�lido.\n";
        //			if(!eregi("^[A-Z0-9/.-]{7,13}$", $RG)) $ret .= "O RG � inv�lido.\n";
        //		}
        //	}
        //}
        ...
        if (!is_null($dataNascimento) || $blCompleto) {
            $dataNascimento = trim($objGamesUsuario->getDataNascimento());
 			...
 			elseif(verifica_data($dataNascimento) == 0)				$ret .= "A Data de Nascimento é inv�lida.\n";
        }
        ...
        /*
          ...
          if(is_null($endereco) || $endereco == "") 	$ret .= "O Endere�o deve ser preenchido.\n";
          elseif(strlen($endereco) > 100) 			$ret .= "O Endere�o deve ter at� 100 caracteres.\n";
          }
          ...
          if(is_null($numero) || $numero == "") 	$ret .= "O N�mero deve ser preenchido.\n";
          elseif(strlen($numero) > 10) 			$ret .= "O N�mero deve ter at� 10 caracteres.\n";
          }
          ...
          if(strlen($complemento) > 100) 				$ret .= "O Complemento deve ter at� 100 caracteres.\n";
          }
          ...
          elseif(strlen($bairro) > 100) 			$ret .= "O Bairro deve ter at� 100 caracteres.\n";
          }
         */
        //Cidade
...
//echo $Tel
        if ((!is_null($Tel) &&  $Tel != "") || $blCompleto) {
            ...
            //			elseif(verifica_tel($Tel) == 0)		$ret .= "O Telefone � inv�lido.\n";
...
// 			elseif(verifica_tel($Cel) == 0)		$ret .= "O Celular � inv�lido.\n";
    ...
    function obterdadosteste($filtro, $orderBy) {
        ...
        if (!is_null($filtro) && $filtro != "") {
            ...
            if (isset($filtro['ug_data_inclusaoMin']) && isset($filtro['ug_data_inclusaoMax']) &&
                !is_null($filtro['ug_data_inclusaoMin']) && !is_null($filtro['ug_data_inclusaoMax'])) {
            ...
            if (isset($filtro['ug_data_ultimo_acessoMin']) && isset($filtro['ug_data_ultimo_acessoMax']) &&
                !is_null($filtro['ug_data_ultimo_acessoMin']) && !is_null($filtro['ug_data_ultimo_acessoMax'])) {
            ...
            if (isset($filtro['ug_data_nascimentoMin']) && isset($filtro['ug_data_nascimentoMax']) &&
                !is_null($filtro['ug_data_nascimentoMin']) && !is_null($filtro['ug_data_nascimentoMax'])) {
            ...
            $sql .= " and (" . (isset($filtro['ug_id']) && !is_null($filtro['ug_id']) ? 0 : 1);
            $sql .= "=1 or ug_id = " . SQLaddFields($filtro['ug_id'], "") . ")";
        
            $sql .= " and (" . (isset($filtro['ug_ativo']) && !is_null($filtro['ug_ativo']) ? 0 : 1);
            $sql .= "=1 or ug_ativo = " . SQLaddFields($filtro['ug_ativo'], "") . ")";
        
            $sql .= " and (" . (isset($filtro['ug_data_inclusaoMin']) && isset($filtro['ug_data_inclusaoMax']) && !is_null($filtro['ug_data_inclusaoMin']) && !is_null($filtro['ug_data_inclusaoMax']) ? 0 : 1);
            $sql .= "=1 or ug_data_inclusao between " . SQLaddFields($filtro['ug_data_inclusaoMin'], "") . " and " . SQLaddFields($filtro['ug_data_inclusaoMax'], "") . ")";
        
            $sql .= " and (" . (isset($filtro['ug_data_ultimo_acessoMin']) && isset($filtro['ug_data_ultimo_acessoMax']) && !is_null($filtro['ug_data_ultimo_acessoMin']) && !is_null($filtro['ug_data_ultimo_acessoMax']) ? 0 : 1);
            ...
            $sql .= " and (" . (isset($filtro['ug_qtde_acessosMin']) && isset($filtro['ug_qtde_acessosMax']) && !is_null($filtro['ug_qtde_acessosMin']) && !is_null($filtro['ug_qtde_acessosMax']) ? 0 : 1);
            $sql .= "=1 or ug_qtde_acessos between " . SQLaddFields($filtro['ug_qtde_acessosMin'], "") . " and " . SQLaddFields($filtro['ug_qtde_acessosMax'], "") . ")";
        
            $sql .= " and (" . (isset($filtro['ug_email']) && !is_null($filtro['ug_email']) ? 0 : 1);
            $sql .= "=1 or ug_email = '" . SQLaddFields($filtro['ug_email'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_emailLike']) && !is_null($filtro['ug_emailLike']) ? 0 : 1);
            $sql .= "=1 or ug_email like '%" . SQLaddFields($filtro['ug_emailLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_nome']) && !is_null($filtro['ug_nome']) ? 0 : 1);
            $sql .= "=1 or ug_nome = '" . SQLaddFields($filtro['ug_nome'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_nomeLike']) && !is_null($filtro['ug_nomeLike']) ? 0 : 1);
            $sql .= "=1 or ug_nome like '%" . SQLaddFields($filtro['ug_nomeLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_cpf']) && !is_null($filtro['ug_cpf']) ? 0 : 1);
            $sql .= "=1 or ug_cpf = '" . SQLaddFields($filtro['ug_cpf'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_rg']) && !is_null($filtro['ug_rg']) ? 0 : 1);
            $sql .= "=1 or ug_rg = '" . SQLaddFields($filtro['ug_rg'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_data_nascimentoMin']) && isset($filtro['ug_data_nascimentoMax']) && !is_null($filtro['ug_data_nascimentoMin']) && !is_null($filtro['ug_data_nascimentoMax']) ? 0 : 1);
            $sql .= "=1 or ug_data_nascimento between " . SQLaddFields($filtro['ug_data_nascimentoMin'], "") . " and " . SQLaddFields($filtro['ug_data_nascimentoMax'], "") . ")";
        
            $sql .= " and (" . (isset($filtro['ug_sexo']) && !is_null($filtro['ug_sexo']) ? 0 : 1);
            $sql .= "=1 or ug_sexo = '" . SQLaddFields($filtro['ug_sexo'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_tipo_end']) && !is_null($filtro['ug_tipo_end']) ? 0 : 1);
            $sql .= "=1 or ug_tipo_end = '" . SQLaddFields($filtro['ug_tipo_end'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_endereco']) && !is_null($filtro['ug_endereco']) ? 0 : 1);
            $sql .= "=1 or ug_endereco = '" . SQLaddFields($filtro['ug_endereco'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_enderecoLike']) && !is_null($filtro['ug_enderecoLike']) ? 0 : 1);
            $sql .= "=1 or ug_endereco like '%" . SQLaddFields($filtro['ug_enderecoLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_numero']) && !is_null($filtro['ug_numero']) ? 0 : 1);
            $sql .= "=1 or ug_numero = '" . SQLaddFields($filtro['ug_numero'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_complemento']) && !is_null($filtro['ug_complemento']) ? 0 : 1);
            $sql .= "=1 or ug_complemento = '" . SQLaddFields($filtro['ug_complemento'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_bairro']) && !is_null($filtro['ug_bairro']) ? 0 : 1);
            $sql .= "=1 or ug_bairro = '" . SQLaddFields($filtro['ug_bairro'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_bairroLike']) && !is_null($filtro['ug_bairroLike']) ? 0 : 1);
            $sql .= "=1 or ug_bairro like '%" . SQLaddFields($filtro['ug_bairroLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_cidade']) && !is_null($filtro['ug_cidade']) ? 0 : 1);
            $sql .= "=1 or ug_cidade = '" . SQLaddFields($filtro['ug_cidade'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_cidadeLike']) && !is_null($filtro['ug_cidadeLike']) ? 0 : 1);
            $sql .= "=1 or ug_cidade like '%" . SQLaddFields($filtro['ug_cidadeLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_estado']) && !is_null($filtro['ug_estado']) ? 0 : 1);
            $sql .= "=1 or ug_estado = '" . SQLaddFields($filtro['ug_estado'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_cep']) && !is_null($filtro['ug_cep']) ? 0 : 1);
            $sql .= "=1 or ug_cep = '" . SQLaddFields($filtro['ug_cep'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_tel_ddi']) && !is_null($filtro['ug_tel_ddi']) ? 0 : 1);
            $sql .= "=1 or ug_tel_ddi = '" . SQLaddFields($filtro['ug_tel_ddi'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_tel_ddd']) && !is_null($filtro['ug_tel_ddd']) ? 0 : 1);
            $sql .= "=1 or ug_tel_ddd = '" . SQLaddFields($filtro['ug_tel_ddd'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_tel']) && !is_null($filtro['ug_tel']) ? 0 : 1);
            $sql .= "=1 or ug_tel = '" . SQLaddFields($filtro['ug_tel'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_telLike']) && !is_null($filtro['ug_telLike']) ? 0 : 1);
            $sql .= "=1 or ug_tel like '%" . SQLaddFields($filtro['ug_telLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_cel_ddi']) && !is_null($filtro['ug_cel_ddi']) ? 0 : 1);
            $sql .= "=1 or ug_cel_ddi = '" . SQLaddFields($filtro['ug_cel_ddi'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_cel_ddd']) && !is_null($filtro['ug_cel_ddd']) ? 0 : 1);
            $sql .= "=1 or ug_cel_ddd = '" . SQLaddFields($filtro['ug_cel_ddd'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_cel']) && !is_null($filtro['ug_cel']) ? 0 : 1);
            $sql .= "=1 or ug_cel = '" . SQLaddFields($filtro['ug_cel'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_celLike']) && !is_null($filtro['ug_celLike']) ? 0 : 1);
            $sql .= "=1 or ug_cel like '%" . SQLaddFields($filtro['ug_celLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_habbo_id']) && !is_null($filtro['ug_habbo_id']) ? 0 : 1);
            $sql .= "=1 or ug_habbo_id = '" . SQLaddFields($filtro['ug_habbo_id'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_habbo_idLike']) && !is_null($filtro['ug_habbo_idLike']) ? 0 : 1);
            $sql .= "=1 or ug_habbo_id like '%" . SQLaddFields($filtro['ug_habbo_idLike'], "r") . "%')";
        
            $sql .= " and (" . (isset($filtro['ug_news_letter']) && !is_null($filtro['ug_news_letter']) ? 0 : 1);
            $sql .= "=1 or ug_news = '" . SQLaddFields($filtro['ug_news_letter'], "r") . "')";
        
            $sql .= " and (" . (isset($filtro['ug_news_letterLike']) && !is_null($filtro['ug_news_letterLike']) ? 0 : 1);
            $sql .= "=1 or ug_news like '%" . SQLaddFields($filtro['ug_news_letterLike'], "r") . "%')";
        
            // Competicao
            $sql .= " and (" . (isset($filtro['ug_compet_lh_ug_id']) && !is_null($filtro['ug_compet_lh_ug_id']) ? 0 : 1);
            $sql .= "=1 or ug_compet_lh_ug_id = " . SQLaddFields($filtro['ug_compet_lh_ug_id'], "") . ")";
        
            // Pagamento com Cielo
            $sql .= " and (" . (isset($filtro['ug_use_cielo']) && !is_null($filtro['ug_use_cielo']) ? 0 : 1);
            $sql .= "=1 or ug_use_cielo = " . SQLaddFields($filtro['ug_use_cielo'], "") . ")";
        }
        

		if(!is_null($orderBy)) $sql .= " order by " . $orderBy;
        fwrite($file, "SQLexecuteQuery222: ".$sql."\n");
        fwrite($file, str_repeat("*", 50)."\n");
        $result = SQLexecuteQuery($sql);
        $rs_row = pg_fetch_array($result);
        fwrite($file, "feliperesult222: ".json_encode($rs_row)."\n");
        fclose($file);
    ...
    function obter($filtro, $orderBy, &$rs) {
        ...
        $file = fopen("/www/log/felipe.txt", "a+");
			...
			fwrite($file, str_repeat("*", 50)."\n");
			fwrite($file, "DATA: ".date("d-m-Y H:i:s")."\n");
			fwrite($file, "Filtro recebido: ".json_encode($filtro)."\n");
        ...
        if (!is_null($filtro) && $filtro != "") {
            ...
            if (!is_null($filtro['ug_data_inclusaoMin']) && !is_null($filtro['ug_data_inclusaoMax'])) {
                $filtro['ug_data_inclusaoMin'] = formata_data_ts($filtro['ug_data_inclusaoMin'] . " 00:00:00", 1, true, true);
                $filtro['ug_data_inclusaoMax'] = formata_data_ts($filtro['ug_data_inclusaoMax'] . " 23:59:59", 1, true, true);
            }

            if (!is_null($filtro['ug_data_ultimo_acessoMin']) && !is_null($filtro['ug_data_ultimo_acessoMax'])) {
                $filtro['ug_data_ultimo_acessoMin'] = formata_data_ts($filtro['ug_data_ultimo_acessoMin'] . " 00:00:00", 1, true, true);
                $filtro['ug_data_ultimo_acessoMax'] = formata_data_ts($filtro['ug_data_ultimo_acessoMax'] . " 23:59:59", 1, true, true);
            }

            if (!is_null($filtro['ug_data_nascimentoMin']) && !is_null($filtro['ug_data_nascimentoMax'])) {
                $filtro['ug_data_nascimentoMin'] = formata_data_ts($filtro['ug_data_nascimentoMin'] . " 00:00:00", 1, true, true);
                $filtro['ug_data_nascimentoMax'] = formata_data_ts($filtro['ug_data_nascimentoMax'] . " 23:59:59", 1, true, true);
            }
		...
		if(!is_null($orderBy)) $sql .= " order by " . $orderBy;
        fwrite($file, "SQLexecuteQuery: ".$sql."\n");
        fwrite($file, str_repeat("*", 50)."\n");
        $result = SQLexecuteQuery($sql);
        $rs_row = pg_fetch_array($result);
        fwrite($file, "feliperesult: ".json_encode($rs_row)."\n");
        fclose($file);
		//if(!$rs) return $ret = "Erro ao obter usu�rio(s).\n";
        return $result;
...
//gravaLog_Temporario("SQL UsuarioGames::existeEmail('$email0' -> '$email', ".(($usuario_id_excessao)?"TRUE":"FALSE").") => ".(($ret)?"TRUE":"FALSE")." (qtde: ".$rs_row['qtde'].")\n  ".$sql."\n");
	...
	public static function existeCPFCadastro($cpf) {
        ...
        try {
		    ...
		    if ($info != false) {
                // Cadastro de usuarios: aqui valida a quantidade de usu�rios criados, antes era 2 e agora � um.
...
//echo "$sql<br>";
    ...
    function existeCPF($cpf, $usuario_id_excessao) {
        ...
        try{
            ...
            // Se o userId for fornecido, adicionar uma condi��o para excluir esse ID da verifica��o
            ...
            // Se o userId for fornecido, bindar o par�metro do ID tamb�m
            if ($usuario_id_excessao !== null) {
...
//echo $sql."<br>";
    ...
    /* Wagner -- M�todos novos login integra��o */
    function autenticarLoginIntegracao($login, $senha) {
        ...
        if ($ret) {
            //testar se � integracao retonando false caso negativo
    ...
    /* Wagner -- fim bloco -- M�todos novos login integra��o */
    ...
    function getUsuarioGamesById($usuario_id) {
        ...
        fwrite($file, str_repeat("*", 50)."\n");
        fwrite($file, "DATA: ".date("d-m-Y H:i:s")."\n");
        fwrite($file, "getUsuarioGamesById: ".json_encode($ret)."\n");
        fwrite($file, str_repeat("*", 50)."\n");
        fclose($file);
    ...
    function getUsuarioGamesByLogin($login) {
        ...
        fwrite($file, str_repeat("*", 50)."\n");
        fwrite($file, "DATA: ".date("d-m-Y H:i:s")."\n");
        fwrite($file, "getUsuarioGamesByLogin: ".json_encode($ret)."\n");
        fwrite($file, str_repeat("*", 50)."\n");
        fclose($file);
    ...
    function adicionarLoginSession($login) {
        ...
        fwrite($file, str_repeat("*", 50)."\n");
        fwrite($file, "DATA: ".date("d-m-Y H:i:s")."\n");
        fwrite($file, "adicionarLoginSession: ".json_encode($ret)."\n");
        fwrite($file, str_repeat("*", 50)."\n");
        fclose($file);
...
//			$usuarioGames->setPerfilSaldo($objEncryption->decrypt(trim($rs_row['ug_perfil_saldo'])));
            ...
            //Buscando Informa��es de observa��es
    ...
    function adicionarLoginSession_ByID($ug_id) {
        ...
        $filtro['ug_ativo'] = 1;
        $instUsuarioGames = new UsuarioGames();
        $result = $instUsuarioGames->obterdadosteste($filtro, null);
        // $ret = (new UsuarioGames)->obter($filtro, null, $rs);
        $file = fopen("/www/log/felipe.txt", "a+");
       
        fwrite($file, str_repeat("*", 50)."\n");
        fwrite($file, "DATA: ".date("d-m-Y H:i:s")."\n");
        fwrite($file, "ug_id : ".$ug_id."\n");


        // Verifique se o resultado � um array e se cont�m dados
        if ($result && count($result) > 0) {
            fwrite($file, "adicionarLoginSession_ByID: " . json_encode($result) . "\n");
            
        } else {
            fwrite($file, "adicionarLoginSession_ByID: Nenhum resultado retornado.\n");
        }
        ...
        if ($result && count($result) > 0) {
            fwrite($file, "entrou no if\n");
            $ret = true;
            $rs_row = $result;
            
            fwrite($file, "nome".$rs_row['ug_nome']."\n");
        ...
        if ($ret) {
            ...
            if ($ret) {
                ...
                /*
                  ...
                  <tr valign='middle' bgcolor='#FFFFFF'>
                  <td align='left' class='texto'>
                  Voc� acessou nosso site e alterou sua senha.<br><br>
                  ...
                  ";
                  $msgEmail .= email_rodape($parametros);
                  enviaEmail($usuarioGames->getEmail(), null, null, "E-Prepag - Altera��o de Senha", $msgEmail);
                 */
...
function b_IsLogin_pagamento_pin_Personalizado() {
    ...
    // Para bloquear o uso de PINs EPP Cash por usu�rios (usar jnto com b_pin_forma_pagamento())
...
//grava_log_integracao("Integra��o Debug 5: ".date("Y-m-d H:i:s")."\n  $sql \n");
            $rs = SQLexecuteQuery($sql);
        }
    }
...
// retorna true -> Usa Ita�
// retorna false -> Usa Bradesco
function b_Is_Boleto_Express_Itau($email, $opr_codigo) {

    // Libera Ita� para todos os Express Money em 2011-11-11
    return false;
    /*
      // Se n�o for Habbo -> usa boleto Itau
      if($opr_codigo!=16) {
      return true;
      }
      return false;
    ...
    /*
      ...
      // Se n�o for da lista -> usa boleto Itau
    ...
    /*
      ...
      return false;
     */
    // Libera para todos os usu�rios, again, em 2011-06-27
    //	return false;

}

// Fun��o que Habilita/Desabilita Boleto SANTANDER
...
// Fun��o que Habilita/Desabilita Boleto Bradesco
...
function b_pin_forma_pagamento($bloqueio = true) {
    ...
    /*
      // Para desabilitar a op��o de pagamento EPP cash -> libera o retorno false
      return false;
     */

    /*
      // Para liberar a op��o de pagamento EPP Cash apenas para estes usu�rios => libera este bloco
      $usuarioGames = unserialize($GLOBALS['_SESSION']['usuarioGames_ser']);
      if(is_object($usuarioGames)) {
