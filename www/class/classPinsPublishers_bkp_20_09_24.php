				...
				if(!$rs_pins || pg_num_rows($rs_pins) == 0) {
					...
					if(!$rs_pins_store || pg_num_rows($rs_pins_store) == 0) {
						...
						if($msg == ""){
							...
							//$sql = "insert into pins (pin_serial, pin_codigo, opr_codigo, pin_valor, pin_lote_codigo, pin_dataentrada, pin_canal, pin_horaentrada,pin_status,pin_validade) values ('".$spin_serial."', '".base64_encode($this->aesPub->encrypt($spin_codigo))."', ".$opr_codigo.", ".$pin_valor.", ".$ilote.", CURRENT_TIMESTAMP, 'w', NOW(),'1',(NOW() + interval '6 month'));";
							//die("SQL : ".$sql);
