...
class FeedWP extends Json
{
    ...
    private $_imagesDir = "/www/arquivos_gerados/imagens/blog/";
    ...
    public function generate($urlJsonBlog)
    {
        ...
        try {
            ...
            $geraLog = new Log("FEEDRSSWP", array(
                "ERROR: " . $ex->getMessage(),
                "FILE: " . $ex->getFile(),
                "LINE " . $ex->getLine()
            ));
    ...
    {
        try {
            $arrImg = []; // Array para guardar imagens em uso

            // 1. Obter o JSON do banco de dados
            $content = $this->getJsonRecursive(); // Este método agora vem do DB

            if ($content) {
                foreach ($content as $ind => $val) {
                    // Acessa como objeto (->) ou array (['src'])?
                    // Vou assumir objeto baseado no seu código original
                    if (isset($val->src[0])) {
            ...
            $logArquivos = "";
            $t = false; // Variável de controle

            while (($arquivo = $diretorio->read()) !== false) {

                if (!in_array($arquivo, $arrImg) && $arquivo != "." && $arquivo != "..") {

                    if (unlink($this->_imagesDir . $arquivo)) {
                        $t = true;
                        $logArquivos .= "Arquivo " . $this->_imagesDir . $arquivo . " removido com sucesso.<br>" . PHP_EOL;
                    } else {
                        $t = true;
                        $logArquivos .= "Erro ao remover arquivo " . $this->_imagesDir . $arquivo . ".<br>" . PHP_EOL;
                    }
                }
            }

            if ($t) { // Verificando se $t foi setado para true
                echo $logArquivos;
            } else {
