 ...
 * de posição fixa (ex.: De coluna 15 até 25 contém o campo Nome)
 *
 */
class FilePosition {
	...
	function __construct($fileName, $dir = null) {
		$this->setFileName(strtolower($fileName));
                $this->setDir($dir);
	}
	...
	function setVetorHeader($vetorHeader) {
                $tmp = "";                  // variável contendo dados temporários
                $this->cleanFile();         // Limpando a variável File
 	       $this->vetorHeader = $vetorHeader;
                // cada posição do vetor deve conter a subestrutura [key][name] e [key][size]
                foreach ($this->vetorHeader as $key => $dados) {
                    if($this->checkNumber($dados['name'])){
                        $tmp .= $this->format_number($dados['name'], $dados['size']);
                    }
                    else {
                        //para não converter automaticamente a string para maiusculo deve ser informado o parametro 'upper' no vetor dados como 'nao'
                        $tmp .= $this->format_string($dados['name'], $dados['size'],((isset($dados['upper']))?FALSE:TRUE));
                    }
                }//end foreach
                $this->setLine($tmp);
	}

	function getVetorHeader(){
            return $this->vetorHeader;
        }
	...
	function format_string($name, $size, $toUpper) {
                if($toUpper) {
		...
		$ret = str_pad(substr(trim($name),0,$size), $size, " ", STR_PAD_RIGHT);
		return $ret;
	}

        //Função para preencher com zeros os números que não completam o valor exigido.
	function format_number($number, $size) {
		$ret = str_pad(substr(trim($number),0, $size), $size, "0", STR_PAD_LEFT);
		return $ret;
	}
		...
		//Arquivo
                if(is_null($this->getDir())) {
                    $file = $GLOBALS['raiz_do_projeto'] . 'backoffice/bacen/';
                }
                else {
                    $file = $this->getDir();
                }
