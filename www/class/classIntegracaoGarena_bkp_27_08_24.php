...
class Garena{
	...
	private function geraHash($conteudo, $ambiente){
		...
		if($ambiente == "producao"){
			$chave = "8SURPwtTqE48px9NkKfDTt2syvbkUEE7Udfp";
		}else{
			$chave = "YSXGf8y9L4L7W2tF2dbhgTkB3Apxs5V2fLQc";
		}
	...
	public function chamaGarena($metodo, $ambiente = "homologacao"){
		...
		if($metodo == "GET"){	
			...
			$this->salvaLog($resultado, "valida", $this->conta, $this->idPin);
			...
			$hashSha256 = $this->geraHash([$this->codGarena, (integer)$this->conta, $this->rolesGarena[0]["code"], $guid, intval($this->valorResgate * 100), $currecy, $ip], $ambiente);
			...
			curl_setopt_array($disparo, [
				...
				CURLOPT_POST => true,
				CURLOPT_TIMEOUT => 150,
			    CURLOPT_POSTFIELDS => json_encode(["test_mode"=> $test, "app_id" => $this->codGarena, "player_id"=> (integer)$this->conta, "packed_role_id" => $this->rolesGarena[0]["code"], "txn_id" => $guid, "amount" => intval($this->valorResgate * 100), "currency_code" => $currecy, "ip_address" => $ip]),
				CURLOPT_HTTPHEADER => [
					"Authorization: Signature ".$hashSha256
				],
			...
			$resultado = curl_exec($disparo);
			$this->salvaLog($resultado, "credito", $this->conta, $this->idPin);
			if($resultado == null || $resultado == "" || $resultado == false){
				//$this->destravaProcesso();
		...
		$validação = $this->verificaProcesso();
		return $validação;
	...
	private function salvaLog($conteudo, $type, $conta, $pin){
