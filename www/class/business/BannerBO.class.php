...
class BannerBO extends BannerDAO
{
    ...
    {
        global $raiz_do_projeto;
        // Garante que o nome n�o contenha caminhos (../)
        $safe_filename = basename($file["name"]);

        $image_info = @getimagesize($file["tmp_name"]);

        if ($image_info === false) {
            $this->erros[] = "Arquivo inv�lido. O arquivo n�o � uma imagem suportada.";
            return false;
        }

        // Usamos a informa��o de MIME que o getimagesize() nos deu.
    ...
    private function validaImagem($file)
    {
        ...
        if (!in_array($ext[0], $this->formatos))
            $this->erros[] = "Formato de imagem inv�lido.";

        if (file_exists($raiz_do_projeto . $this->pasta . $file["name"]))
            $this->erros[] = "J� existe uma imagem com esse nome, por favor, renomeie-a e tente novamente.";
            ...
            foreach ($categorias as $categoria) {
                ...
                if (!empty($banners)) {
                    ...
                    if (!isset($arrBanners)) {
                        ...
                        echo "N�o est� setado arrBanners" . PHP_EOL;
                    }
    ...
    public function getBannersFromJson($posicao, $categoria, $currJsonFile = 1)
    {
        try {
            ...
            if ($json) {
                // getJsonRecursive() j� retorna o objeto/array decodificado
                $posicao = html_entity_decode($posicao);
                return isset($json->$posicao) ? $json->$posicao : false;
            } else {
                // Se $json for false, o DB n�o encontrou.
                // Lan�amos uma exce��o para acionar a l�gica de fallback (tentar -2.json, -3.json)
                throw new Exception("JSON '$nomeJson' n�o encontrado no banco.");
            }
        } catch (Exception $ex) {
            $this->erros[] = $ex->getMessage();
            ...
            // setando qual arquivo json ser� usado em caso de erro
            $currJsonFile++;

            // A l�gica de fallback � mantida
            if ($currJsonFile <= 3) {
