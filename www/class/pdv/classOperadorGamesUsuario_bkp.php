...
//echo "ret3: ".(($ret)?"ret OK":"Not ret")."<br>";
    ...
    function atualiza_ultimo_acesso($login) {
            ...
            //------------------------------------------------------------------
            if($login){
				$sql = "UPDATE dist_usuarios_games_operador 
				SET ugo_data_ultimo_acesso = CURRENT_TIMESTAMP, 
					ugo_qtde_acessos = ugo_qtde_acessos + 1 
				WHERE ugo_login = :login";
				$con = ConnectionPDO::getConnection();
				$pdo = $con->getLink();
				$stmt = $pdo->prepare($sql);
