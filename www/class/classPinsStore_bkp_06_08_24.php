				...
				if(!$rs_pins || pg_num_rows($rs_pins) == 0) {
					...
					if(!$rs_pins_pub || pg_num_rows($rs_pins_pub) == 0) {
						...
						if(!$rs_pins_gocash || pg_num_rows($rs_pins_gocash) == 0) {
							//transacao
							if($msg == ""){
								$sql = "insert into pins_store (pin_serial, pin_codigo, pin_caracter, distributor_codigo, pin_valor, pin_lote_codigo, pin_dataentrada, pin_canal, pin_formato) values ('".$spin_serial."', '".base64_encode($this->aes->encrypt($spin_codigo))."', '', ".$distributor_codigo.", ".$pin_valor.", ".$ilote.", CURRENT_TIMESTAMP, 'w', '".$sformato."');";
