		...
		while ($i <= $qtde) {
			...
			if ($aux_spin_codigo == $spin_codigo) {
				...
				if(pg_num_rows($rs_pins) == 0) {
                                        //transacao
                                        if($msg == ""){
                                                $sql = "insert into pins_card (pin_serial, pin_codigo, opr_codigo, distributor_codigo, pin_valor, pin_lote_codigo, pin_dataentrada, pin_formato) values ('".$spin_serial."', '".base64_encode($this->aes->encrypt($spin_codigo))."', ".$opr_codigo.", ".$distributor_codigo.", ".$pin_valor.", ".$ilote.", CURRENT_TIMESTAMP, '".$sformato."');";
