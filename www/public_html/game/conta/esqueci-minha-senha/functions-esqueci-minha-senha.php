...
require_once __DIR__ . '/configuracoes-email.php';
...
function registraNovaSolicitacao($dadosUsuario, $codigoValidacao, $timestamp)
{
	...
	$tabela = 'tb_esqueci_minha_senha_gamer';
...
function enviaEmailParaValidarSolicitacao($dadosUsuario, $codigoValidacao)
{
	...
	$body_html = "<html><head><title>Esqueci Minha Senha</title></head><body><h1>Olá, {$dadosUsuario['primeiroNomeUsuario']}</h1><p>Você poderá cadastrar uma nova senha clicando no link abaixo:</p><a target='_blank' href='" . EPREPAG_URL_HTTPS . "/game/conta/esqueci-minha-senha/verifica-solicitacao.php?redirected=true&codigoValidacao={$codigoValidacao}&origemUsuario={$dadosUsuario['origemUsuario']}'>Mudar a Senha</a><p>Caso você não tenha feito essa solicitação, ignore o link acima e nos avise: <a href='" . EPREPAG_URL_HTTPS . "/game/suporte.php' target='_blank' title='Entre em contato conosco'>aqui</a>.</p></body></html>";

	$body_html = mb_convert_encoding($body_html, 'UTF-8', 'ISO-8859-1');
...
function verificaCodigoValidacao($codigoValidacao)
{
	...
	$sql = "select * from tb_esqueci_minha_senha_gamer where codigo_verificacao = :CODE ;";
	$query = $conexao->prepare($sql);
...
function expiraSolicitacao($codigoValidacao)
{
	...
	$sql = "update tb_esqueci_minha_senha_gamer set status = :STATUS where codigo_verificacao = :CODE ;";
	$query = $conexao->prepare($sql);
	$query->bindValue(':CODE', $codigoValidacao);
...
function registraNovaSenha($novaSenhaCriptografada, $idUsuario)
{
	...
	$sql = "update usuarios_games set ug_senha = :NOVA_SENHA where ug_id = :ID ;";
	$query = $conexao->prepare($sql);
...
function atualizaHistoricoCliente($idUsuario, $idIp, $idDataTrocaSenha)
{
	...
	$sql = "insert into usuarios_games_log(ugl_data_inclusao, ugl_ip, ugl_uglt_id, ugl_ug_id) values(:DATA, :IP, :STATUS, :ID) ;";
	$query = $conexao->prepare($sql);
	$query->bindValue(':ID', $idUsuario);
