...
// normalizar/validar valores
$userId         = (int) $controller->usuario->getId();
$start          = Util::getData($data_inicio, true) . ' 00:00:00';
$end            = Util::getData($data_fim, true) . ' 23:59:59';
$label_deposito = (string) $label_deposito;
$label_compra   = (string) $label_compra;
...
FROM (
    (
        ...
        WHERE ug_id = ? AND pta_data >= ? AND pta_data <= ?
    )
    ...
    (
        SELECT
            ...
            0 as saldo
        FROM tb_pag_estorno 
        WHERE ug_id = ? AND tpe_data >= ? AND tpe_data <= ?
    )
    ...
    (
        ...
        WHERE ug_id = ? AND scf_data_deposito >= ? AND scf_data_deposito <= ?
    )
    ...
    (
        ...
        WHERE vg_ug_id = ? AND vg_pagto_data >= ? AND vg_pagto_data <= ?
        ...
        <div class="row">
            <div class="col-md-3 txt-azul-claro">
                <span class="glyphicon glyphicon-triangle-right graphycon-big pull-left" aria-hidden="true"></span><strong class="pull-left">
                    <h4 class="top20">Cartão E-Prepag</h4>
                </strong>
            </div>
        </div>
                                        ...
                                        <td class="text-right">
                                            ...
                                            if ($value['saldo'] > 0) {
                                                ...
                                                $sql = "
                                                        ...
                                                        WHERE ugsl_data_inclusao >= ?::timestamp - '1 second'::interval 
                                                          AND ugsl_data_inclusao <= ?::timestamp + '1 second'::interval 
                                                          AND ugsl_ug_id = ?;
                                                        ";
