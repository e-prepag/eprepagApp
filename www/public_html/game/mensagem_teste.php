...
require_once DIR_CLASS ."gamer/controller/HeaderController_teste.class.php";
...
<div class="container txt-azul-claro bg-branco p-bottom40">
    <div class="col-md-12 top20 txt-vermelho">
        <?php echo str_replace("\n", "<br>", htmlspecialchars($msg, ENT_NOQUOTES));?>
    </div>
...
<script>
    manipulaModal(1,'<?php echo str_replace("\n","<br>",htmlspecialchars($msg, ENT_NOQUOTES));?>','<?php echo htmlspecialchars($titulo, ENT_NOQUOTES);?>');
<?php 
    if(!empty($link)){ 
?>
    $('#modal-load').on('hidden.bs.modal', function () { location.href='<?php echo htmlspecialchars($link, ENT_NOQUOTES);?>' });
<?php
    } 
?>
</script>
