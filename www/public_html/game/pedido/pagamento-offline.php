...
<div class="container txt-cinza bg-branco  p-bottom40">
    <div class="row top10">
        <div class="col-md-12 txt-verde top10">
            <span class="glyphicon glyphicon-triangle-right graphycon-big pull-left" aria-hidden="true"></span><strong class="pull-left"><h4 class="top20">acesse sua conta</h4></strong>
        </div>
            ...
            <form id="formLogin" method="post">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="form-group top10">
                                        <label for="senha">E-mail do cadastro:</label>
                                        <input type="text" class="form-control" id="emailEsqueciSenha" onpaste="return false;" autocomplete="new-password" name="emailEsqueciSenha" placeholder="Digite seu e-mail de cadastro">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <div class="alert alert-danger" role="alert"> 
                                      <h5><span id="msg-modal">Dados em destaque estão incorretos.</span> Por favor, confira e tente novamente.</h5>
                                  </div>
                                </div>
                                <div class="modal-footer">
                                  <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
                ...
                <div class="row top10 form-group">
                    <div class="col-md-12 dislineblock">
                        <button id="prosseguir" class="pull-right btn btn-success">Prosseguir</button>
                    </div>
                    <div class="top10 col-md-12 dislineblock">
                        <a href="/game/conta/nova.php" class="pull-right btn btn-info"><strong>Não tem cadastro?</strong></a>
                    </div>
                </div>
            </form>
        </div>
...
<?php
if($permitidoBoletoExpress && FALSE) {
?>        
        <div class="col-md-6">
            <form id="boletoExpress" action="boleto-express.php" method="post">
                <div class="col-md-12 txt-azul-claro">
                    <div class="form-group col-md-10">
                        <h4>Comprar por Boleto Express</h4>
                        <h5 class="txt-cinza">Não é necessário cadastro, cartão ou conta bancária.</h5>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group col-md-6">
                        <input class="form-control" placeholder="E-mail" onpaste="return false;" autocomplete="off" id="email" name="email" type="text" value="">
                    </div>
                </div>
                <div class="col-md-12">    
                    <div class="form-group col-md-6">
                        <input class="form-control" placeholder="Confirmar e-mail" onpaste="return false;" autocomplete="off" id="confirma_email" name="confirma_email" type="text" value="">
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group col-md-6">
                        <button id="btnBoletoExpress" class="btn btn-success">Confirmar pedido por Boleto Express</button>
                    </div>
                </div>
                <div class="col-md-4 col-xs-6 col-sm-12  text-left">
                    <img class="p-left15" src="/imagens/boleto_express.jpg" title="Formas de Pagamento">
                </div>
                <div class="col-md-8">
<?php
            if($_POST['totalCarrinho'] < $GLOBALS['RISCO_GAMERS_VALOR_MIN_PARA_TAXA']) {
...
?>
                    <div class="txt-cinza-claro2 text-left">
<?php                
                ...
                $.ajax({
                    ...
                    success: function(ret){
                        ...
                        if(ret == <?php echo RETURN_SUCCESS; ?> ){
                            window.location.href= "passo-2.php";
                        }else if(ret == <?php echo RETURN_WRONG;?> || ret == <?php echo RETURN_EMPTY;?>){
                            erro = true;
                            $("#modal-msg").text("Usuário ou senha inválidos ou não encontrados.");
                            $("label[for='login']").addClass("txt-vermelho");
                            $("label[for='senha']").addClass("txt-vermelho");
                            $('#myModal').modal('show');
                            return false;
...
?> 
       ...
       $("#reenviarSenha").click(function(){
           waitingDialog.show('Por favor, aguarde...',{dialogSize: 'sm'});
           var erro = "";
           if(checkEmail($("#emailEsqueciSenha").val())){
               $.ajax({
                    type: "POST",
                    dataType: "JSON",
                    url: "/game/ajax/dados-acesso.php",
                    data: {type: "esqueciMinhaSenha", email: $("#emailEsqueciSenha").val(), senha: true },
                    success: function(obj){
                        ...
                        if(obj.erro.length > 0){
                            ...
                            $("#modal-esqueci-senha").modal('hide');
                            manipulaModal(2,"Um e-mail foi enviado para "+$("#emailEsqueciSenha").val()+" com a sua senha.","Sucesso");
                            return false;
                        }
                    },
                    error: function(){
...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 bg-info align-center borda-direita-basica p-bottom10">
        <h3>
            ...
            <a href="/creditos/cadastro.php" class="txt-branco" target="_blank"><b><span class="link-destaque">Cadastre-se</span></b></a>
            ou 
