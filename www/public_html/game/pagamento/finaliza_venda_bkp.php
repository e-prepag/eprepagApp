...
if(empty($strRedirect)){
        ...
        $ret = SQLexecuteQuery($sql);
        if(!$ret) $ret = "Erro ao inserir venda.";
        else{
...
if(empty($strRedirect)) {
    ...
    */
            ...
            if ($total_carrinho < $RISCO_GAMERS_VALOR_MIN_PARA_TAXA) $taxa_adicional = $GLOBALS['BOLETO_MONEY_BRADESCO_TAXA_ADICIONAL'];
            else $taxa_adicional = 0;
            $bco_codigo = $GLOBALS['BOLETO_MONEY_BRADESCO_COD_BANCO'];
            // Usa Boleto Itau para alguns usu�rios
            if($controller->logado){
                    if($controller->usuario->b_Is_Boleto_Bradesco()) {
                        if ($total_carrinho < $GLOBALS['RISCO_GAMERS_VALOR_MIN_PARA_TAXA']) $taxa_adicional = $GLOBALS['BOLETO_MONEY_BRADESCO_TAXA_ADICIONAL'];
                            else $taxa_adicional = 0;
                        $qtde_dias_venc = $GLOBALS['BOLETO_MONEY_BRADESCO_QTDE_DIAS_VENCIMENTO'];
                        $bco_codigo = $GLOBALS['BOLETO_MONEY_BRADESCO_COD_BANCO'];
                        $num_doc = "2" . "00" . str_pad($venda_id, 8, "0", STR_PAD_LEFT);
                    ...
                    elseif($controller->usuario->b_Is_Boleto_Banespa()) {
                            ...
                            if ($total_carrinho < $RISCO_GAMERS_VALOR_MIN_PARA_TAXA) $taxa_adicional = $GLOBALS['BOLETO_MONEY_BANESPA_TAXA_ADICIONAL'];
                            else $taxa_adicional = 0;
                    ...
                    elseif($controller->usuario->b_Is_Boleto_Itau()) {
                            ...
                            if ($total_carrinho < $RISCO_GAMERS_VALOR_MIN_PARA_TAXA) $taxa_adicional = $GLOBALS['BOLETO_MONEY_ITAU_TAXA_ADICIONAL'];
                            else $taxa_adicional = 0;
                    }
            } 
            ...
            if($ret){
                    ...
                    $ret = SQLexecuteQuery($sql);
                    if(!$ret) $ret = "Erro ao atualizar status da venda.\n";
                    else $ret = ""; //limpa resourceId
            }
