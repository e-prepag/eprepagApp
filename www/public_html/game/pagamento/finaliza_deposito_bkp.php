...
function depositoBoleto($total_geral, $usuarioId) {
    ...
    if($msg == ""){
            ...
            if ($total_geral < $GLOBALS['RISCO_GAMERS_VALOR_MIN_PARA_TAXA']) $taxa_adicional = $GLOBALS['BOLETO_MONEY_BRADESCO_TAXA_ADICIONAL'];
            ...
            if($controller->logado){
                    ...
                    elseif($controller->usuario->b_Is_Boleto_Banespa()) {
                            ...
                            if ($total_geral < $GLOBALS['RISCO_GAMERS_VALOR_MIN_PARA_TAXA']) $taxa_adicional = $GLOBALS['BOLETO_MONEY_BANESPA_TAXA_ADICIONAL'];
                            else $taxa_adicional = 0;
                    ...
                    elseif($controller->usuario->b_Is_Boleto_Itau()) {
                            ...
                            if ($total_geral < $GLOBALS['RISCO_GAMERS_VALOR_MIN_PARA_TAXA']) $taxa_adicional = $GLOBALS['BOLETO_MONEY_ITAU_TAXA_ADICIONAL'];
                            else $taxa_adicional = 0;
                    }
            } 
            ...
            if($ret){
                    ...
                    $ret = SQLexecuteQuery($sql);
                    if(!$ret) $msg = "Erro ao atualizar status da venda (3223).\n";
            } 
    } else {
