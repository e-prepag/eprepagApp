...
class RecebePix
{
	...
	private function atualizaVenda($idVenda)
	{
        ...
        ";
			...
			$stmt = $this->conexao->prepare($sqlSelecao);
			...
			if ($stmt->rowCount() > 0) {
				...
				$numDocto = "PIXR_" . date("YmdHis") . "591";
				$dataConc = $data_venda . ".431227";
				$dataPagto = $data_venda . ".097958";
				$userIdConcilia = '0401121156014';
				$banco = '400';
				$ultimoStatus = 5;
                ...
                SET vg_ultimo_status_obs = '',
                    ...
                    vg_ultimo_status = :ultimoStatus,
                    vg_pagto_data = :dataPagto,
                    vg_pagto_banco = :banco,
                    vg_pagto_num_docto = :numDocto,
                    vg_concilia = 1,
                    vg_data_concilia = :dataConc,
                    vg_user_id_concilia = :userIdConcilia,
                    vg_pagto_valor_pago = :valor
                WHERE vg_id = :idVenda
            ";

				$stmtUpd = $this->conexao->prepare($sqlAtualizaVenda);
				$stmtUpd->bindParam(':data_venda', $data_venda, PDO::PARAM_STR);
				$stmtUpd->bindParam(':ultimoStatus', $ultimoStatus, PDO::PARAM_INT);
				$stmtUpd->bindParam(':dataPagto', $dataPagto, PDO::PARAM_STR);
				$stmtUpd->bindParam(':banco', $banco, PDO::PARAM_STR);
				$stmtUpd->bindParam(':numDocto', $numDocto, PDO::PARAM_STR);
				$stmtUpd->bindParam(':dataConc', $dataConc, PDO::PARAM_STR);
				$stmtUpd->bindParam(':userIdConcilia', $userIdConcilia, PDO::PARAM_STR);
				$stmtUpd->bindParam(':valor', $valor, PDO::PARAM_STR);
				$stmtUpd->bindParam(':idVenda', $idVenda, PDO::PARAM_INT);
				$stmtUpd->execute();
