...
class RecebePix
{
	...
	{
		try {
			$tableName = ($this->ambiente == "PDV")
				? "tb_dist_venda_games"
				: "tb_venda_games";

			$sql = "SELECT idvenda, status, status_processed, vg_data_inclusao, tipo_deposito, vg_ultimo_status
                FROM tb_pag_compras
		...
		try {
			...
			$sqlUpdate = "UPDATE {$tableName} 
                      SET vg_ultimo_status_obs = '',
                          ...
                          vg_ultimo_status = 5,
                          vg_pagto_data = :data_venda_097958,
                          vg_pagto_banco = '400',
                          vg_pagto_num_docto = :doc_number,
                          vg_concilia = 1,
                          vg_data_concilia = :data_venda_431227,
			...
			$stmt->bindParam(':data_venda_097958', $data_venda . ".097958");
			$stmt->bindParam(':data_venda_431227', $data_venda . ".431227");
			$stmt->bindParam(':doc_number', 'PIXR_' . date("YmdHis") . '591');
			...
			// Atualização do saldo, apenas se PDV
			if ($this->ambiente == "PDV") {
				$sqlSaldo = "UPDATE dist_usuarios_games 
                         SET ug_perfil_saldo = ug_perfil_saldo + :valor
                         WHERE ug_id = :ugId";

				$stmt = $this->conexao->prepare($sqlSaldo);
				$stmt->bindParam(':valor', $valor);
				$stmt->bindParam(':ugId', $venda["vg_ug_id"], PDO::PARAM_INT);
				$stmt->execute();
