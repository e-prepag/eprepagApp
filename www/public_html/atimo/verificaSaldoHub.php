<?php
    ...
    if(!empty($result)){
        ...
        if($dados["result"][0][0]["saldo"] <= 2000 && isset($dados["result"][0][0]["saldo"]) && !empty($dados["result"][0][0]["saldo"])){
            ...
            $retorno = $enviaEmail('felipe.farias@easygroupit.com', 'Saldo Hud do desenvolvedor', utf8_decode($mensagem));
			if(!$retorno){
				$enviaEmail('felipe.farias@easygroupit.com', 'Saldo Hud do desenvolvedor', utf8_decode($mensagem));
			}
        }
    }
exit;
