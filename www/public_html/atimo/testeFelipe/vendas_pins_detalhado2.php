<?php
/*
 * QUERY  Considerando totalização por utilização ou venda dinamicamente para o canal PDV
 select 
    t0.pin_codinterno, 
...
if($BtnSearch) {
        ...
        if(strtoupper($fcanal) == 'L' || empty($fcanal)) { //distinct on(pin_codigo) 
            ...
            /* Modelo abaixo PINs por utilização para Gamers         
            $sql .= "
                    UNION ALL
            ...
            $sql .= "
                   ( SELECT 
                    ...
                    WHERE pih_codretepp='2'
                        ...
                        AND CASE WHEN pih_id = 90 THEN pin_lote_codigo > 6 ELSE pin_lote_codigo > 0 END -- Codigo de lotes menor e igual a 6 foram utilizados para testes da RIOT 
                        "; 
            if($dd_opr_codigo) {
                    $sql .= " 
...
<?php  
                    if($total_table > 0) 
                    {
                        $cabecalho = (!b_is_Publisher()?"'Publisher',":"")."'GUID E-Prepag','GUID Partner','PIN','". LANG_PINS_TRANSACTION_TYPE."','".LANG_PINS_ID."','".LANG_PINS_DATE."','".LANG_PINS_QUANTITY."'"; //,'Comissão','Net Amount'
?>
