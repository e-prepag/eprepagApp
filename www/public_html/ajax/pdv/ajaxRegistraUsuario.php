...
if (!isset($_POST['termos']) || empty($_POST['termos'])) {
    die("Erro: Voc� deve aceitar os termos de uso para prosseguir.");
}
// Step 1
$username = getInputRequest('username');
if (preg_match('/[\[\]{}*;()"\']/', $username) || $username == "") {

    die("Erro: O user $username n�o pode ser vazio ou possuir caracteres n�o permitidos. (, ), [, ], {, }, *, ;, \", ')");
}
$email = getInputRequest('email');
if (!isValidEmail($email)) {
    die("Erro: O e-mail fornecido � inv�lido.");
}
$email_confirmacao = getInputRequest('email_confirmacao');
if (!isValidEmail($email_confirmacao)) {
    die("Erro: O e-mail de confirma��o � inv�lido.");
}
...
if ($password !== $password_confirmacao) {
    die("Erro: A senha e a confirma��o de senha s�o diferentes.");
}
...
if (getenv('AMBIENTE') == "HOMOLOGACAO") {
    ...
    echo "Voc� deve realizar a verifica��o do RECAPTCHA para prosseguir.<br>";
    exit;
}

$msgErroLocation = "Para seguir com o cadastro, precisamos da sua autoriza��o para acessar sua localiza��o. Essa informa��o nos ajuda a garantir mais seguran�a no processo. Sua geolocaliza��o ser� usada somente para esse fim e protegida conforme a Lei Geral de Prote��o de Dados (LGPD).";
...
if ($msg == "") {
    ...
    if ($row) {
        ...
        die("N�o foi poss�vel cadastrar o usu�rio");
    }
}
...
// Fun��o para remover caracteres especiais indesejados
