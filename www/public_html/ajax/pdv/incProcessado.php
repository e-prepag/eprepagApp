...
if (isset($_SESSION['dist_usuarioGames_ser']) && !is_null($_SESSION['dist_usuarioGames_ser'])) {
    ...
    $sql = "
            SELECT 
                ...
                p.pin_lote_codigo,
                p.pin_serial,
                vgmp.vgmp_impressao_qtde,
                vgmp.vgmp_impressao_ult_data,
                vgm.vgm_id,
                vgm.vgm_pin_request
            FROM pins_dist p
            INNER JOIN tb_dist_venda_games_modelo_pins vgmp 
                ON p.pin_codinterno = vgmp.vgmp_pin_codinterno
            INNER JOIN tb_dist_venda_games_modelo vgm 
                ON vgm.vgm_id = vgmp.vgmp_vgm_id
            INNER JOIN tb_dist_venda_games vg 
                ON vg.vg_id = vgm.vgm_vg_id
            WHERE vg.vg_id = $1
                AND vg.vg_ug_id = $2
            ORDER BY 
                vgmp.vgmp_impressao_ult_data DESC,
                vgmp.vgmp_impressao_qtde,
                p.pin_serial
        ";

    $params = array(
        $GLOBALS['_SESSION']['venda'],   // $1
...
?>
    ...
    $params = array($GLOBALS['_SESSION']['venda']);

    $rsTotal = SQLexecuteQueryParams($sql, $params);
