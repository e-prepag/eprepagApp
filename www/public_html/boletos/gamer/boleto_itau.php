...
$params = array($venda_id, $usuario_id);
...
if ($msg == "") {
	$sql = "SELECT * FROM tb_venda_games vg WHERE vg.vg_id = $1";
	$params = array($venda_id);
...
if ($msg == "") {
	...
	if ($ug_id == $GLOBALS['MONEY_EXPRESS_ID_USUARIO_MONEY']) {
		...
		$sqlVenda = "SELECT * FROM tb_venda_games_cpf_boleto_express WHERE vgcbe_vg_id = $1";
		$params = array($rs_venda_row["vg_id"]);
		$rs_venda_cpf = SQLexecuteQueryParams($sqlVenda, $params);
