...
<script>
    manipulaModal(1, '<?php echo addslashes(nl2br(htmlspecialchars($msg, ENT_QUOTES, 'UTF-8'))); ?>', '<?php echo addslashes(htmlspecialchars($titulo, ENT_QUOTES, 'UTF-8')); ?>');
 <?php 
    if(!empty($link)){ 
?>
    $('#modal-load').on('hidden.bs.modal', function () { 
        location.href = '<?php echo htmlspecialchars($link, ENT_QUOTES, 'UTF-8'); ?>';
    });
<?php
    } 
?>
