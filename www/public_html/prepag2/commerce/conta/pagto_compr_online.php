...
";
	...
	if (!$rs_pagto || pg_num_rows($rs_pagto) == 0) {
		$msg = "N�o foi encontrado o pagamento para a venda " . $venda_id . ".\n";
	} else {
...
// Redireciona para a p�gina final no site do Banco
if ($iforma == $FORMAS_PAGAMENTO['TRANSFERENCIA_ENTRE_CONTAS_BRADESCO']) {
	...
	//		$simg_bank = "<img src='/imagens/pag/botao_conta_itau_.jpg' width='136' height='48' border='0' title='Banco Ita� Shopline'>";
	...
	$simg_bank = "<img src='/imagens/pag/$img_bank_logo' width='$img_bank_logo_size[0]' height='$img_bank_logo_size[1]' border='0' title='Banco Ita� Shopline'>";
	...
	<tr valign="top" align="center">
		<td>
			...
			if ($vg_integracao_parceiro_origem_id) {
				...
				//Testando a necessidade de solicita��o de CPF para Gamer
		...
		<tr>
			<td class="texto" align="center" height="25">
				...
				<?php if ($vg_integracao_parceiro_origem_id) { ?>
					Ap�s o pagamento o cr�dito ser� automaticamente ativado na conta do seu jogo.
				<?php } else { ?>
					Ap�s o pagamento a senha ser� automaticamente enviada para o seu email.
				<?php } ?>
			</td>
		</tr>
	</table>
		...
		<table border="0" cellspacing="0" bgcolor="#FFFFFF" class="wrapper">
			...
			<tr>
				<td align="center" class="texto">
					<input type="button" name="btOK" value="Clique aqui para emitir o Boleto Banc�rio"
	...
	<table border="0" cellspacing="0" bgcolor="#FFFFFF" class="wrapper">
		<tr>
			<td class="texto" align="center" height="25">
				...
				Obrigado por comprar conosco!<br><br>
				<?php if ($vg_integracao_parceiro_origem_id) { ?>
					Sua compra foi processada e o cr�dito ser� ativado diretamente na sua conta no jogo.
				<?php } else { ?>
	...
	<?php if (($banco == "237") && ($assinatura)) { ?>
		<center>
			<table border="0" cellspacing="0" align="center">
				<tr bgcolor="F0F0F0">
					<td class="texto" align="center" height="25"><b>Autentica��o</b></td>
				</tr>
				<tr bgcolor="F0F0F0">
	...
	<center>
		<TABLE cellspacing="0" cellpadding="0" class="wrapper int-box" style="border-top: 0px;">
			<tr>
				<td align="center">
					<div id="link_bank"> <?php // bloco pagamento inicio 
						...
						<table border="0" cellspacing="0" width="75%" class="int-pagamento-compr-online">
							...
							?>
									...
									<td class="texto" height="25" align="center">
																		...
																		if ($ativoNome["nome"] == "asaas") {
																			...
																			while ($venda_modelo = pg_fetch_assoc($rs_venda_modelos)) {
																				$itens[] = [
																					...
																					"description" => $venda_modelo['vgm_nome_modelo'] ? $venda_modelo['vgm_nome_modelo'] : 'Sem descri��o',
																			...
																			// Garante pelo menos 1 item, mesmo se $arr_venda_modelos estiver vazio ou inv�lido
																			if (empty($itens)) {
																				$itens[] = [
																					"title" => "Produto sem nome",
																					"description" => "N�o foi poss�vel recuperar o produto",
																			...
																			echo "Pix n�o dispon�vel no momento.";
																		}
																	} else {
																		require_once RAIZ_DO_PROJETO . 'banco/pix/' . $ativoNome["nome"] . '/config.inc.pix.php';
																...
																<tr>
																	...
																	<td valign='middle'>
																		<font color='#FF0000' size='1'
																			...
																			abaixo para efetuar o pagamento.<br>Ap�s o pagamento o
										...
										?>
											<div id="info_pagamento" pix="1">
												<b>ATEN��O: N�o efetue o Pix fora do nosso site. Para cada pagamento ser� necess�rio
								...
								<tr bgcolor="ffffff">
									...
									<?php
										//ELSE para exibi��o do pagamento PINs E-PREPG
									} else {
						...
						";
										// Deserializa os dados da sess�o
						...
						";
										...
										if (($total_geral_epp_cash / 100 + $taxa) > 0) {
											...
											/*
											Na confec��o do vetor abaixo onde est� sendo mencionado:
											[ $total_geral_epp_cash/100 ]
											n�o se trata de convers�o e sim uma divis�o simples para que o ajax receba o valor dividido por 100 com a finalidade
							...
							<div class='divToken'><span class='titulo'>Valida��o do cart�o</span><br><br>
								Para a seguran�a desta transa��o, pedimos que digite abaixo o c�digo de 6 d�gitos que aparece na fatura do seu cart�o. ( Este c�digo aparece ap�s o *, na descri��o de sua �ltima compra pela E-prepag realizada em <nobr>" . htmlspecialchars($data_exibicao, ENT_QUOTES, 'UTF-8') . "</nobr><br><br>
								Esta opera��o ser� solicitada somente uma vez para este cart�o.<br><br>
								<form action='" . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . "' method='POST'>
									...
									<input type='submit' name='btnToken' id='btnToken' value='Enviar'>
											...
											//$cielo_pan contem o hash do cart�o
												...
												<td class="texto" align="center" width="33%">
													...
													if (($pagto_tipo == $FORMAS_PAGAMENTO['TRANSFERENCIA_ENTRE_CONTAS_BRADESCO']) || ($pagto_tipo == $FORMAS_PAGAMENTO['PAGAMENTO_FACIL_BRADESCO_DEBITO']) || ($pagto_tipo == $FORMAS_PAGAMENTO['PAGAMENTO_FACIL_BRADESCO_CREDITO'])) {
														...
														if (is_null($array_infos_ws)) {
															$msg_problem = "Sua sess�o expirou!";
															$titulo = "Sess�o expirada";
														} else {
															$comunica = $obj_pagamento->Req_EfetuaConsultaURL($array_infos_ws, $lista_resposta);

															if (is_null($comunica)) {
																$titulo = "ERRO - Problema na valida��o de seus dados";
																$msg_problem = "Problema ao validar seus dados cadastrados! Por favor, relate o problema ao Suporte";
															} else {
																if (is_array($comunica)) {
																	$titulo = "ERRO - Problema de comunica��o com o Bradesco";
																	$msg_problem = "Houve um problema de comunica��o com o Bradesco! Tente novamente mais tarde. Obrigado!";
																} else {
																	$location = $comunica;
																}
															}
			...
			?>
				...
				<tr>
					<td align="center" class="texto" colspan="2">Os seus dados ser�o fornecidos com toda seguran�a apenas ao seu
						banco.</td>
				</tr>
			<?php
			}
		...
		<div id="pagamento_ok"> <?php // bloco pagamento efetuado inicio 
			...
			<center>
				...
				<?php if ($vg_integracao_parceiro_origem_id) {
				?>
					Em alguns minutos o cr�dito ser� ativado no jogo.<br>
					<?php //" e uma mensagem ser� enviada para seu email cadastrado" 
					?>
					Para retornar ao site do jogo <a href="javascript:send_to_trans()">clique aqui</a><br>
				<?php } else { ?>
					O cr�dito foi enviado para seu Email cadastrado.<br>
				<?php } ?>
				<br>
