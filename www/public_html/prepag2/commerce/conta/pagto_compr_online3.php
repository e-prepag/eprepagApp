...
if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE') )
{
        ...
        echo "<meta http-equiv=\"X-UA-Compatible\" content=\"IE=".$teste."\" />";
}
...
<?php  }  ?>
	...
	<table class="wrapper" border="0" cellspacing="0" bgcolor="#FFFFFF" <?php if($GLOBALS['_SESSION']['is_integration']==true) echo 'style="display: none"'; ?>>
	...
	<table  border="0" cellspacing="0" bgcolor="#FFFFFF" width="100%">
        <tr>
          <td class="texto" align="center" height="25">
          	<br>
          	Obrigado por comprar conosco!<br><br>
          	Sua compra foi cancelada pelo sistema. Tente novamente.</td>
        </tr>
	</table>
...
<?php  } else { //	apenas $GLOBALS['STATUS_VENDA']['PEDIDO_EFETUADO']?>
	...
	<tr>
      <td align="center">
		...
		<table border="0" cellspacing="0" width="60%" class="int-pagamento-compr-online">
            <?php
	        ...
	        <tr>
	          <td class="texto" height="25" align="center">
                ...
                ?>
				<table border="0" cellspacing="0" width="90%" class="int-pagamento-compr-online-logo hide-pix-success">
					<tr>
					  <td class="texto" align="left" width="50%"><?php echo $simg_bank; ?></td>
                                          <td class="texto" align="center" width="50%">&nbsp;<?php
                                          if($pagto_tipo == $PAGAMENTO_PIX_NUMERIC){ 
												...
												if(PAGAMENTO_PIX_CHAVEAMENTO == "a") {
													if(number_format(($total_carrinho+$taxa),2,'.','') > 50.00){
														...
														echo $pix->callService($params);
														 ...
														 require_once '/www/banco/pix/cielo/config.inc.pix.php'; 													
														 $pix = new Pix(
															"CPF", 
...
?></td>
				...
				<tr>
					<td align="center" class="texto" align="center" height="100">
						<div id="info_pagamento"> <table><tr><td valign='middle'><img src='/imagens/loading1.gif' width='42' height='42' border='0' title='Aguardando pagamento...' class="int-pagamento-compr-online-loading"></td><td>&nbsp;</td><td valign='middle'><font color='#FF0000' size='1' class="int-pagamento-compr-online-message1">Clique abaixo para efetuar o pagamento.<br>Após o pagamento o processamento leva alguns instantes.</font></td></tr></table></div>
					</td>
				</tr>
				</table>
				<?php
	        ...
	        <tr bgcolor="ffffff">
                ...
                <?php
					//ELSE para exibição do pagamento PINs E-PREPG
						...
						else echo "Dados da compra recebido sem valores.";
						echo "
						</td></tr>
						</div>";
							?>
                    ...
                    if(b_IsPagtoCielo($pagto_tipo)) {
						...
						if($limite->getPrimeiraVendaGamers($cielo_pan,$data_exibicao)) {
							...
							<div class='divToken'><span class='titulo'>Validação do cartão</span><br><br>
								Para a segurança desta transação, pedimos que digite abaixo o código de 6 dígitos que aparece na fatura do seu cartão. ( Este código aparece após o *, na descrição de sua Última compra pela E-prepag realizada em <nobr>$data_exibicao)</nobr><br><br>
								Esta operação será solicitada somente uma vez para este cartão.<br><br>
								<form action='".$_SERVER['PHP_SELF']."' method='POST'>
									<input type='hidden' name='cielo_pan' id='cielo_pan' value='".$cielo_pan."'/>
						...
						<td align='right'>
							<form action="/cielo/pages/novoPedidoAguarde.php" method="POST" target="_blank">
								...
								?>
                                    ...
                                    <input type="submit" name="btnIrCielo" class="btn btn-success" value="Clique aqui para pagar">
								<?php
								}
								?>
							</form>
			...
			<td align="center" class="texto">
				<table border="0" cellspacing="0" width="90%">
					...
					<?php
						if(!b_isIntegracao()) {
					?>
					  <input type="button" name="btVoltar" value="Voltar" OnClick="window.location='/prepag2/commerce/conta/lista_vendas.php';" class="botao_simples">
					<?php
						}
					?>&nbsp;
					  </td>
...
?>
                                    <form action="" method="post" target="_blank">
                                        <input class="btn btn-success top50" type="button" name="btnIrAoBanco" value="Clique aqui para pagar" onclick="window.open('<?php echo $location; ?>')" class="int-btn1 grad1 int-pagamento-compr-online-btn1 btn btn-success btn btn-sm btn-success">
                                    </form>
<?php
                                }
//Dummy
...
//gravaLog_TMP($smsg);
					  ...
					  ?>
							<form action="<?php echo $location; // "../debug.php" ?>" method="post" target="_blank"><input type="submit" name="btnIrAoBanco" value="Clique aqui para pagar" class="int-btn1 grad1 int-pagamento-compr-online-btn1 btn btn-success btn btn-sm btn-success">
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------
						?>
							<form action="<?php echo $location; ?>" method="post" target="_blank">
								...
								<input type="submit" name="btnIrAoBanco" value="Clique aqui para pagar" class="int-btn1 grad1 int-pagamento-compr-online-btn1 btn btn-success">
							</form>
					  <?php } else if($pagto_tipo == $PAGAMENTO_BANCO_EPP_ONLINE_NUMERIC){ ?>
						  <?php
							...
							?>
						  <form action="" method="post" target="_blank" name="formEPP"><input type="button" name="btnIrAoBanco" value="Clique aqui para pagar" onclick="window.open('<?php echo $location; ?>')">
						  </form>
					  <?php
...
<?php
			...
			<tr>
			  <td align="center" class="texto" colspan="2">Os seus dados serão fornecidos com toda segurança apenas ao seu banco.</td>
			</tr>
			<?php
				}
			?>
...
-->
			...
			<form method=post name="last_trans" <?php if(!getAutoClose_By_ID($vg_integracao_parceiro_origem_id)) { echo "target=\"_blank\" "; } ?>action="<?php echo getPartner_return_url_By_ID($vg_integracao_parceiro_origem_id) ?>">
				...
				<p><?php //echo getPartner_return_url_By_ID($vg_integracao_parceiro_origem_id) ?><p>
			...
			<?php if($vg_integracao_parceiro_origem_id){
			?>
			Em alguns minutos o crédito será ativado no jogo.<br> <?php //" e uma mensagem será enviada para seu email cadastrado" ?>
			...
			<input type="button" name="btVoltar" value="Voltar" OnClick="window.location='/prepag2/commerce/index.php';" class="botao_simples">
			<?php } ?>
