	...
	if (is_Integracao_params_valida($parceiro_params)) {
		...
		if (true || $idcliente > 0) {
			...
			$sql = "
				...
				WHERE ip.ip_store_id = $1
					AND ip.ip_currency_code = $2
					AND ip.ip_order_id = $3
					AND ip.ip_amount = $4
					AND ip.ip_client_email = $5
					AND vg_id > 0
					AND ip_vg_id > 0
			...
			$params = [
				$integracao_store_id,      // $1
				$integracao_currency_code, // $2
				$integracao_order_id,      // $3
				$integracao_amount,        // $4
				$integracao_client_email   // $5
			];

			grava_log_integracao("Integração SQL em EPP_NOTIFY: " . date("Y-m-d H:i:s") . "\n$sql\n" . "\nParametros:\n" . "\n$params\n");


			$retCompra = SQLexecuteQueryParams($sql, $params);
	...
	*/
					...
					if ($ip_status_confirmed == 1 && $vg_ultimo_status == $GLOBALS['STATUS_VENDA']['VENDA_REALIZADA'] && $vg_id > 0) {
						...
						if ($vg_ultimo_status == $GLOBALS['STATUS_VENDA']['VENDA_REALIZADA']) {
							...
							$sql = "
								UPDATE tb_integracao_pedido
								SET
									ip_status_confirmed = 1,
									ip_data_confirmed = CURRENT_TIMESTAMP
								WHERE
									ip_store_id = $1
									AND ip_currency_code = $2
									AND ip_order_id = $3
									AND ip_amount = $4
