...
$idsArray = array_map('intval', explode(',', $partner_produto_id)); // garante que só haja números
...
$sql = "
    SELECT ogpm_id, ogpm_pin_valor
    FROM tb_operadora_games_produto_modelo ogpm
    WHERE ogpm.ogpm_ogp_id IN (" . implode(',', $placeholders) . ")
";
...
$sql = "
    ...
    COALESCE(
        (
            ...
            WHERE ip.ip_store_id = $2
                AND ip_status_confirmed = 0
                AND vg_ultimo_status = 5
            ORDER BY ip_data_inclusao DESC
            LIMIT 1
        ),
        '-1'
    ) AS ip_order_id_not_selected
";

$params = [
	$partner_params["store_id"], // $1
	$partner_params["store_id"]  // $2
];
