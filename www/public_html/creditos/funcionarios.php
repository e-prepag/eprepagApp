...
$params = array($controller->usuarios->getId());
$sql = "select * from dist_usuarios_games_operador ugo where ugo.ugo_ug_id = $1 order by ugo.ugo_nome";
...
$sql .= " limit $2";
$sql .= " offset $3";
$rs_operadores = SQLexecuteQueryParams($sql, $params); //echo "sql: $sql<br>";

if ($max + $inicial > $total_table) $reg_ate = $total_table;
else $reg_ate = $max + $inicial;

$pagina_titulo = "Lista Funcionï¿½rios";
                ...
                <div class="col-md-12 bg-cinza-claro">
                    <form method="post" id="main_form" action="/creditos/funcionario/edita.php">
                        ...
                        ?>
                            ...
                            <tr>
                                <td colspan='8'>
                                    <p class="txt-vermelho">Não foram encontrados funcionários</p>
                                </td>
                            </tr>
                        <?php
                        }
