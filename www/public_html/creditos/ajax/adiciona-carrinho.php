...
function getModelo($modelo)
{
	try {
		...
		$sql = "select 1 from tb_dist_operadora_games_produto_modelo where ogpm_ativo = 1 and ogpm_id = :modelo";
...
function getProduto($produto)
{
	try {
		...
		$sql = "select 1 from tb_dist_operadora_games_produto where ogp_ativo = 1 and ogp_id = :produto";
		$stmt = $pdo->prepare($sql);
