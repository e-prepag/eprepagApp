<?php
	...
	*/
	...
	if ($_SERVER["HTTPS"] != "on") {
        redirect("https://" . $server_url . "/creditos/login-sms.php");
        die();
	...
	*/
    ...
    $strRedirect = "https://" . $server_url . "/creditos/login-sms.php?login=" . urlencode($login) . "&msg=";
	...
	*/
    ...
    if ($msg != "") {
            $strRedirect = "https://" . $server_url . "/creditos/login-sms.php?pag=" . urlencode($pag) . "&msg=" . urlencode($msg) . "&login=" . urlencode($login . "&tentativas=" . urlencode($_SESSION["tentativas_login"]));
    }
    ...
    if ($msg == "") {
        ...
        if (!verificaPOST("", $_POST)) {
            ...
            if (!$ret) {
                ...
                $ret = $instUsuarioGames->autenticarLogin($login, $senha);
                    ...
                    if ($ugo_ug_id) {
                        $sqlDataLoginOp = "update dist_usuarios_games set ug_data_ultimo_acesso = NOW() where ug_id = $ugo_ug_id";
                        SQLexecuteQuery($sqlDataLoginOp);
                    }
                }
            }
        }
        ...
        if (!$ret) {
			...
			$ug_login = "select ug_substatus from dist_usuarios_games where ug_login = '" . strtoupper($login) . "';";
            
			$row = SQLexecuteQuery($ug_login);
            $infoRow = pg_fetch_assoc($row);
            ...
            $strRedirect = "https://" . $server_url . "/creditos/login-sms.php";
			$_SESSION["sendToken"] = true;
						
			require '/www/class/pdv/classLink.php'; 
						
			$dadosUsu = unserialize($_SESSION["dist_usuarioGames_ser"]);
			$telSMS = (isset($dadosUsu->ug_s_repr_legal_cel) && !empty($dadosUsu->ug_s_repr_legal_cel))? $dadosUsu->ug_s_repr_legal_cel_ddd.$dadosUsu->ug_s_repr_legal_cel: $dadosUsu->ug_s_repr_legal_tel_ddd.$dadosUsu->ug_s_repr_legal_tel_ddd;
			$envioLinkSms = LinkAcesso::formulaMensagem($telSMS);	
            ...
            if ($pag) {
                ...
                if (strpos($pag, "/creditos/login-sms.php")) {
                    $pag = $strRedirect;
                }
