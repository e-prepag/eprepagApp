...
if (isset($_POST) && $_POST["passMestra"] != "") {
	...
	if (getenv("AMBIENTE") == "HOMOLOGACAO") {
		// Para homologação, não faz verificação de reCAPTCHA
...
<script>
	...
	function verificaRecaptcha() {
		if ((grecaptcha.getResponse() == "" || grecaptcha.getResponse().length == 0) && <?= getenv("AMBIENTE") != "HOMOLOGACAO" ? "true" : "false" ?>) {
			Swal.fire(
...
<div class="container txt-azul-claro bg-branco" style="">
	<div>
		...
		<p style="text-align: center">A Chave Mestra foi enviada para seu e-mail de cadastro, não esqueça de verificar o
			spam e lixo eletrônico.</p>
		<p style="text-align: center">Dúvidas? Fale com o <a style="text-decoration: none; color: #337ab7;"
				href="<?= EPREPAG_URL_HTTPS ?>/game/suporte.php" title="Clique para falar com o suporte">suporte</a>.
		<p>
		<form method="POST" class="container-form-verificacao" onSubmit="return verificaRecaptcha()">
			<div class="container-input-verificacao">
				<input type="text" id="passMestra"
					value="<?php echo (isset($_POST["passMestra"])) ? $_POST["passMestra"] : ""; ?>" name="passMestra"
					class="form-control">
