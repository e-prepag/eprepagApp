...
$dadosEstiloLink = json_decode($controller->usuarios->getReprLegalMSN(), true);
if($dadosEstiloLink == null){
	$dadosEstiloLink["LOGO"] = "";
}
...
if(isset($_POST['telefone_contato']))
{
    ...
    if(!$controller->atualizaCadastro($_POST, $_FILES, $dadosEstiloLink["LOGO"]))
    {
    ...
    else
    {
		...
		// pega os dados novo atualizados no metodo acima
        $dadosEstiloLink = json_decode($controller->usuarios->getReprLegalMSN(), true);
...
?>                
				...
				<div class="row top5">
                    <div class="col-md-3 col-lg-3 col-sm-3 col-xs-3 text-right">E-mail marketing: </div>
				    <div class="col-md-9 col-lg-9 col-sm-9 col-xs-9"><input type="text" name="emailMark"  value="<?php echo !empty($dadosEstiloLink["EMAILMARK"])?$dadosEstiloLink["EMAILMARK"]:"";?>" id="emailMark" class="form-control w-auto"></div>
                </div> 
                <div class="row top10">
...
<!--                <div class="row top5" id="divOutros_tipo_estabelecimento" style="display:none;">
				...
				<div class="row top10">
                    <p class="p-left25"><strong>Estilo do link de resgate Garena:</strong></p>
                </div>
				<div class="row top5">
                    <div class="col-md-3 col-lg-3 col-sm-3 col-xs-3 text-right">Cor da caixa: </div>
                    <div class="col-md-9 col-lg-9 col-sm-9 col-xs-9"><input type="color" style="width: 100px;" class="form-control" name="corCaixa" value="<?php echo !empty($dadosEstiloLink["CAIXA"])?$dadosEstiloLink["CAIXA"]:"";?>"></div>
                </div>
				<div class="row top5">
                    <div class="col-md-3 col-lg-3 col-sm-3 col-xs-3 text-right">Cor do botão: </div>
                    <div class="col-md-9 col-lg-9 col-sm-9 col-xs-9"><input type="color" style="width: 100px;" class="form-control" name="corBotao" value="<?php echo !empty($dadosEstiloLink["BOTAO"])?$dadosEstiloLink["BOTAO"]:"";?>"></div>
                </div>
				<div class="row top5">
                    <div class="col-md-3 col-lg-3 col-sm-3 col-xs-3 text-right">Cor de fundo: </div>
                    <div class="col-md-9 col-lg-9 col-sm-9 col-xs-9"><input type="color" style="width: 100px;" class="form-control" name="corFundo" value="<?php echo !empty($dadosEstiloLink["FUNDO"])?$dadosEstiloLink["FUNDO"]:"";?>"></div>
                </div>
				<div class="row top5">
                    <div class="col-md-3 col-lg-3 col-sm-3 col-xs-3 text-right">Cor do texto: </div>
                    <div class="col-md-9 col-lg-9 col-sm-9 col-xs-9"><input type="color" style="width: 100px;" class="form-control" name="corTexto" value="<?php echo !empty($dadosEstiloLink["TEXTO"])?$dadosEstiloLink["TEXTO"]:"";?>"></div>
                </div>
				<div class="row top5">
                    <div class="col-md-3 col-lg-3 col-sm-3 col-xs-3 text-right">Logo: </div>
                    <div class="col-md-9 col-lg-9 col-sm-9 col-xs-9">
					      <label id="labelfile" for="logo">Escolha seu logo</label>
					      <input style="display: none;" type="file" class="form-control w-auto" id="logo" name="logo">
						  <p id="nenhum" style="<?php echo empty($dadosEstiloLink["LOGO"])?"display: block;color: red;":"display:none;"; ?>">Você ainda não tem logo cadastrado</p>
						  <img style="<?php echo !empty($dadosEstiloLink["LOGO"])?"display: block;width: 100%;object-fit: contain;background: #eeeeee;padding: 10px;border-radius: 5px;":"display:none;"; ?>" src="<?php echo !empty($dadosEstiloLink["LOGO"])?"https://www.e-prepag.com.br/imagens/pdv/logos/".strtolower($dadosEstiloLink["LOGO"])."?".date("YmdHis"):""; ?>" id="preview">
					</div>
                </div>
                <div class="row top10">
...
$(function(){
	...
	function readImage() {
		if (this.files && this.files[0]) {
			...
			var indexFile = this.files[0];
			file.onload = function(e) {
				if(indexFile.type == "image/png" || indexFile.type == "image/jpeg"){
				    document.getElementById("preview").src = e.target.result;
					document.getElementById("preview").style.display = "block";
					document.getElementById("nenhum").style.display = "none";
					document.getElementById("preview").style.padding = "10px";
					document.getElementById("preview").style.background = "#eeeeee";
					document.getElementById("preview").style.borderRadius = "5px";
					document.getElementById("preview").style.width = "100%";
					document.getElementById("preview").style.objectFit = "contain";	
				}else{
					document.getElementById("nenhum").innerText = "O tipo de arquivo é invalido";
	...
	document.getElementById("logo").addEventListener("change", readImage, false);
