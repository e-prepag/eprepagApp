...
if(isset($_POST["btnEnviar"])){
	...
	if(empty($dataInicial) || empty($dataFinal)){
		...
		# and extract('Year' from vg_data_inclusao ) = '".date("Y")."'
		$sqlFind = "select * from tb_dist_venda_games inner join tb_dist_venda_games_modelo on vgm_vg_id = vg_id inner join tb_dist_venda_games_modelo_pins on vgmp_vgm_id = vgm_id inner join pins on vgmp_pin_codinterno = pin_codinterno where vg_ug_id = ".$controller->usuarios->getId()." and vgm_opr_codigo = ".$selectOperadora." and pin_status = '6' and pin_game_id is null and pin_guid_parceiro is null and date(vg_data_inclusao) between '".$dataInicial."' and '".$dataFinal."' order by vg_data_inclusao;";
		$rs = SQLexecuteQuery($sqlFind);
