...
$sql = "select * from dist_usuarios_games where ug_login = ? and ug_senha = ? and ug_ativo = 1";
...
if (empty($user)) {
    ...
    $sql = "select * from dist_usuarios_games_operador where ugo_login = ? and ugo_senha = ? and ugo_ativo = 1";
...
if ($usuario_operador) {
    ...
    if (empty($auth['ugo_chave_autenticador'])) {
        header("Location: https://www.e-prepag.com.br/creditos/adicionar-autenticacao.php");
        exit;
    }
    $msg = "";
    if (checkDevice($user['ugo_id'], $pdo, true)) {
        //$msg = "Dispositivo já autenticado.";
        header("Location: https://www.e-prepag.com.br/creditos/loginEf_teste.php");
        exit;
    }
