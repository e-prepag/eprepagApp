...
if (isset($_POST['tf_v_data_inclusao_ini']) && isset($_POST['tf_v_data_inclusao_fim'])) {
    if (verifica_data($_POST['tf_v_data_inclusao_ini']) != 0 && verifica_data($_POST['tf_v_data_inclusao_fim']) != 0) {
        ...
        $d_ini_idx = "$" . count($params);
        $params[] = formata_data($_POST['tf_v_data_inclusao_fim'], 1) . " 23:59:59";
        $d_fim_idx = "$" . count($params);
        $d_ini_where = " and vg_pagto_data_inclusao >= $d_ini_idx and vg_pagto_data_inclusao <= $d_fim_idx";
        $d_ini_compra_where = " and datacompra >= $d_ini_idx and datacompra <= $d_fim_idx";
    }
}
...
$sql .= "	from tb_pag_compras 
        	where idcliente= $ug_id_idx  
                        and tipo_cliente = 'LR'
        		and status = 3  \n";
$sql .= $d_ini_compra_where;
$sql .= "	) b \n";
$sql .= "	order by vg_pagto_data_inclusao desc \n";


$res_count = SQLexecuteQueryParams($sql, $params);
$total_table = (($res_count) ? pg_num_rows($res_count) : 0);

$params[] = (int)$max;
$max_idx = "$" . count($params);
$params[] = (int)$inicial;
$inicial_idx = "$" . count($params);

$sql .= " limit $max_idx";
$sql .= " offset $inicial_idx";
$rs_boletos = SQLexecuteQueryParams($sql, $params);
                        ...
                        <tbody class="trPaginacao">
                            ...
                            ?>
                                    <tr class="texto trListagem">
                                        <td><?php echo $rs_boletos_row['bol_codigo'] ?> </font>
                                        </td>
                                        <td><?php echo substr(formata_data_ts($rs_boletos_row['vg_pagto_data_inclusao'], 0, true, true), 0, 16); ?> </font>
                                        </td>
                                        <td><?php echo substr(formata_data_ts($rs_boletos_row['bol_importacao'], 0, true, true), 0, 16); ?> </font>
                                        </td>
