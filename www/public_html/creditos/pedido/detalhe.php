...
$linkPageGarena = "" . EPREPAG_URL_HTTPS . "/resgate/garena/creditos-novo.php?partner=" . urlencode($usuarioEnc);
					...
					<div class="col-md-12 col-lg-12 col-xs-12 col-sm-12 bg-cinza-claro espacamento">
						<div class="col-xs-12 col-sm-12 hidden-lg hidden-md">
							...
							<div class="col-xs-4 col-sm-4 bg-cinza-claro">
								...
								?>
								<p><a href="#reemitepins" class="btn btn-success pull-right reemite" title="">Emitir pin</a>
								</p>
								<?php
								//}
								?>
						...
						<table class="table hidden-sm hidden-xs bg-branco txt-preto fontsize-p">
							<thead>
								...
								<tr class="bg-cinza-claro text-left">
									...
									<th rowspan="2" colspan="1"><button type="button"
											class="btn btn-success pull-right reemite" title="">Emitir pin</button></th>
...
order by vgmp.vgmp_impressao_ult_data desc, vgmp.vgmp_impressao_qtde, p.pin_serial;";
						...
						?>
						<div class="col-md-12 espacamento">
							<form name="form1" id="form1" target="_blank" action="/creditos/imprimir_cupom.php"
								method="post">
								<input type="hidden" name="tf_v_codigo_detalhe" id="tf_v_codigo_detalhe"
									value="<?php echo $GLOBALS['_SESSION']['venda']; ?>">
								<input type="hidden" name="nao_emitidos" id="nao_emitidos"
									value="<?php echo $_REQUEST['nao_emitidos']; ?>">
        ...
        LIMIT 1
										...
										if ($totalGarena["tot"] > 0) { //&&($controller->usuarios->getId() == 17371 || $controller->usuarios->getId() == 7503 || $controller->usuarios->getId() == 7630 || $controller->usuarios->getId() == 13885)
											...
											if ($controller->usuarios->getReprLegalMSN() == "" || $controller->usuarios->getReprLegalMSN() == null) {
												echo '<div style="margin-top:25px;"><a target="_blank" href="' . EPREPAG_URL_HTTPS . '/resgate/garena/creditos.php?game=' . $jogoGarena . '" style="display:block;margin-bottom: 5px;width: 200px;" class="btn btn-success copiar">link para resgate Garena</a></div>';
											} else {
												echo '<div style="margin-top:25px;"><a target="_blank" href="' . $linkPageGarena . '&game=' . $jogoGarena . '" style="display:block;margin-bottom: 5px;width: 200px;" class="btn btn-success copiar">link para resgate Garena</a></div>';
											}
										}
											...
											<tbody>
												...
												while ($rs_row = pg_fetch_array($rs)) {
													...
													<tr>
														...
														<td class="d-none" id="td3id<?php echo $contador; ?>">
															<?php echo $rs_row["vgm_opr_codigo"]; ?>
														</td> <!-- G -->
														<td>
															<input type="checkbox" value="<?php echo $rs_row['pin_codinterno']; ?>"
																id="emitir<?php echo $contador; ?>"
																name="emitir<?php echo $contador; ?>">
														...
														<td class="txt-verde">R$
															<?php echo number_format($rs_row['pin_valor'], 2, ',', '.'); ?>
														</td>
						...
						<script language="javascript">
							...
							$(function () {
								...
								$(document).on("click", "#confirmaContaGarena", function (event) {
									...
									let idGarena = $("#id_garena").val();

									if (idGarena != null && idGarena != "" && idGarena != undefined) {
