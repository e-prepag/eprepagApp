...
if ($_SESSION['usuario_operador']) {
    ...
    if (!$auth) {
        ...
        $strRedirect = "login_teste.php?msg=" .
            urlencode($msgAuth) .
            "&login=" .
        ...
        // Defina o prazo máximo permitido (por exemplo, 7 dias)
        $prazoMaximo = 28;
...
if ($msg == "") {
    ...
    if (!verificaPOST("", $_POST)) {
        ...
        $ret = $instUsuarioGames->autenticarLogin($login_usuario, $senha);
        if (!$ret) {
    ...
    if (!$ret) {
        ...
        $loginUpper = strtoupper($login_usuario); // Converte o login para maiúsculas
