...
$banks = array(
	...
	'1' => array(
		'0' => '23456789abcdefghjkmnpqrstvwxyz',
		'1' => '16',
		'2' => '4'
	),
	'2' => array(
		'0' => '23456789ABCDEFGHIJKLMNPQRSTUVWXYZ',
		'1' => '16',
		'2' => '4'
	),
	'3' => array(
		'0' => 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNPQRSTUVWXYZ23456789',
		'1' => '16',
		'2' => '4'
	),
	'4' => array(
	...
	foreach ($ret as $key => $value) {
		...
		if ($value["quantidade"] <= 10) {
			...
			while ($i <= 2) {
				...
				$spin_codigo = gera_pin($value["formato"], 4.49);
				...
				if ($aux_spin_codigo == $spin_codigo) {
					...
					if (count($rs_pins) == 0) {
						...
						if (count($rs_pins_store) == 0) {
							...
							$sql = "insert into pins (pin_serial, pin_codigo, opr_codigo, pin_valor, pin_lote_codigo, pin_dataentrada, pin_canal, pin_horaentrada,pin_status,pin_validade) values ('" . $spin_serial . "', '" . $spin_codigo . "', " . $value["opr_codigo"] . ", " . $value["ogpm_valor"] . ", " . $rs_lote["max_pin_lote_codigo"] . ", CURRENT_TIMESTAMP, 's', NOW(),'1',(NOW() + interval '2 month'));";
							$query = $connection->prepare($sql);
							$query->execute();
