		    ...
		    <form action="" method="POST">
				<div class="col-md-5">
					...
					<div style="display: flex;">
						<input type="date" min="<?php echo date("Y-m-d", strtotime("-1 year"));?>" value="<?php echo isset($_POST["dataDevolucaoInicio"])? $_POST["dataDevolucaoInicio"]: ""; ?>" max="<?php echo date("Y-m-d");?>" name="dataDevolucaoInicio" class="form-control" id=""> 
						<span style="margin: 8px;">a</span>
					    <input type="date" min="<?php echo date("Y-m-d", strtotime("-1 year"));?>" value="<?php echo isset($_POST["dataDevolucaoFinal"])? $_POST["dataDevolucaoFinal"]: ""; ?>" max="<?php echo date("Y-m-d");?>" name="dataDevolucaoFinal" class="form-control" id="">
					</div>
				</div>
			    <div class="col-md-3">
					<label><?php echo LANG_NUM_REQUEST;?></label>
					<input type="number" name="numPedido" class="form-control" id="" value="<?php echo isset($_POST["numPedido"])? $_POST["numPedido"]: ""; ?>" placeholder="00000">
				</div>
				<div class="col-md-3">
				...
				<table class="table table-bordered bg-branco txt-preto fontsize-pp text-center">
					...
					<tbody>
						<tr>
							...
							?>
								<tr class="trListagem c-pointer estornoChargebackOpt" id="<?php echo $informacoes['id']; ?>">
									<td><?php echo Util::getData($informacoes['ec_data_devolucao']); ?></td>
									<td><?php echo Util::getData($informacoes['vg_data_inclusao']); ?></td>
									<td><?php echo $FORMAS_PAGAMENTO_DESCRICAO_NUMERICO[$informacoes['vg_pagto_tipo']]; ?></td>
									<td><?php echo $vetorTipo[$informacoes['ec_tipo']]; ?></td>
									<td><?php echo (isset($informacoes['ec_forma_devolucao'])?$vetorFormaDevolucao[$informacoes['ec_forma_devolucao']]:""); ?></td>
									<td><?php echo $informacoes['ug_id']; ?></td>
									<td><?php echo ucwords(strtolower((isset($informacoes['edb_titular'])?$informacoes['edb_titular']:$informacoes['usuarioNome']))); ?></td> 
									<td class="nobr"><?php echo (isset($informacoes['edb_cpf_cnpj'])?$informacoes['edb_cpf_cnpj']: substr($informacoes['ug_cpf'], 0, 3).".".substr($informacoes['ug_cpf'], 3, 3).".".substr($informacoes['ug_cpf'], 6, 3)."-".substr($informacoes['ug_cpf'], 9, 2)); ?></td>
									<td><?php echo $informacoes['vg_id']; ?></td>
									<td class="nobr"><?php echo $vetorPublisher[$informacoes['opr_codigo']]; ?></td>
									<?php if($_SESSION["opr_vinculo"] == 124 || $_SESSION["opr_vinculo"] == 0){ ?>
										<td>
										    <?php 
									...
									<?php } ?>
									<td><?php echo $vetorPINsBloqueados[$informacoes['ec_pin_bloqueado']]; ?></td>
									<td><?php echo Util::getNumero($informacoes['ec_valor']); ?></td> 
								</tr>
							<?php
									}
							?>
