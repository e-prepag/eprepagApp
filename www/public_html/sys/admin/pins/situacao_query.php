...
if ($BtnSearch) {
    ...
    if ($tf_data_inicial) {
        ...
        if (!empty($dd_california)) {
            ...
            $sql_data = "between '" . trim($data_inic) . "' and  '" . trim($data_fim) . "'";
            $sql_data_inclusao = "AND vg.vg_data_inclusao";
            $sql_data_pin = "and (pin_datavenda||' '||pin_horavenda)::timestamp";
        } else {
            $data_inic .=  " 00:00:00";
            $data_fim .=  " 23:59:59";
            $sql_data = "between '" . trim($data_inic) . "' and  '" . trim($data_fim) . "'";
    ...
    $sql = "SELECT
                    t0.pin_codinterno,
                    CASE
                        WHEN t0.pin_codigo = '0000000000000000' THEN t0.pin_caracter
                        ELSE t0.pin_codigo
                    END AS case_codigo,
                    t1.opr_nome,
                    t0.pin_serial,
        ...
        : "") . "
                ...
                INNER JOIN tb_dist_venda_games_modelo_pins vgmp ON vgmp.vgmp_pin_codinterno = t0.pin_codinterno
                ...
                WHERE 1=1 $sql_data_inclusao $sql_data $sql_wheres
                UNION ALL
                SELECT
                    t0.pin_codinterno,
                    CASE
        ...
        : "") . "
                ...
                INNER JOIN operadoras t1 ON t0.opr_codigo = t1.opr_codigo
                INNER JOIN pins_status t3 ON t0.pin_status = t3.stat_codigo
                INNER JOIN tb_venda_games_modelo_pins vgmp ON vgmp.vgmp_pin_codinterno = t0.pin_codinterno
                INNER JOIN tb_venda_games_modelo vgm ON vgm.vgm_id = vgmp.vgmp_vgm_id
                INNER JOIN tb_venda_games vg ON vg.vg_id = vgm.vgm_vg_id
                WHERE 1=1 $sql_data_inclusao $sql_data $sql_wheres
                UNION ALL
                SELECT
                    t0.pin_codinterno,
        ...
        : "") . "
                ...
                INNER JOIN operadoras t1 ON t0.opr_codigo = t1.opr_codigo
                INNER JOIN pins_status t3 ON t0.pin_status = t3.stat_codigo
                WHERE 1=1 $sql_data_pin $sql_data $sql_wheres
                  AND NOT EXISTS (
                      SELECT 1 
                      ...
                      WHERE vgmp.vgmp_pin_codinterno = t0.pin_codinterno
                        $sql_data_inclusao $sql_data
                  )
                  AND NOT EXISTS (
                      SELECT 1 
                      FROM tb_venda_games_modelo_pins vgmp
                      INNER JOIN tb_venda_games_modelo vgm ON vgm.vgm_id = vgmp.vgmp_vgm_id
                      INNER JOIN tb_venda_games vg ON vg.vg_id = vgm.vgm_vg_id
                      WHERE vgmp.vgmp_pin_codinterno = t0.pin_codinterno
                        $sql_data_inclusao $sql_data
    ...
    if ($b_debug && ($_SESSION['userlogin_bko'] == "WAGNER")) {
        echo "<hr>Gamers vg_id: " . $s_id_gamer . "<hr>Lans vg_id: " . $s_id_lans . "<hr>";
    }
    ...
    <script>
        ...
        $(function() {
            ...
            $('#btn-download').click(function(e) {
                ...
                var $btn = $(this);
                var originalText = $btn.val(); // Salva o texto original

                $btn.attr('disabled', 'true').val('<?php echo LANG_PINS_HOPE; ?>...');


                //Cria um formulário temporário e invisível
                var $form = $('<form>', {
                    'action': 'situacao_query.php',
                    'method': 'POST',
                    // 'target': '_blank' // Opcional: abre em nova aba (bom para downloads)
                }).hide();
                ...
                $form.remove();
                setTimeout(function() {
                    $btn.removeAttr('disabled').val(originalText);
                }, 2000); // Reativa o botão após 2 segundos
            });
