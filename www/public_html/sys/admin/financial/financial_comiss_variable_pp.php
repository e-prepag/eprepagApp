...
<?php
	...
	if($_SESSION["tipo_acesso_pub"]=='PU') {
        ...
        if(($tf_data_inicial) && ($tf_data_final)&& ($dd_operadora)) {
                ...
                $endDateTime = $fim[2]."-".$fim[1]."-".$fim[0]." 00:00:00";
                $sql_total_mes = "
                                select 
                            ...
                            if(empty($dados_comissao_row['co_volume_tipo'])){
                                ...
                                foreach ($aVendas as $canal => $value) {
                                        if($aVendas[$canal] > $dados_comissao_row['co_volume_min'] && $aVendas[$canal] <= $dados_comissao_row['co_volume_max']) {
                                            ...
                                            $dados_update = SQLexecuteQueryParams($sql_aliquot, array($dados_comissao_row['co_comissao'], $canal, (int)$dd_operadora, $inicio[2]."-".$inicio[1]."-".$inicio[0]." 00:00:00", $fim[2]."-".$fim[1]."-".$fim[0]." 00:00:00"));
                    ...
                    if(validacaoPublisherEppPagamentosFacilitadora($dd_operadora)) {
                            $sql_calculo = "update financial_processing 
                                            ...
                                            where fp_publisher = $3
                                                ...
                                                and fp_freeze=0";
                            //echo $sql_calculo." <br>";
                            $dados_calculo = SQLexecuteQueryParams($sql_calculo, array($inicio[2]."-".$inicio[1]."-".$inicio[0]." 00:00:00", $fim[2]."-".$fim[1]."-".$fim[0]." 00:00:00", (int)$dd_operadora, $inicio[2]."-".$inicio[1]."-".$inicio[0]." 00:00:00", $fim[2]."-".$fim[1]."-".$fim[0]." 00:00:00"));
                            if(!$dados_calculo) {
...
<?php
echo htmlspecialchars($email_texto, ENT_QUOTES, 'UTF-8');
?>
</textarea>
<br><br>
