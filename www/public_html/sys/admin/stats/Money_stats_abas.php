...
if($abanome == LANG_STATISTICS_FOR_MONTH) {
	...
	if($vendas_estado) {
		while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
			...
			//$percvalor[] 	= number_format(100*($vendas_estado_row['vendas'])/$total_vendas, 2, ',', '.');
...
if($abanome == LANG_STATISTICS_FOR_DAY) {
	...
	echo "<br><br><table><tr bgcolor='#CCFFCC'><td align='right' colspan='2'><a name='#Totais'></a><b>Total</b></td><td align='center'><b>".$n_vendas."</b></td><td align='center'><b>".number_format(($total_vendas), 2, ',', '.')."</b> <br><b>";
	if($_SESSION["tipo_acesso_pub"]=='PU') {
...
if($abanome == LANG_STATISTICS_FOR_GAME_THIS_MONTH) {
	...
	if($vendas_estado) {
		...
		//echo "<pre>".print_r($valorvendas,true)."<pre>";
		
		$retorno =  $grid->gera_grid_jogo_mes_MONEY_STATS($colunas,$jogo,$nvenda,$valorvendas,$crescente_depois,LANG_STATISTICS_FOR_GAME_THIS_MONTH,$ordem);
		require_once('grid.php');	
				
		echo "<table><tr bgcolor='#FFFFCC'><td align='center'><b>Total</b></td><td align='center'><b>".$ntmp."</b></td><td align='center'><b>".number_format(($valtmp), 2, ',', '.')."</b></td></tr>";
	} else {
		echo "<font color='#FF0000'>".LANG_NOT_FOUND_2."&nbsp;</font>";
	}
	echo "</table><br>";
...
//echo "sql: $sql<br>";
		...
		if($vendas_estado) {
			...
			while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
				...
				$priultven[] 	= get_delay_alert_live($vendas_estado_row['primeira_venda'], $vendas_estado_row['ultima_venda']);
				$cidade[] 		= $vendas_estado_row['ve_cidade'];
				$estado[] 		= $vendas_estado_row['ve_estado'];
				$nven 			= $vendas_estado_row['n'];
				$vven 			= number_format(($vendas_estado_row['vendas']), 2, ',', '.');
			...
			$retorno =  $grid->gera_grid_Total_Usuario($colunas,$user,$dtini,$dtfim,$aband,$priultven,$cidade,$estado,$nven,$vven,$vper,$crescente_depois,LANG_STATISTICS_FOR_USER,$ordem);
			require_once('grid.php');
		} else {
			echo "<font color='#FF0000'>".LANG_NOT_FOUND_2."&nbsp;</font>";
		}
