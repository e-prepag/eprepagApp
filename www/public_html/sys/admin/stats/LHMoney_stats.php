...
//echo "<b>sql</b>: ".str_replace("\n","<br>\n",$sql)."<br>";
		...
		echo "<tr><th align='center' colspan='7'><b><font color='#337ab7'>".LANG_STATISTICS_TOTAL." ".$dd_operadora_nome." ".LANG_STATISTICS_FOR_USER." (".(($vendas_estado) ? pg_num_rows($vendas_estado) : 0)." lanhouses, ".number_format((100*(($vendas_estado)?pg_num_rows($vendas_estado):"0")/(($n_cadastros>0)?$n_cadastros:1)), 2, ',', '.')."% ".LANG_STATISTICS_OF_REGISTER.") [".date("d/M/Y",strtotime($data_min)).", ".date("d/M/Y",strtotime($data_max))."]</font></b></th></tr>";
