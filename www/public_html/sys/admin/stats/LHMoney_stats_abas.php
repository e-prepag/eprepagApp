...
if($abanome == LANG_STATISTICS_FOR_DAY) {
	...
	echo "<br><br><table><tr bgcolor='#CCFFCC'><td align='right' colspan='2'><a name='#Totais'></a><b>".LANG_STATISTICS_TOTAL."</b></td><td align='center'><b>".$n_vendas."</b></td><td align='center'><b>".number_format(($total_vendas), 2, ',', '.')."</b> <br><b>";
	if($_SESSION["tipo_acesso_pub"]=='PU') {
...
if($abanome == LANG_STATISTICS_FOR_GAME_THIS_MONTH) {
	...
	if($vendas_estado) {
		...
		while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
			...
			$jogo[] 			= $nome_jogo;
			$nvenda[] 			= $vendas_estado_row['n'];
			$valorvendasa[] 	= number_format(($vendas_estado_row['vendas']), 2, ',', '.');
		...
		//print_r($jogo);
		//print_r($nvenda);
		//print_r($valorvendasa);
		//print_r($percvenda);
		//echo "</pre>";			
			
		$retorno =  $grid->gera_grid_jogo_mes_MONEY_STATS($colunas,$jogo,$nvenda,$valorvendasa,$crescente_depois,LANG_STATISTICS_FOR_GAME_THIS_MONTH,$ordem);
		require_once('grid.php');
	...
	if($_SESSION["tipo_acesso_pub"]!='PU') {
		...
		if($vendas_estado) {
			...
			while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
				...
				//$lanhouse[] = (($vendas_estado_row['ug_contatada_ultimo_mes']=="1")?" style='color:#6600CC' title='Contatada no último mês'":"").">".$vendas_estado_row['ve_nome'].(($vendas_estado_row['ug_contatada_ultimo_mes']=="1")?" (CONTATO) ":"");
				$lanhouse[] = $vendas_estado_row['ve_nome'].(($vendas_estado_row['ug_contatada_ultimo_mes']=="1")?" (CONTATO) ":"");
				$priulven[] = get_delay_alert_live($vendas_estado_row['primeira_venda'], $vendas_estado_row['ultima_venda']);
				$cidade[]   = $vendas_estado_row['ve_cidade'];
		...
		//$retorno =  $grid->gera_grid_lans($colunas,$lanhouse,$cidade,$uf,$nven,$vven,$vper);
		$retorno =  $grid->gera_grid_lans($colunas,$lanhouse,$priulven,$cidade,$uf,$nven,$vven,$vper,$crescente_depois,'Lan-Houses',$ordem);
		require_once('grid.php');			
		} else {
			echo "<br><font color='#FF0000'>".LANG_NOT_FOUND_2."&nbsp;</font>";
		}
	}
}
...
//echo "sql: ".$sql."<br>";
		...
		if($vendas_estado) {
			...
			$retorno =  $grid->gera_grid_usuario($colunas,$user,'','','',$priulve,$nven,$vven,$vper,$crescente_depois,LANG_STATISTICS_FOR_USER,$ordem);
			require_once('grid.php');			
		} else {
			echo "<font color='#FF0000'>".LANG_NOT_FOUND_2."&nbsp;</font>";
		}
