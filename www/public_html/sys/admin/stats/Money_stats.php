...
require_once $raiz_do_projeto . "includes/sys/inc_stats.php";
	...
	$queryParams = array();
	if($_SESSION["tipo_acesso_pub"]=='PU') {
		$dd_operadora = (int)$_SESSION["opr_codigo_pub"]; // Sanitize as integer
		$dd_mode = "S";
...
<div class="container-fluid">
    <div class="container txt-azul-claro bg-branco txt-cinza">
        <div class="row">
            <div class="col-md-12 ">
                <strong><h3><?php echo LANG_STATISTICS_PAGE_TITLE_3; ?> <?php if(strlen($dd_operadora_nome)>0) echo "<span class='txt-azul-claro'>".$dd_operadora_nome."</span> ";?> - <?php echo LANG_STATISTICS_PAGE_TITLE_2; ?> (<?php echo get_current_date()?>)</h3></strong>
            </div>
        </div>
...
<?php 
            ...
            {
?>
                <select name="dd_mode" id="dd_mode" class="form-control pull-left" onChange="document.form1.submit()">
...
<?php
                ...
                {
?>
                <select name="dd_operadora" id="dd_operadora" class="form-control pull-left" onChange="document.form1.submit()">
...
//echo "$total_vendas em $n_vendas vendas<br>";
	...
	// Datas Limites no BD
	$sql = get_sql_query("M", "datas_limites_no_bd", addWhereClause($extra_where, $where_operadora), $smode);
	$data_min = date("Y-m-d");
	$data_max = date("Y-m-d");
	$vendas_estado = SQLexecuteQueryParams($sql, $queryParams);
	if($vendas_estado) {
...
//echo "sql: $sql<br>";
	...
	echo "<a name='Totalpormes'><table border='1' cellpadding='0' cellspacing='1' bordercolor='#cccccc' class='txt-cinza' style='border-collapse:collapse;'>";
	echo "<tr><th align='center' colspan='4'><b><font color='#337ab7'>".LANG_STATISTICS_TOTAL." ".$dd_operadora_nome." ".LANG_STATISTICS_FOR_MONTH." (".(($vendas_estado)?pg_num_rows($vendas_estado):"0")." ".LANG_MONTHS.")</font></b></th></tr>";
...
//echo "sql: $sql<br>";
	...
	$bg_col = $bg_col_01;
	$n_dias = (($vendas_estado) ? pg_num_rows($vendas_estado) : 0);
	echo "<a name='Totalpordia'></a><table class='txt-cinza' border='1' cellpadding='0' cellspacing='1' bordercolor='#cccccc' style='border-collapse:collapse;'>";
	echo "<tr><th align='center' colspan='5'><b><font color='#337ab7'>".LANG_STATISTICS_TOTAL." ".$dd_operadora_nome." por dia (".(($vendas_estado) ? pg_num_rows($vendas_estado) : 0)." dias)</font></b></th></tr>";
	...
	if($_SESSION["tipo_acesso_pub"]=='PU') {
		...
		echo ANG_STATISTICS_PROJECTION_EPP." $days_in_month dias: R\$".number_format(($days_in_month*$total_vendas*0.04/(($n_dias>0)?$n_dias:1)), 2, ',', '.');
	} else {
		echo LANG_STATISTICS_PROJECTION." $days_in_month dias: R\$".number_format(($days_in_month*$total_vendas/(($n_dias>0)?$n_dias:1)), 2, ',', '.');
	}
	echo "&nbsp;</td></tr>";
	echo "</table><br>";
	...
	echo "<tr><th align='center' colspan='4'><b><font color='#337ab7'>".LANG_STATISTICS_TOTAL." ".$dd_operadora_nome." ".LANG_STATISTICS_FOR_GAME." (".(($vendas_estado) ? pg_num_rows($vendas_estado) : 0)." ".LANG_STATISTICS_GAME.") [".date("d/M/Y",strtotime($data_min)).", ".date("d/M/Y",strtotime($data_max))."]</font></b></th></tr>";
...
//echo $sql."<br>";
	...
	echo "<tr bgcolor='#99CCFF'><td align='center'>".LANG_STATISTICS_STATES."</td><td align='center'>".LANG_STATISTICS_SALES_NUMBER."</td><td align='center'>".LANG_STATISTICS_SALES_VALUE." (".LANG_STATISTICS_IN." R\$)</td><td align='center'>% ".LANG_STATISTICS_IN_VALUE."</td></tr>";
	if($vendas_estado && $b_show_listings) {
		while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
...
//echo "sql: $sql<br>";
		...
		if($vendas_estado && $b_show_listings) {
			...
			while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
				...
				$bg_col = ($previous_value!=$vendas_estado_row['vendas'])?(($bg_col==$bg_col_01)?$bg_col_02:$bg_col_01):$bg_col;
				echo "<tr bgcolor='".$bg_col."'><td align='left'>".$vendas_estado_row['ve_nome']."</td><td align='center'> ".get_delay_alert_live($vendas_estado_row['primeira_venda'], $vendas_estado_row['ultima_venda'])." </td><td align='center'>".$vendas_estado_row['ve_cidade']."</td><td align='center'>".$vendas_estado_row['ve_estado']."</td><td align='center'>".$vendas_estado_row['n']."</td><td align='center'>".number_format(($vendas_estado_row['vendas']), 2, ',', '.')."</td><td align='center'>".number_format(100*($vendas_estado_row['vendas'])/$total_vendas, 2, ',', '.')."</td></tr>";
				$previous_value = $vendas_estado_row['vendas'];
			}
		} else {
			echo "<tr bgcolor='".$bg_col."'><td align='center' colspan='6'><font color='#FF0000'>".LANG_NOT_FOUND_2."&nbsp;</font></td></tr>";
		}
		echo "</table><br>";
...
//echo "sql: $sql<br>";
		...
		echo "<tr><th align='center' colspan='5'><b><font color='#337ab7'>".LANG_STATISTICS_TOTAL." ".$dd_operadora_nome." ".LANG_STATISTICS_FOR_USER." (".(($vendas_estado)?pg_num_rows($vendas_estado):"0")." ".LANG_STATISTICS_USERS.")  (<font color='#FF0000'>último mês desde ".date("d/M/Y",$previousmonth)."</font>)</font></b></th></tr>";
		...
		if($vendas_estado && $b_show_listings) {
			...
			while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
				...
				echo "<tr bgcolor='".$bg_col."'><td align='left'>".$vendas_estado_row['ve_nome']."</td><td align='center'>".$sarrow.(($ipos>0)?"+":(($ipos==0)?"&nbsp;":"")).$ipos."</td><td align='center'>".$vendas_estado_row['n']."</td><td align='center'>".number_format(($vendas_estado_row['vendas']), 2, ',', '.')."</td><td align='center'>".number_format(100*($vendas_estado_row['vendas'])/$total_vendas, 2, ',', '.')."</td></tr>";
				$previous_value = $vendas_estado_row['vendas'];
	...
	//echo "sql: $sql<br>";
		...
		echo "<tr><th align='center' colspan='5'><b><font color='#337ab7'>".LANG_STATISTICS_TOTAL." ".$dd_operadora_nome." ".LANG_STATISTICS_FOR_USER." (".(($vendas_estado)?pg_num_rows($vendas_estado):"0")." ".LANG_STATISTICS_USER.")  (<font color='#FF0000'>".LANG_STATISTICS_LAST_WEEK_SINCE." ".date("d/M/Y",$previousweek)."</font>)</font></b></th></tr>\n";
		echo "<tr bgcolor='#99CCFF'><td align='center'>".LANG_STATISTICS_USER."</td><td align='center'>&nbsp;Pos.&nbsp;</td><td align='center'>".LANG_STATISTICS_SALES_NUMBER."</td><td align='center'>".LANG_STATISTICS_SALES_VALUE."  (".LANG_STATISTICS_IN." R\$)</td><td align='center'>% ".LANG_STATISTICS_IN_VALUE."</td></tr>\n";
		if($vendas_estado && $b_show_listings) {
			$iorder = 0;
			while ($vendas_estado_row = pg_fetch_array($vendas_estado)){
