...
/*function aprova_password_formato($pass, $user)
{
	if(find_special_character($pass))
	{
		for($i = 0 ; $i < strlen($user) ; $i++)
		{
			$parse = substr($user, $i, strlen($pass));
			
			if(strtolower($parse) == $pass)
			{
				$pass_into_log = true;
				break;
			}
...
---  Não utilizado, em SYS/ADMIN utiliza	"C:\Sites\E-Prepag\www\web\sys\incs\functions.php"
*/
function paginacao_query($inicial, $total_table, $max, $qtde_colunas, $img_anterior, $img_proxima, $default_add, $range, $range_qtde, $ncamp, $varsel)
{
	...
	if ($total_table > $max) // Só mostra a numeração das páginas se a quantidade de registros for maior do que o máximo permitido na tela
	{
		...
		for ($s = $range; $s <= $limite; $s++) // Monta a paginação propriamente dita
		{
			...
			echo "<a href=" . $default_add . "?inicial=" . $indice . "&range=" . $range . "&ncamp=" . $ncamp . $varsel . " class='link_azul'><font color='#00008C' size='1' face='Arial, Helvetica, sans-serif'>" . $s . "</font></a>";
			if ($s != $limite)
...
function monta_ddd_string($campoddd)
{
	...
	for ($x = 1; $x <= strlen($recebeDDD); $x += 1) {
		...
		if (ord($ch) >= 48 && ord($ch) <= 57) {
			$alerta = 1;
			break;
		} // Se a string tiver número 
		else {
			$alerta = 0;
		} // Se a string NÃO tiver número
	}
...
function imprimeComboSeuBanco($mensagem)
{
	...
	//$URL_BANCOS[4] = array("Caixa Econômica Federal", "https://internetcaixa.caixa.gov.br/NASApp/SIIBC/index_verif.processa");
?>
