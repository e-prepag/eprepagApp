...
if($BtnSearch && $BtnSearch!=1 && $FrmEnviar==1 ) {
        ...
        $estat = "
            ...
            FROM pedidos_egift_bhn b
			inner join tb_operadora_games_produto_modelo mo on mo.ogpm_pin_resquest_id = b.bhn_product_id
            inner join tb_operadora_games_produto pro on mo.ogpm_ogp_id = pro.ogp_id
            WHERE bhn_data >= '".$data_inic."'
                AND bhn_data < '".$data_fim."'
				and (select vgm_vg_id from tb_venda_games_modelo where vgm_valor < 0 and vgm_vg_id = vg_id limit 1) is null 
                ";
        if(empty($dd_todos)) {
                $estat .=  "AND bhn_status_read_account = '00' 
...
$sql_produtos = "select ogpm_pin_resquest_id,ogpm_nome,ogpm_pin_valor from tb_operadora_games_produto_modelo where ogpm_pin_resquest_id is not null and ogpm_ativo=1;";
...
<div class="container-fluid">
    <div class="container txt-azul-claro bg-branco">
        <form name="form1" method="post" action="">
                <div class="row">
                    <div class="col-md-12 ">
                        <strong>Relatório de Vendas BHN eGift</strong>
                    </div>
                </div>
...
?>
                    ...
                    <div class="col-md-2">
                        <span class="pull-right">Registro BHN eGift</span>
                    </div>
                    <div class="col-md-3">
...
<?php
                            $total_considerado = 0;
                            $vetorAudit = array();
							//var_dump(pg_fetch_array($rs));
                            while ($pgrow = pg_fetch_array($rs))
                            {
							     
                                //Decodificando o JSON
                                $pgrow['bhn_json_retorno_read_account'] = str_replace("\n", "", $pgrow['bhn_json_retorno_read_account']);
                                $instrucoes = json_decode($pgrow['bhn_json_retorno_read_account']);
                                $generate = json_decode($pgrow['bhn_json_retorno_generate']);

                                //Ex. 2018-10-31T18:19:04.000+0000
                                $data_registro_bhn = substr(str_replace("T"," ",$instrucoes->updatedTimestamp),0,19);
...
?>                                
                                        <tr class="trListagem">
                                            ...
                                            <td class="text-right"><?php  echo $instrucoes->activationAccountNumber; ?></td>
                                            <td class="text-right"><?php  echo $pgrow['bhn_id']; ?></td>
                                            <td class="text-right"><?php  echo $generate->retrievalReferenceNumber; ?></td>
<?php
                                        if(b_is_Financeiro()) {
?>
                                            <td class="text-right"><a href="https://<?php echo $_SERVER["SERVER_NAME"] ?>:8080/gamer/vendas/com_venda_detalhe.php?venda_id=<?php  echo "".$pgrow['vg_id']; ?>" target="_blank"><?php  echo $pgrow['vg_id']; ?></a></td>
<?php 
                                        }
