...
if ($_SERVER['HTTP_REFERER'] != "https://www.e-prepag.com.br/sys/admin/vendas/agrupado/vendas_dia.php") {
...
$sql  = "select dia, mes, ano, sum(quantidade) as quantidade, sum (total) as total 
                from (
                        ...
                        where vg.vg_data_inclusao>='2008-01-01 00:00:00' and vg.vg_ultimo_status='5' ".$where_opr_3." ".$where_ano_3a." ".$where_mes_3a." ".$where_canal_1a." ".$where_operadora." ".$where_opr_venda_lan."
...
if(count($vetorPublisherPorUtilizacao)>0) {
    $sql .= "
                        ...
                        select round(CAST (extract (day from pih_data::date) as int), 0) as dia, 
                                round(CAST (extract (month from pih_data::date) as int),0) as mes, 
...
<?php
	if ($_SERVER['REMOTE_ADDR'] == '201.93.162.169') {
		echo "<div style='background-color:#000; height: 50vh'><br>".var_dump($where_opr_venda_lan)."<br>".var_dump($where_opr_venda_lan_negativa)."<br>".var_dump($where_opr_utilizacao_lan)."</div>";
	}
?>
