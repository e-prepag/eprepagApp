...
// Dados do Email
$email  = "rc@e-prepag.com.br,rc1@e-prepag.com.br"; //,rc@e-prepag.com.br,rc1@e-prepag.com.br
...
// Código anterior que não estava mandando os avisos
...
function buscarray($cpfs){
    ...
    $sql = "SELECT * FROM pep WHERE cpf in($cpfs) and bloqueio_temporario != 1;";
	$rsEncontrado = SQLexecuteQuery($sql);
	...
	if($rsEncontradoRow != false){
		...
		foreach($rsEncontradoRow as $key => $value){
			...
			$encontrados[$key]["cpf"] = substr($value["cpf"], 0, 3).".".substr($value["cpf"], 3, 3).".".substr($value["cpf"], 6, 3)."-".substr($value["cpf"], 9, 2); //000.000.000-00 *** $value["cpf"]
			$encontrados[$key]["funcao"] = $value["descricao_funcao"];
		}
		
		SQLexecuteQuery("UPDATE pep SET bloqueio_temporario = 1 WHERE cpf in($cpfs);");
...
for($num = 0; $num < count($dadosTotais); $num++){
   ...
   $dadosin .= $dadosTotais[$num]["ug_cpf"].",";
   //$dadosin .= substr($dadosTotais[$num]["ug_cpf"], 0, 3).".".substr($dadosTotais[$num]["ug_cpf"], 3, 3).".".substr($dadosTotais[$num]["ug_cpf"], 6, 3)."-".substr($dadosTotais[$num]["ug_cpf"], 9, 2).","; //000.000.000-00

}

$search = substr($dadosin, 0, -1);
$retorno = buscarray(str_replace(".", "", str_replace("-", "", $search)));
...
foreach($retorno as $key => $value){
        ...
        for($i = 0;$i < count($dadosTotais);$i++){
			...
			if(strpos($dadosTotais[$i]["ug_cpf"], "-")){
				$cpf = $dadosTotais[$i]["ug_cpf"];
			}else{
				$cpf = substr($dadosTotais[$i]["ug_cpf"], 0, 3).".".substr($dadosTotais[$i]["ug_cpf"], 3, 3).".".substr($dadosTotais[$i]["ug_cpf"], 6, 3)."-".substr($dadosTotais[$i]["ug_cpf"], 9, 2);
			}
			
			if(strlen($value["cpf"]) != 14){
				$cpf_sem_pontos = str_replace([".","-"], "", $value["cpf"]);
				$value["cpf"] = "0".substr($cpf_sem_pontos, 0, 2).".".substr($cpf_sem_pontos, 2, 3).".".substr($cpf_sem_pontos, 5, 3)."-".substr($cpf_sem_pontos, 8, 2);
			}
		    ...
		    if($cpf == $value["cpf"]){
			   ...
			   $exibicaoDadosProblemas .= " 
					<tr class='trListagem'>
						<td style='padding:5px;text-align: center;'>".$encontrado[$count]["tipo"]."</td>
						<td style='padding:5px;text-align: center;'>".$encontrado[$count]["cpf"]."</td>
						<td style='padding:5px;text-align: center;'>".$encontrado[$count]["nome"]."</td>
						<td style='padding:5px;text-align: center;'>".$value["nome"]."</td>
						<td style='padding:5px;text-align: center;'>".$value["funcao"]."</td>
						<td style='padding:5px;text-align: center;'>".date("d/m/Y H:i:s")."</td>
					</tr>
				";   
				$count++;
