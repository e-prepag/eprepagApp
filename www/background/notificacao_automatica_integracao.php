...
// Notifica��o Automaticamente de Pedidos de Integra��o
...
if(!$arquivoLog->haveFile()) {
    ...
    // Parametros
    $qtde_minutos_considerados = 30; //j� conciliado
    ...
    ";
   // echo "SQL para levantamento de Pedidos a serem notificados autom�ticamente:".PHP_EOL.$sql.PHP_EOL;
    //die();
   ...
   // echo "Encontrado".(($n_updates>1)?"s":"")." : ".$n_updates." Registro".(($n_updates>1)?"s":"")." para serem notificados manualmente".PHP_EOL;
    ...
    if(!$rs || pg_num_rows($rs) == 0) {
           ...
           // echo "Pedidos que ser�o considerados nestas notifica��es:".PHP_EOL;
            $msg .= "<html>Pedidos que ser�o considerados nestas notifica��es: Total [".$n_updates."]<br><br>";
            ...
            while($rs_row = pg_fetch_array($rs)) {
                  ...
                  //  echo " Pedido ".$rs_row['vg_id']."\t\t=> Conciliado em [".$rs_row['vg_data_concilia']."]\t\t=> Tipo de pagamento [".$rs_row['vg_pagto_tipo']."];".PHP_EOL;
                    ...
                    // => ".((strstr($sret[0], '200'))?"Notificado com Sucesso":"N�O Notificado")." -- teste para verificar se existiu sucesso ba notifica��o
