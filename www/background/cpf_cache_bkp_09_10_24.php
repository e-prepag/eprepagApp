...
// Dados do Email
//$email  = "victormonterisi@gokeitecnologia.com.br";
$email  = "wagner@e-prepag.com.br";
$cc     = "glaucia@e-prepag.com.br";
$bcc    = "";
$subject= "Atualização Diária do Cache de Consultas de CPF considerados nos últimos ".PERIODO_CONSIDERADO." dias";
$msg    = "";
...
echo PHP_EOL.str_repeat("=", 80).PHP_EOL."Atualização Diária do Cache de Consultas de CPF (".date("Y-m-d H:i:s").") considerados nos últimos ".PERIODO_CONSIDERADO." dias".PHP_EOL.PHP_EOL;
...
//Verificando Dados
$msg .= PHP_EOL."Total de CPF ÚNICOS consultados em ".date("Y-m-d",$currentmonth)."(YYYY-MM-DD):  [".(($rs_dados_cpf) ? pg_num_rows($rs_dados_cpf) : 0)."] CPFs<br>".PHP_EOL;
...
while($rs_dados_cpf_row = pg_fetch_array($rs_dados_cpf)) {
    ...
    //echo($sql);
    if((($rs_existe) ? pg_num_rows($rs_existe) : 0) > 0) {
        
        //Atualizando o registro na tabela cd CPF CACHE
        echo "CPF [".$rs_dados_cpf_row['ug_cpf']."] já existe na tabela de CACHE".PHP_EOL;
    ...
    } //end if((($rs_existe) ? pg_num_rows($rs_existe) : 0) > 0)
    else {
    ...
    } //end else do if((($rs_existe) ? pg_num_rows($rs_existe) : 0) > 0)
    
} //end while
$msg .=  "Total de CPFs já existentes na tabela de CACHE que já foram validados na Receita: <b>".$total_cpfs_ja_existentes_checados." Atualizados</b><br>".PHP_EOL."Total de CPFs já existentes na tabela de CACHE SEM validação na Receita: <b>".$total_cpfs_ja_existentes." Atualizados</b><br>".PHP_EOL."Total de CPFs novos na tabela de CACHE: <b>".$total_cpfs_novos." Atualizados</b><br>".PHP_EOL;
