...
$ogp_ativo = $_REQUEST['ogp_ativo'] ?? null;
$ogp_mostra_integracao = $_REQUEST['ogp_mostra_integracao'] ?? null;
$ogp_iof = $_REQUEST['ogp_iof'] ?? null;
$ogp_nome = $_REQUEST['ogp_nome'] ?? null;
$ogp_idade_minima = $_REQUEST['ogp_idade_minima'] ?? null;
$ogp_descricao = $_REQUEST['ogp_descricao'] ?? null;
$ogp_pin_request = $_REQUEST['ogp_pin_request'] ?? null;
$ogp_valor_minimo = $_REQUEST['ogp_valor_minimo'] ?? null;
$ogp_valor_maximo = $_REQUEST['ogp_valor_maximo'] ?? null;
$ogp_detalhes_utilizacao = $_REQUEST['ogp_detalhes_utilizacao'] ?? null;
$ogp_termos_condicoes = $_REQUEST['ogp_termos_condicoes'] ?? null;
$ogp_opr_codigo = $_REQUEST['ogp_opr_codigo'] ?? null;
$BtnAtualizar = $_REQUEST['BtnAtualizar'] ?? null;
...
$instProdMod = new ProdutoModelo();
	...
	if($msg == ""){
		...
		if($BtnAtualizar){
			...
			//cria objeto produto
			$produto = new Produto($produto_id, $ogp_nome, $ogp_descricao, $ogp_ativo, null, null, $ogp_opr_codigo, $ogp_mostra_integracao, $ogp_iof, $ogp_pin_request, $ogp_detalhes_utilizacao, $ogp_termos_condicoes, $ogp_valor_minimo, $ogp_valor_maximo, $ogp_idade_minima);
 			//valida campos e atualiza
			$msgAcao = $produto->atualizar($produto);
			if($msgAcao == ""){
		...
		if($acao){
			...
			//excluir modelo
			if($acao == "e"){
				$msgAcao = ProdutoModelo::excluir($modelo_id);
	...
	if($msg == ""){
		...
		if($ret != "") $msg = $ret;
			...
			$ogp_data_atualizacao	= $rs_produto_row['ogp_data_atualizacao'];
			$ogp_opr_codigo 	= $rs_produto_row['ogp_opr_codigo'];
			$opr_status 		= $rs_produto_row['opr_status'];
			$opr_nome 		= $rs_produto_row['opr_nome'];
			$ogp_mostra_integracao	= $rs_produto_row['ogp_mostra_integracao'];
			$ogp_iof		= $rs_produto_row['ogp_iof'];
            $ogp_pin_request = $rs_produto_row['ogp_pin_request'];
            $ogp_detalhes_utilizacao = $rs_produto_row['ogp_detalhes_utilizacao'];
            $ogp_termos_condicoes = $rs_produto_row['ogp_termos_condicoes'];
	...
	if($msg == ""){
		...
		if($ogp_id && is_numeric($ogp_id)) {
			...
			$ret = $instProdMod->obter($filtro, "ogpm_valor", $rs_produto_modelos);
			if($ret != "") $msg = $ret;
		}
	}
...
function abreUpload(produto_id){
	...
	url = "/gamer/com_imagem_upload.php?produto_id=" + produto_id;
...
<div class="col-md-12">
    <ol class="breadcrumb top10">
        ...
        <li><a href="/gamer/produtos/com_pesquisa_produtos.php">Listar Produtos</a></li>
  ...
  <tr> 
	...
	<form name="form1" id="form_cadastro" method="post" action="com_produto_detalhe.php">
        ...
        <table class="table top10">
          ...
          <tr bgcolor="#F5F5FB"> 
            <td width="150"><b>Data de Atualização</b></td>
            <td><?php echo formata_data_ts($ogp_data_atualizacao,0,true,true) ?></td>
          </tr>
          <tr bgcolor="#F5F5FB"> 
          ...
          <tr bgcolor="#F5F5FB"> 
            <td><b>Descrição</b></td>
            <td><textarea name="ogp_descricao" id="descricao" cols="80" rows="15" class="form2"><?php echo $ogp_descricao ?></textarea></td>
          </tr>
...
?>
        <tr bgcolor="#F5F5FB" id="mostrar_valor_variavel">
            <td>
        ...
        <tr class="input_valor <?php if($hide_input_valor) echo "hide"; ?>" bgcolor="#F5F5FB">
          ...
          <tr bgcolor="#F5F5FB" id="mostrar_details">           
            ...
            <td><textarea name="ogp_detalhes_utilizacao" cols="80" rows="8" class="form2"><?php echo $ogp_detalhes_utilizacao; ?></textarea></td>
          </tr>
          <tr bgcolor="#F5F5FB" id="mostrar_terms"> 
            <td><b>Termos e condições:</b></td>
            <td><textarea name="ogp_termos_condicoes" cols="80" rows="8" class="form2"><?php echo $ogp_termos_condicoes; ?></textarea></td>
          </tr>
<?php
        } else{
?>
...
?>         
		  	...
		  	<td colspan="2">
                <table class="table">
					<tr bgcolor="F0F0F0" class="texto">
					  ...
					  <td align="center"><b>Valor do PIN EPPCash</b></td>
		...
		?>
					<tr class="texto" bgcolor="#F5F5FB">
					  ...
					  <td align="right"><?php echo number_format($rs_produto_modelos_row['ogpm_valor'], 2, ',', '.') ?></td>
					  <td align="right"><?php echo number_format($rs_produto_modelos_row['ogpm_pin_valor'], 2, ',', '.') ?></td>
					  <td align="right"><?php echo number_format($rs_produto_modelos_row['ogpm_valor_eppcash'], 0, ',', '') ?></td>
          ...
          <tr>
            <td colspan="2" align="right">
                <a href="com_modelo_detalhe_insere.php?produto_id=<?php echo $produto_id ?>" class="btn btn-sm btn-info">Inserir novo modelo</a>
            </td>
          </tr>
		</table>
