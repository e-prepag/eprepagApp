...
if ($msg == "") {
  ...
  if ($BtnAtualizar) {
    ...
    $ogpm_valor = str_replace(",", ".", $ogpm_valor);
    $ogpm_valor_eppcash = str_replace(",", ".", $ogpm_valor_eppcash);
    $ogpm_pin_valor_markup = ($opr_markup == "0") ? str_replace(",", ".", $ogpm_valor) : str_replace(",", ".", $ogpm_pin_valor_markup);

    //cria objeto produto
    $modelo = new ProdutoModelo($modelo_id, $ogpm_ogp_id, $ogpm_nome, $ogpm_descricao, $ogpm_valor, $ogpm_ativo, null, null, $ogpm_pin_valor, $ogpm_valor_eppcash, $ogpm_pin_resquest_id, $ogpm_pin_valor_markup);

    //valida campos e atualiza
    $msgAcao = $instProdMod->atualizar($modelo);
  ...
  if ($acao) {
    ...
    //excluir imagem
    if ($acao == "ei") {
      $sql = "update tb_operadora_games_produto_modelo set ogpm_nome_imagem = NULL
...
if ($msg == "") {
  ...
  $rs_modelo = null;
  $ret = $instProdMod->obter($filtro, null, $rs_modelo);
  if ($ret != "") $msg = $ret;
    ...
    $ogpm_valor_eppcash  = $rs_modelo_row['ogpm_valor_eppcash'];
...
<script language="javascript">
  ...
  function abreUpload(modelo_id) {
    ...
    url = "/gamer/com_imagem_upload.php?modelo_id=" + modelo_id;
    ...
    <td>
      <?php if ($msg != "") { ?>
        <table class="table">
          <tr>
            <td align="center">
              <font color="#FF0000" size="2" face="Arial, Helvetica, sans-serif"><?php echo str_replace("\n", "<br>", $msg) ?></font>
            </td>
          </tr>
        </table>
      <?php } ?>
      ...
      <form name="form1" method="post" action="com_modelo_detalhe.php">
        ...
        <table class="table">
          <tr bgcolor="#FFFFFF">
            <td colspan="2" bgcolor="#ECE9D8">Produto</font>
            </td>
          </tr>
          <tr bgcolor="#F5F5FB">
        ...
        <table class="table">
          <tr bgcolor="#FFFFFF">
            <td colspan="2" bgcolor="#ECE9D8" class="texto">Operadora</font>
            </td>
          </tr>
          <tr bgcolor="#F5F5FB">
        ...
        <table class="table">
          <tr bgcolor="#FFFFFF">
            <td colspan="2" bgcolor="#ECE9D8">Modelo</font>
            </td>
          </tr>
          <tr bgcolor="#F5F5FB">
            <td width="150"><b>C&oacute;digo</b></td>
          ...
          <tr bgcolor="#F5F5FB">
            ...
            <td>
              ...
              <?php if ($ogpm_nome_imagem && $ogpm_nome_imagem != "" && file_exists($GLOBALS['FIS_DIR_IMAGES_PRODUTO'] . $ogpm_nome_imagem)) { ?>
                <img src="http://<?php echo $_SERVER['SERVER_NAME'] . $URL_DIR_IMAGES_PRODUTO . $ogpm_nome_imagem ?>" border="0">
          ...
          <tr bgcolor="#F5F5FB">
            <td><b>Valor EPPCash</b></td>
            <td><input name="ogpm_valor_eppcash" type="text" class="form2" value="<?php echo number_format($ogpm_valor_eppcash, 0, ',', ''); ?>"></td>
          </tr>
          <?php
          if ($opr_markup) {
          ?>
