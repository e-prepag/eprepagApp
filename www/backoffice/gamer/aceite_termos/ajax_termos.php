...
if (isset($_POST["acao"]) && $_POST["acao"] == "listar") {
	...
	$sql = "SELECT uat.versao_termo, uat.aceitou, uat.data_aceite, uat.ip, uat.dispositivo, uat.localizacao, ug.ug_nome_cpf, ug.ug_id from usuarios_games ug
	...
	if ($usuario != null) {
		if (is_numeric($usuario)) {
			...
			$usuario = "%$usuario%";
			$sql .= "ug.ug_nome_cpf ILIKE :usuario_id ";
		}
	} else {
		$sql .= "1=1 ";
	}
	...
	if (count($resultRows) > 0) {
		foreach ($resultRows as $key => $value) {
			...
			$dataLine = [
				...
				$dataKeys[6] => ($value["ug_nome_cpf"] ? mb_convert_encoding($value["ug_nome_cpf"], 'UTF-8', 'latin1') : "Não encontrado"),
	...
	$sql = "DELETE from usuarios_aceito_termos where ug_id = :user_id";
	$stmt = $conexao->prepare($sql);
