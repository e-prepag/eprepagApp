...
//echo "tf_confirmed: '".$tf_confirmed."'<br>";
	if(!($tf_data_ini && $tf_data_fim)) {
		...
		$sql_where .= "and iph.iph_data_inclusao between '".formata_data_ts_integracao($tf_data_ini)." 00:00:00' and '".formata_data_ts_integracao($tf_data_fim)." 23:59:59' ";
	}
...
//echo "$tf_data_ini - $tf_data_fim<br>";
			...
			if($tf_data_ini && $tf_data_fim) {
				...
				$sql_where .= "and iph.iph_data_inclusao between '".formata_data_ts_integracao($filtro['dataMin'])." 00:00:00' and '".formata_data_ts_integracao($filtro['dataMax'])." 23:59:59' ";
			}
			if($tf_data_conf_ini && $tf_data_conf_fim) {
				$filtro['dataConfMin'] = $tf_data_conf_ini;
				$filtro['dataConfMax'] = $tf_data_conf_fim;
				$sql_where .= "and iph.iph_data_confirmed between '".formata_data_ts_integracao($filtro['dataConfMin'])." 00:00:00' and '".formata_data_ts_integracao($filtro['dataConfMax'])." 23:59:59' ";
			}
...
//				$sql_where .= "and vg.vg_data_concilia between '".formata_data_ts_integracao($filtro['dataConciliaMin'])." 00:00:00' and '".formata_data_ts_integracao($filtro['dataConciliaMax'])." 23:59:59' ";
			...
			if($tf_store_id) {
				...
				$sql_where .= "and iph.iph_store_id = '".$filtro['store_id']."' ";
			}

			if($tf_cliente_email) {
				$filtro['cliente_email'] = $tf_cliente_email;
				$sql_where .= "and iph.iph_client_email = '".$filtro['cliente_email']."' ";
			}

			if($tf_amount) {
				$filtro['amount'] = $tf_amount;
				$sql_where .= "and iph.iph_amount = ".(100*$filtro['amount'])." ";
			}
			if($tf_v_status) {
...
<div class="col-md-12">
    <ol class="breadcrumb top10">
        ...
        <li class="active">Pesquisa de pedidos de integração de Parceiros <font color="red">(pedidos perdidos sem referência a uma venda existente)</li>
    </ol>
</div>
<table class="table fontsize-p txt-preto">
  <tr> 
    <td> 
		<form name="form1" method="post" action="com_pesquisa_integracao_pedidos_perdidos.php">
        <table class="table">
          <tr bgcolor="#F5F5FB"> 
        ...
        <table class="table">
          ...
          <tr bgcolor="#F5F5FB"> 
			...
			<?php 
				/*
			?>
				<select name="tf_cliente_email" class="form2">
					<option value="" <?php if($tf_cliente_email == "") echo "selected" ?>>Selecione de <?php echo $n_clientes; ?> usuários</option>
					<?php if($rs_clientes) while($rs_clientes_row = pg_fetch_array($rs_clientes)){ ?>
					<option value="<?php echo $rs_clientes_row['cliente']; ?>" <?php if ($tf_cliente_email == $rs_clientes_row['cliente']) echo "selected";?>><?php echo "ID: ".$rs_clientes_row["cliente"]." (".$rs_clientes_row["n"]." registro".(($rs_clientes_row["n"]>1)?"s":"").")"; ?></option>
					<?php } ?>
				</select>
			<?php 
				*/
                ...
                <tr bgcolor="#00008C"> 
                  <td height="11" colspan="3" bgcolor="#FFFFFF"> 
                      ...
                      <tr  bgcolor="#ECE9D8"> 
                        ...
                        <td align="center"><strong><font class="texto">Usuário</font>
                          </strong></td>
...
//echo "<pre>".print_r($filtro, true)."</pre><br>";
		...
		$sql  = "select * \n";
		$sql .= "from tb_integracao_pedido_historico iph \n";
		$sql .= "	inner join tb_integracao_pedido ip on ip.ip_id = iph.iph_ip_id \n";
...
//echo getListaCharacterParaPagtoOnline()."<br>";	//	-> '5','6','7','9','A','E','B','P','Z'
			...
			if($filtro['vg_id']) {
				...
				$sql .= "=1 or (iph.iph_ip_vg_id ".(($filtro['tf_v_codigo_include']=="-1")?"not":"")." in (" . str_replace("'", "", $filtro['vg_id']) . ")) ) \n";
			}
			if($filtro['order_id']) {
