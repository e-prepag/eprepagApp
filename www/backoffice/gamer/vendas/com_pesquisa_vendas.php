...
$downloadCsv = (isset($_GET["downloadCsv"]) && $_GET["downloadCsv"] == 1);
if ($downloadCsv) {
	ob_start();
}
...
$tf_v_origem = $_REQUEST['tf_v_origem'] ?? null;
$tf_v_codigo = $_REQUEST['tf_v_codigo'] ?? null;
$tf_v_codigo_include = $_REQUEST['tf_v_codigo_include'] ?? null;
$tf_v_status = $_REQUEST['tf_v_status'] ?? null;
$tf_v_qtde_produtos = $_REQUEST['tf_v_qtde_produtos'] ?? null;
$tf_v_qtde_itens = $_REQUEST['tf_v_qtde_itens'] ?? null;
$tf_v_valor = $_REQUEST['tf_v_valor'] ?? null;
$tf_v_data_inclusao_ini = $_REQUEST['tf_v_data_inclusao_ini'] ?? null;
$tf_v_data_inclusao_fim = $_REQUEST['tf_v_data_inclusao_fim'] ?? null;
$tf_v_concilia = $_REQUEST['tf_v_concilia'] ?? null;
$tf_v_data_concilia_ini = $_REQUEST['tf_v_data_concilia_ini'] ?? null;
$tf_v_data_concilia_fim = $_REQUEST['tf_v_data_concilia_fim'] ?? null;
$conciliacao_manual = $_REQUEST['conciliacao_manual'] ?? null;
$tf_v_data_cancelamento_ini = $_REQUEST['tf_v_data_cancelamento_ini'] ?? null;
$tf_v_data_cancelamento_fim = $_REQUEST['tf_v_data_cancelamento_fim'] ?? null;
$tf_v_dep_codigo = $_REQUEST['tf_v_dep_codigo'] ?? null;
$tf_v_bol_codigo = $_REQUEST['tf_v_bol_codigo'] ?? null;
$tf_d_forma_pagto = $_REQUEST['tf_d_forma_pagto'] ?? null;
$tf_d_banco = $_REQUEST['tf_d_banco'] ?? null;
$tf_d_local = $_REQUEST['tf_d_local'] ?? null;
$tf_numdoc = $_REQUEST['tf_numdoc'] ?? null;
$tf_d_num_docto = $_REQUEST['tf_d_num_docto'] ?? null;
$tf_d_data_ini = $_REQUEST['tf_d_data_ini'] ?? null;
$tf_d_data_fim = $_REQUEST['tf_d_data_fim'] ?? null;
$tf_d_valor_pago = $_REQUEST['tf_d_valor_pago'] ?? null;
$tf_d_data_inclusao_ini = $_REQUEST['tf_d_data_inclusao_ini'] ?? null;
$tf_d_data_inclusao_fim = $_REQUEST['tf_d_data_inclusao_fim'] ?? null;
...
$tf_v_integracao = $_REQUEST['tf_v_integracao'] ?? null;
$tf_v_drupal = $_REQUEST['tf_v_drupal'] ?? null;
$tf_opr_codigo = $_REQUEST['tf_opr_codigo'] ?? null;
$ex_tf_opr_codigo = $_REQUEST['ex_tf_opr_codigo'] ?? null;
$tf_d_canal = $_REQUEST['tf_d_canal'] ?? null;
$com_saldo = $_REQUEST['com_saldo'] ?? null;
$somente_gocash = $_REQUEST['somente_gocash'] ?? null;
$tf_produto = $_REQUEST['tf_produto'] ?? null;
$tf_pins = $_REQUEST['tf_pins'] ?? null;
...
$DISTRIBUIDORAS_CANAIS = $DISTRIBUIDORAS_CANAIS ?? ($GLOBALS['DISTRIBUIDORAS_CANAIS'] ?? array());
$STATUS_VENDA_DESCRICAO = $STATUS_VENDA_DESCRICAO ?? ($GLOBALS['STATUS_VENDA_DESCRICAO'] ?? array());
$FORMAS_PAGAMENTO_DESCRICAO = $FORMAS_PAGAMENTO_DESCRICAO ?? ($GLOBALS['FORMAS_PAGAMENTO_DESCRICAO'] ?? array());
$PAGTO_BANCOS = $PAGTO_BANCOS ?? ($GLOBALS['PAGTO_BANCOS'] ?? array());
$PAGTO_LOCAIS = $PAGTO_LOCAIS ?? ($GLOBALS['PAGTO_LOCAIS'] ?? array());
$PAGTO_NOME_DOCTO = $PAGTO_NOME_DOCTO ?? ($GLOBALS['PAGTO_NOME_DOCTO'] ?? array());
$server_url_complete = $server_url_complete ?? "";
$buscaOper = $buscaOper ?? null;
$msg = '';
$total_table = 0;
$reg_ate = 0;
$totalGeral_valor = 0;
...
if (isset($BtnSearch)) {
	...
	if ($msg == "") {
		...
		if ($total_table > 0) {
			...
			$n_lista_ug_email = 0;
			$n_lista_id_ped = 0;
			while ($rs_venda && ($rs_venda_row = pg_fetch_array($rs_venda))) {
...
$currentAbaOrdem = "";
$currentAbaDescricao = "";
if (isset($currentAba) && is_object($currentAba)) {
	$currentAbaOrdem = $currentAba->getOrdem();
	$currentAbaDescricao = $currentAba->getDescricao();
}
$sistemaMenu = null;
if (isset($sistema) && is_object($sistema) && isset($sistema->menu[0])) {
	$sistemaMenu = $sistema->menu[0];
}
$sistemaMenuDescricao = is_object($sistemaMenu) ? $sistemaMenu->getDescricao() : "";
$sistemaItem = (isset($sistema) && is_object($sistema) && isset($sistema->item)) ? $sistema->item : null;
$sistemaItemLink = is_object($sistemaItem) ? $sistemaItem->getLink() : "";
$sistemaItemDescricao = is_object($sistemaItem) ? $sistemaItem->getDescricao() : "";
if ($downloadCsv && ob_get_level() > 0) {
	ob_end_flush();
}
?>
...
<script language="javascript">
	...
	$(document).ready(function() {
		...
		$("#tf_v_data_concilia_ini").AnyTime_noPicker().AnyTime_picker({
			baseYear: 2008,
		...
		$("#tf_v_data_concilia_fim").AnyTime_noPicker().AnyTime_picker({
			baseYear: 2008,
			earliest: rangeDemoConv.format(new Date(2008, 1, 1, 0, 0, 0)),
			format: rangeDemoFormat,
			latest: rangeDemoConv.format(new Date(year, 11, 31, 23, 59, 59)),
...
<div class="col-md-12">
	<ol class="breadcrumb top10">
		<li><a href="#" class="muda-aba" ordem="<?php echo $currentAbaOrdem; ?>">BackOffice - <?php echo $currentAbaDescricao; ?></a></li>
		<li class="active"><?php echo $sistemaMenuDescricao; ?></li>
		<li class="active"><a href="<?php echo $sistemaItemLink; ?>"><?php echo $sistemaItemDescricao; ?></a></li>
	</ol>
</div>
	...
	<tr>
		<td>
			<form name="form1" id="form1" method="post" action="com_pesquisa_vendas.php<?php
				...
				<table class="table">
					<tr bgcolor="#FFFFFF">
						<td colspan="6" bgcolor="#ECE9D8" class="texto">Venda</font>
						</td>
					</tr>
					<?php
					//					if(b_IsUsuarioReinaldo()) { 
					?>
					<tr bgcolor="#F5F5FB">
						<td class="texto">Tipo de pedido</font>
						</td>
						<td>
					...
					?>
					<tr bgcolor="#F5F5FB">
						<td width="100" class="texto">Origem</font>
						</td>
						<td>
					...
					<tr bgcolor="#F5F5FB">
						<td width="100" class="texto">Número do Pedido</font>
						</td>
						<td>
					...
					<tr bgcolor="#F5F5FB">
						<td class="texto">Conciliação</font>
						</td>
						<td>
						...
						<td class="texto">Data Conciliação <nobr>DD/MM/AAAA hh:mm &nbsp;</nobr>
							</font>
						</td>
						<td class="texto">
					...
					<tr bgcolor="#F5F5FB">
						<td class="texto">Conciliação Manual</font>
						</td>
						<td>
						...
						<td class="texto">Data Cancelamento</font>
						</td>
						<td class="texto">
					...
					<tr bgcolor="#FFFFFF">
						<td colspan="4" bgcolor="#ECE9D8" class="texto">Dados do Pagamento</font>
						</td>
					</tr>
					<tr bgcolor="#F5F5FB">
					...
					<script>
						function populate_local(combo1, combo2, combo2_default) {
							...
							//seleciona opcao
							for (var i = combo2.length - 1; i >= 0; i--)
								if (combo2[i].value == combo2_default) combo2.selectedIndex = i;
						}
					</script>
					<script>
						populate_local(document.form1.tf_d_banco, document.form1.tf_d_local, "<?php echo $tf_d_local ?>");
					</script>
					...
					<tr bgcolor="#F5F5FB">
						<td class="texto" colspan="2">&nbsp;<input type="button" value="Obtem 'N. Docto' de código de barras Itaú" onClick="get_vg_id()">&nbsp;</font>
						</td>
						<td class="texto" colspan="2">cod.barras.Itaú&nbsp;<input type="text" name="tf_numdoc" id="tf_numdoc" value="" size="54" maxlength="54"></font>
						</td>
					</tr>

					<tr bgcolor="#F5F5FB">
						<td class="texto">N. Docto</font>
						</td>
						<td>
							<input name="tf_d_num_docto" id="tf_d_num_docto" type="text" class="form2" value="<?php echo $tf_d_num_docto ?>" size="25" maxlength="15">
						</td>
						<td class="texto">Data Informada</font>
						</td>
						<td class="texto">
					...
					<tr bgcolor="#F5F5FB">
						<td class="texto">Valor Pago</font>
						</td>
						<td>
					...
					<tr bgcolor="#FFFFFF" id="divUsuario">
						<td colspan="4" bgcolor="#ECE9D8" class="texto">
							...
							<table width="100%" border="0" cellpadding="0" bgcolor="#FFFFFF">
								<tr bgcolor="#FFFFFF">
									<td colspan="4" bgcolor="#ECE9D8" class="texto">Usuário</font>
									</td>
								</tr>
							</table>
							...
							<div id='DivCodigoNome'>
								<table width="100%" border="0" cellpadding="0" bgcolor="#FFFFFF">
									<tr bgcolor="#F5F5FB" id="divUsuario1">
										<td class="texto" width='98px'>C&oacute;digo</font>
										</td>
										<td width='208px'>
											<input name="tf_u_codigo" type="text" class="form2" value="<?php echo $tf_u_codigo ?>" size="30">
										</td>
										<td class="texto" width='110px'>Nome</font>
										</td>
										<td>
							...
							<table width="100%" border="0" cellpadding="0" bgcolor="#FFFFFF">
								<tr bgcolor="#F5F5FB">
									<td class="texto" width='98px'>Email</font>
									</td>
									<td width='208px'>
					...
					<tr bgcolor="#FFFFFF" id="divUsuario">
						<td colspan="4" bgcolor="#ECE9D8" class="texto">
							...
							<table width="100%" border="0" cellpadding="0" bgcolor="#FFFFFF">
								<tr bgcolor="#FFFFFF">
									<td colspan="4" bgcolor="#ECE9D8" class="texto">Integração</font>
									</td>
								</tr>
							</table>
							...
							<div id='DivCodigoNome'>
								<table width="100%" border="0" cellpadding="0" bgcolor="#FFFFFF">
									<tr bgcolor="#F5F5FB" id="divUsuario1">
										<td class="texto" width='98px'>Integração</font>
										</td>
										<td width='208px'>
										...
										<td class="texto" width='110px'>&nbsp;</font>
										</td>
										<td>&nbsp;</td>
									</tr>
								</table>
					...
					<tr bgcolor="#FFFFFF" id="divUsuario">
						<td colspan="4" bgcolor="#ECE9D8" class="texto">
							...
							<table width="100%" border="0" cellpadding="0" bgcolor="#FFFFFF">
								<tr bgcolor="#FFFFFF">
									<td colspan="4" bgcolor="#ECE9D8" class="texto">Drupal</font>
									</td>
								</tr>
							</table>
							...
							<div id='DivCodigoNome'>
								<table width="100%" border="0" cellpadding="0" bgcolor="#FFFFFF">
									<tr bgcolor="#F5F5FB" id="divUsuario1">
										<td class="texto" width='98px'>Drupal</font>
										</td>
										<td width='208px'>
					...
					<tr bgcolor="#FFFFFF">
						<td colspan="4" bgcolor="#ECE9D8" class="texto">Produto</font>
						</td>
					</tr>
					<tr bgcolor="#F5F5FB">
						<td width="100" class="texto">Operadora</font>
						</td>
						<td>
...
?>
	<table class="table txt-preto fontsize-pp bg-branco">
		...
		<tr bgcolor="#ECE9D8">
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=vg_id&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">C&oacute;d.
					</a><?php if ($ncamp == 'vg_id') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=vg_data_inclusao&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">Data Inclusão</font>
					</a><?php if ($ncamp == 'vg_data_inclusao') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=vg_pagto_tipo&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">Forma de<br>Pagamento</font>
					</a><?php if ($ncamp == 'vg_pagto_tipo') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong>
					<font class="texto">Dados de pagamento</font>
				</strong>
			</td>
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=valor&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">Valor</font>
					</a><?php if ($ncamp == 'valor') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong>
					<font class="texto">Depósito/<br>Boleto</font>
				</strong>
			</td>
			<td align="left">
				<strong>
					<font class="texto">Produtos</font>
				</strong>
			</td>
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=qtde_itens&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">Qtde<br>Total</font>
					</a><?php if ($ncamp == 'qtde_itens') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=ug_id&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">C&oacute;d.<br>Usuário</font>
					</a><?php if ($ncamp == 'ug_id') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=ug_nome&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">Nome Usuário</font>
					</a><?php if ($ncamp == 'ug_nome') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong>
					<font class="texto">Venda&nbsp;completa</font>
				</strong>
			</td>
			<td align="left">
				<strong><a href="<?php echo $default_add . "?ordem=" . $ordem . "&ncamp=vg_concilia&inicial=" . $inicial . $varsel ?>" class="link_branco">
						<font class="texto">Conciliação</font>
					</a><?php if ($ncamp == 'vg_concilia') echo "<img src=" . $img_seta . " width='10' height='7' align='absmiddle'>"; ?></strong>
			</td>
			<td align="left">
				<strong>
					<font class="texto">Produtos</font>
		...
		?>
			<tr bgcolor="<?php echo $cor1 ?>" onmouseover="bgColor='#CFDAD7'" onmouseout="bgColor='<?php echo $cor1 ?>'" valign="top">
				...
				<td class="texto" align="left">
					...
					$pagto_nome_docto = $PAGTO_NOME_DOCTO[$vg_pagto_banco][$vg_pagto_local] ?? "";
					$pagto_nome_docto_Ar = explode(";", $pagto_nome_docto);
					$pagto_banco_nome = $PAGTO_BANCOS[$vg_pagto_banco] ?? "";
					$pagto_local_nome = $PAGTO_LOCAIS[$vg_pagto_banco][$vg_pagto_local] ?? "";

					$dado = "Banco: " . $pagto_banco_nome;
					$dado .= " Local " . $pagto_local_nome;
					?>
					<b>Banco:</b> <?php echo $pagto_banco_nome ?><br>
					<b>Local:</b> <?php echo $pagto_local_nome ?><br>
					<?php
				...
				?>
					<td class="texto" align="center" title="<?php echo "status: '$status'" ?>">
						...
						?>
							<nobr>
								<font color="#ff0000">Venda incompleta</font>
							</nobr>
						<?php
						} else {
						...
						?>
							<nobr>
								<font color="#009933">Venda realizada</font>
							</nobr>
						<?php
						}
						?>
				...
				if ($tf_d_forma_pagto == 'E') {
					...
					if (b_IsUsuarioReinaldo() || b_IsUsuarioWagner()) {
						...
						//if($somente_gocash) 
						{
							...
							$b_has_eppcash	= (($apagtos['valorpagtopin'] ?? 0) > 0);
							$b_has_saldo	= (($apagtos['valorpagtosaldo'] ?? 0) > 0);
							$s_canal_gocash .= "[" .
								(($b_has_eppcash) ? "P" : "-") .
								(($b_has_saldo) ? "S" : "-") .
								(($b_has_gocash) ? "G" : "-") . "] ";
							if ($b_has_eppcash) {
								$s_canal_gocash .= "PIN_EPP";
							}
							if ($b_has_saldo) {
								$s_canal_gocash .= "Saldo";
							}
							if ($b_has_gocash) {
								$s_canal_gocash .= "GoCash";
							}
							...
							if ($b_has_gocash) {
								$lineCsv = array();
								$lineCsv[] = ($apagtos['valorpagtogocash'] ?? 0);

								$objCsv->setLine($espacamento . implode(";", $lineCsv));

								$s_composicao .=    "<tr class='texto' align='center'>
                                                                            <td>" . ($apagtos['valorpagtogocash'] ?? 0) . " </td> 
                                                                            <td>-</td> 
                                                                            <td>C</td>
                                                                        </tr>\n";
							}
				...
				?>
					<td align="center" valign="top">
						<strong>
							<font class="texto">$$$$$<?php echo $s_canal_gocash;		//$DISTRIBUIDORAS_CANAIS[$rs_venda_row['tvgpo_canal']]." <br>(".$rs_venda_row['comissao_canal'].")";
														?></font>
						</strong><br>
						<?php
						echo $s_composicao;
					...
					<td align="center" title="<?php if (b_IsUsuarioReinaldo() || b_IsUsuarioWagner()) {
													echo "Publisher: " . $rs_venda_row['opr_nome'] . " (ID: " . $rs_venda_row['vgm_opr_codigo'] . ")\nComissão Publisher: " . $rs_venda_row['comissao_publisher'] . "%\nCanal venda PIN EPP Cash: " . $rs_venda_row['tvgpo_canal'] . "\nDesconto canal: " . number_format($rs_venda_row['tvgpo_perc_desconto'], 0, '.', '.') . "%\n";
												} ?>">
						@@@@@<strong>
							<font class="texto"><?php if (b_IsUsuarioReinaldo() || b_IsUsuarioWagner()) {
													echo number_format((100 * $rs_venda_row['comissao_epp']), 0, '.', '.');
												} ?>%</font>
						</strong>
					</td>
				<?php
		...
		<tr>
			<td colspan="20" bgcolor="#FFFFFF" class="texto"><?php echo $search_msg . number_format(getmicrotime() - $time_start, 2, '.', '.') . $search_unit ?></font>
			</td>
		</tr>
