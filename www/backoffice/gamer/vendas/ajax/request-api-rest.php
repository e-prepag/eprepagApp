<?php
	...
	if (trim($resultado) == 'Pagamento já conciliado') {
		...
		try {
			$query_consulta = "select * from tb_venda_games where vg_id = :venda_id and vg_ultimo_status = :vg_ultimo_status;";
			...
			if (count($resultado_consulta) > 0) {
				...
				$query_atualiza = "update tb_venda_games set vg_usuario_obs = vg_usuario_obs || ' - {$dataHoraFormatada} Conciliado manualmente por ' || :dados_operador WHERE vg_id = :venda_id;";
		...
		echo 'Concilização realizada com sucesso!';
