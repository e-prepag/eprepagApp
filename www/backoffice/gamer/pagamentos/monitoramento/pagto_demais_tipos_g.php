<?php
    ...
    $contents = "";
    try {
        $smonfilename = $raiz_do_projeto . 'arquivos_gerados/logs/monitorprocessapagtoonline.txt';
    ...
    $contents = "";
    try {
        $smonfilename = $raiz_do_projeto . 'arquivos_gerados/logs/monitor_concilia.txt';
        if ($handle = fopen($smonfilename, 'r')) { 
            //echo "\nMonitor File opened\n";
            // Exemplo:
            // Tempo médio de processamento (2011-11-17 12:16:03): 0.22 s/processamento<br>11 pagamentos em aberto<br><br><br>
...
//										$dif = strtotime(date("Y-m-d H:i:s"))-strtotime($data_created); 
            ...
            echo "\n<span class='txt-vermelho'>Error: Couldn't open Monitor File (concilia) for reading\n";
        }
    } catch (Exception $e) {
        echo "\n<span class='txt-vermelho'>Error(17) opening monitor file [".date("Y-m-d H:i:s")."]: ".$e->getMessage()."\n";
    }

    echo "<div class='top10 lista bg-amarelo txt-azul'>Monitor de vg_concilia (cada 30mins)".str_replace("Incompleto", "<span class='txt-vermelho'>Incompleto</span>", str_replace("\n","<br>\n",$contents)).""."</span></div>";