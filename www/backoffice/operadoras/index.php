...
if($acao == 'inserir') {
	if (!in_array($opr_codigo,$OPR_CODIGO_BLOCK)) {
		...
		if($rs_opr_codigo_row = pg_fetch_array($rs_opr_codigo)) {
			...
			$opr_valor = array();
			for($i=1;$i<=15;$i++) {
				$var_name = "opr_valor".$i;
				$var_value = str_replace(",", ".", $$var_name);		// Note the double sign "$$" -> we are getting the value of the variable with the name in $var_name
		//		echo "'$var_name' = '$var_value'<br>";
				if($var_value>0) $opr_valor[] = $var_value;
			}
		//	echo "<pre>".print_r($opr_valor, true)."</pre>";
			...
			$sql .= "				'$opr_numero_conta',
									...
									'NUMERO DO PIN',";
			for($i = 0; $i < 15; $i++){
				if (empty($opr_valor[$i]))
					$sql .= "0,";
				else $sql .= str_replace(",", ".", $opr_valor[$i]).",";
			}	
			$sql .= "				'131313131313131', 
									'$opr_contato',
									'$opr_status',
		...
		else $msg .= "Erro ao selecionar o NOVO opr_codigo.<br>";
	}
	else $msg .= "Opr_Codigo da Operadora n&atilde;o permitido.<br>";
	$acao = 'listar';
}
...
if($acao == 'atualizar') {
	...
	for($i = 0; $i < 15; $i++){
		if (empty($opr_valor[$i]))
			$sql .= "opr_valor".($i+1)."=0,";
		else $sql .= "opr_valor".($i+1)."=".str_replace(",", ".", $opr_valor[$i]).",";
	}	
...
//echo "obtem valores do BD<br>";
		for($i=1;$i<=15;$i++) {
			$var_name = "opr_valor".$i;
			$var_value = number_format($rs_operadoras_row[$var_name], 2, ',', '');		
			if($var_value>0) {
				$$var_name = $var_value;	// Note the double sign "$$" -> we are saving the value into the variable with the name in $var_name
			}
		}
