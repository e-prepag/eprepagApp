...
//	if($BtnSearch) $total_table = 0;
    ...
    $sql ="select ped_codigo, ped_est_codigo, nome_fantasia, ped_cod_documento,ped_valor,ped_cod_doc_equiv 
		   ...
		   where  t1.est_codigo=ped_est_codigo 
		   and ped_valor > 1 
		   and est_teste =0 and ped_valor = $BolValor and ped_banco = $BolBanco";
		// and ped_concilia = 0 and ped_aprovado = 1 
	$res_count = pg_query($sql);
	$total_table = pg_num_rows($res_count);
	$bol_valor_total = $total_table*$BolValor;
	$sql .= " order by ".$ncamp." ";
      ...
      <table border='0' width="100%" cellpadding="2" cellspacing="1">
        <tr bgcolor="#00008C"> 
          ...
          <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=nome_fantasia" . $varsel ?>"><font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF">Nome 
            ...
            <?php if($ncamp == 'bol_banco') echo "<img src=".$img_seta." width='10' height='7'>"; ?>
          </td>
          <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_documento" . $varsel ?>"><font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF"><span class="link_br">Documento 
