...
if (!isset($tf_data_inic_imp) || !$tf_data_inic_imp) $tf_data_inic_imp = date('d/m/Y');
if (!isset($tf_data_final_imp) || !$tf_data_final_imp) $tf_data_final_imp = date('d/m/Y');
...
if($importacao_data) $sql .= "and (bol_importacao >= '" . formata_data($tf_data_inic_imp, 1) . "' and bol_importacao <= '" . formata_data($tf_data_final_imp, 1) . "') ";
...
if ($max + $inicial > $total_table)
  ...
  if (!isset($importacao_data))
  $importacao_data = null;
...
if (!isset($tf_data_final))
  ...
  if (!isset($tf_data_inic_imp))
  $tf_data_inic_imp = null;

if (!isset($tf_data_final_imp))
  $tf_data_final_imp = null;
...
$varsel =  "&importacao_data=$importacao_data&tf_data_inic=$tf_data_inic&tf_data_final=$tf_data_final&tf_data_inic_imp=$tf_data_inic_imp&tf_data_final_imp=$tf_data_final_imp&dd_banco=$dd_banco&tf_cod_documento=$tf_cod_documento&tf_documento=$tf_documento&dd_situacao=$dd_situacao&tf_valor=$tf_valor&tf_valor_oper=$tf_valor_oper&dd_tipodoc=$dd_tipodoc";
if (isset($_POST['BtnExport'])) {
  // Remonta a SQL sem limite de paginação
  $sql_export = "select bol_codigo, bol_valor, bol_data, bol_importacao, bol_banco, bol_documento, bol_cod_documento, bol_banco, bco_nome, bol_aprovado ";
  $sql_export .= "from bancos_financeiros, boletos_pendentes bp ";

  if (isset($dd_usuario) && $dd_usuario) {
    $sql_export .= "inner join tb_dist_venda_games vg on bp.bol_venda_games_id = vg.vg_id ";
    $sql_export .= "inner join dist_usuarios_games ug on ug.ug_id = vg.vg_ug_id ";
  }

  $sql_export .= "where (bol_banco = bco_codigo) and (bco_rpp = 1) and (bol_data >= '" . formata_data($tf_data_inic, 1) . "' and bol_data <= '" . formata_data($tf_data_final, 1) . "') ";

  if($importacao_data) $sql_export .= "and (bol_importacao >= '" . formata_data($tf_data_inic_imp, 1) . "' and bol_importacao <= '" . formata_data($tf_data_final_imp, 1) . "') ";
  ...
  header('Content-Disposition: attachment;filename="boletos_pendentes_' . date('dmY_His') . '.xls"');
  header('Cache-Control: max-age=0');

  echo "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
  echo "<Workbook xmlns=\"urn:schemas-microsoft-com:office:spreadsheet\">\n";
  echo "<Worksheet ss:Name=\"Boletos Pendentes\">\n";
  echo "<Table>\n";
  ...
  echo "<Cell><Data ss:Type=\"String\">Importação</Data></Cell>\n";
  echo "<Cell><Data ss:Type=\"String\">Banco</Data></Cell>\n";
  echo "<Cell><Data ss:Type=\"String\">Cod Documento</Data></Cell>\n";
  echo "<Cell><Data ss:Type=\"String\">Documento</Data></Cell>\n";
  echo "<Cell><Data ss:Type=\"String\">Valor</Data></Cell>\n";
  echo "<Cell><Data ss:Type=\"String\">Situação</Data></Cell>\n";
  echo "</Row>\n";

  // Dados
  $resexport = pg_exec($connid, $sql_export);
  $total_export = 0;
  while ($pgest = pg_fetch_array($resexport)) {
    $bol_aprovado = ($pgest['bol_aprovado'] == 1) ? "Conciliado" : "Disponível";
    $total_export += $pgest['bol_valor'];

    echo "<Row>\n";
    echo "<Cell><Data ss:Type=\"String\">" . $pgest['bol_codigo'] . "</Data></Cell>\n";
    echo "<Cell><Data ss:Type=\"String\">" . formata_data($pgest['bol_data'], 0) . "</Data></Cell>\n";
    echo "<Cell><Data ss:Type=\"String\">" . formata_timestamp($pgest['bol_importacao'], 2) . "</Data></Cell>\n";
    echo "<Cell><Data ss:Type=\"String\">" . $pgest['bco_nome'] . "</Data></Cell>\n";
    echo "<Cell><Data ss:Type=\"String\">" . $pgest['bol_cod_documento'] . "</Data></Cell>\n";
    echo "<Cell><Data ss:Type=\"String\">" . $pgest['bol_documento'] . "</Data></Cell>\n";
    echo "<Cell><Data ss:Type=\"Number\">" . str_replace(",", ".", $pgest['bol_valor']) . "</Data></Cell>\n";
    echo "<Cell><Data ss:Type=\"String\">" . $bol_aprovado . "</Data></Cell>\n";
    echo "</Row>\n";
  }

  echo "<Row>\n";
  echo "<Cell><Data ss:Type=\"String\">TOTAL</Data></Cell>\n";
  echo "<Cell></Cell><Cell></Cell><Cell></Cell><Cell></Cell><Cell></Cell>\n";
  echo "<Cell><Data ss:Type=\"Number\">" . str_replace(",", ".", $total_export) . "</Data></Cell>\n";
  echo "<Cell></Cell>\n";
  echo "</Row>\n";
...
<script language="JavaScript">
  $(document).ready(function() {
    ...
    setDateInterval('tf_data_inic_imp', 'tf_data_final_imp', optDate);
  ...
  <tr>
    <td valign="top">
      <form name="form1" method="post" action="">
        <table class="table">
          <tr bgcolor="#F5F5FB">
            <td width="0" height="0" colspan="1">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Intervalo Data Emissão</font>
            </td>
            ...
            <td width="0" colspan="2" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Intervalo Data Importação</font>
            </td>
            <td height="0" colspan="3">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">
                <input name="importacao_data" type="checkbox" class="form" id="importacao_data" <?= $importacao_data ? "checked" : "" ?> value="1">
                <input name="tf_data_inic_imp" type="text" class="form" id="tf_data_inic_imp" value="<?php echo $tf_data_inic_imp ?>" size="9" maxlength="10">
                -
                <input name="tf_data_final_imp" type="text" class="form" id="tf_data_final_imp" value="<?php echo $tf_data_final_imp ?>" size="9" maxlength="10">
            </td>
          </tr>
          <tr bgcolor="#F5F5FB">
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Banco</font>
            </td>
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">
                <select name="dd_banco" id="dd_banco" class="combo_normal" style="max-width: 200px;">
            ...
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Situa&ccedil;&atilde;o</font>
            </td>
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">
            ...
            <td>
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Valor
              </font>
            </td>
            <td><select name="tf_valor_oper">
                ...
                <option value="<" <?php if (isset($tf_valor_oper) && $tf_valor_oper == "<") echo "selected" ?>>
                  << /option>
              </select></td>
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">
          ...
          <tr bgcolor="#F5F5FB">
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Cod.
                Documento</font>
            </td>
            <td width="0" height="0">
              <font color="#FF0000" size="2" face="Arial, Helvetica, sans-serif">
            ...
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Documento</font>
            </td>
          ...
          <tr bgcolor="#F5F5FB">
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Usuário:</font>
            </td>
            <td width="0" height="0">
              <font color="#FF0000" size="2" face="Arial, Helvetica, sans-serif">
                <select name="dd_usuario" id="dd_usuario" class="combo_normal" style="max-width: 200px;">
            ...
            <td width="0" height="0">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">Tipo de doc.</font>
            </td>
            <td height="0" colspan="3">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">
    ...
    <table class="table" id="tabelaBoletos">
      <tr bgcolor="#0086c5">
        <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_codigo" . $varsel ?>">
              <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF" class="link_br">C&oacute;digo</font>
            </a></strong>
          <?php if ($ncamp == 'bol_codigo') echo "<img src=" . $img_seta . " width='10' height='7'>"; ?>
        </td>
        <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_data" . $varsel ?>">
              <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF" class="link_br">Dt Emissão</font>
            </a></strong>
          <?php if ($ncamp == 'bol_data') echo "<img src=" . $img_seta . " width='10' height='7'>"; ?>
        </td>
        <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_importacao" . $varsel ?>">
              <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF" class="link_br">Dt Importa&ccedil;&atilde;o</font>
            </a></strong>
          <?php if ($ncamp == 'bol_importacao') echo "<img src=" . $img_seta . " width='10' height='7'>"; ?>
        </td>
        <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bco_nome" . $varsel ?>">
              <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF"><span class="link_br">Banco</span></font>
            </a></strong>
          <?php if ($ncamp == 'bco_nome') echo "<img src=" . $img_seta . " width='10' height='7'>"; ?>
        </td>
        <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_cod_documento" . $varsel ?>">
              <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF"><span class="link_br">Cod
                  Documento</span></font>
            </a></strong>
          <?php if ($ncamp == 'bol_cod_documento') echo "<img src=" . $img_seta . " width='10' height='7'>"; ?>
        </td>
        <td><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_documento" . $varsel ?>">
              <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF"><span class="link_br">Documento</span></font>
            </a></strong>
        ...
        <td>
          <div align="right">
            ...
            <strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_valor" . $varsel ?>">
                <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF"><span class="link_br">Valor</span></font>
              </a></strong>
          </div>
        </td>
        <td>
          <div align="center"><strong><a href="<?php echo $default_add . "?ordem=" . !$ordem . "&inicial=" . $inicial . "&ncamp=bol_aprovado" . $varsel ?>">
                <font face="Arial, Helvetica, sans-serif" size="2" color="#FFFFFF"><span class="link_br">Situa&ccedil;&atilde;o</span></font>
              </a>
            </strong></div>
        </td>
        <td>
          <font face="Arial, Helvetica, sans-serif" size="2" color="#0086c5">x</font>
        </td>
      </tr>
      <?php
      $cor1 = "#F5F5FB";
      ...
      while ($pgest = pg_fetch_array($resest)) {
        ...
        if ($pgest['bol_aprovado'] == 1)
          ...
          $bol_aprovado = "Disponï¿½vel";
      ?>
        <tr>
          <td bgcolor="<?php echo $cor1 ?>">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif">
              <a href="altera.php?BolCod=<?php echo $pgest['bol_codigo']; ?>" class="link_azul">
                <?php echo $pgest['bol_codigo'] ?></a>
            </font>
          </td>
          <td bgcolor="<?php echo $cor1 ?>">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><?php echo formata_data($pgest['bol_data'], 0) ?></font>
          </td>
          <td bgcolor="<?php echo $cor1 ?>">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><?php echo formata_timestamp($pgest['bol_importacao'], 2) ?></font>
          </td>
          <td bgcolor="<?php echo $cor1 ?>">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><?php echo $pgest['bco_nome'] ?></font>
          </td>
          <td bgcolor="<?php echo $cor1 ?>">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><?php echo $pgest['bol_cod_documento'] ?></font>
          </td>
          <td bgcolor="<?php echo $cor1 ?>">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><?php echo $pgest['bol_documento'] ?></font>
          </td>
          <td bgcolor="<?php echo $cor1 ?>" class="text-right <?php echo ($bol_aprovado == "Disponï¿½vel") ? "txt-azul" : "txt-preto"; ?>">
            <?php echo number_format($pgest['bol_valor'], 2, ',', '.') ?>
          </td>
          <td bgcolor="<?php echo $cor1 ?>">
            <div align="center">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><?php echo $bol_aprovado ?></font>
            </div>
          </td>
          <td bgcolor="<?php echo $cor1 ?>">
            <div align="center"><a href="deleta.php?BolCod=<?php echo $pgest['bol_codigo'] . $varsel ?>" onClick="GP_popupConfirmMsg('Deseja excluir esta informação de boleto?');return document.MM_returnValue"><img src="/images/deletar.gif" alt="Excluir Registro" width="12" height="14" border="0"></a></div>
          </td>
        </tr>
        <?php
      ...
      <?php  } else { ?>
        <tr bgcolor="#E4E4E4">
          <td colspan="6">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><strong>SUBTOTAL</strong></font>
          </td>
          <td>
            <div align="right">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><strong><?php echo number_format($bol_valor_total, 2, ',', '.') ?></strong></font>
            </div>
          </td>
          <td>&nbsp;</td>
          <td>
        ...
        <tr bgcolor="#E4E4E4">
          <td colspan="6">
            <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><strong>TOTAL</strong></font>
          </td>
          <td>
            <div align="right">
              <font color="#666666" size="2" face="Arial, Helvetica, sans-serif"><strong><?php echo number_format($bol_valor_total_i, 2, ',', '.') ?></strong></font>
            </div>
          </td>
          <td>&nbsp;</td>
          <td>
        ...
        <tr bgcolor="#E4E4E4">
          <td colspan="9" bgcolor="#FFFFFF">
            <font size="1" face="Arial, Helvetica, sans-serif" color="#666666"><strong>
                OBS: Valores expressos em R$.</strong></font>
          </td>
        </tr>
        <script language="JavaScript">
        ...
        <tr>
          <td colspan="9" bgcolor="#FFFFFF">
            <font size="1" face="Arial, Helvetica, sans-serif" color="#666666"><?php echo $search_msg . number_format($time, 2, '.', '.') . $search_unit ?>
            </font>
          </td>
        </tr>
        <?php
...
<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    tabela?.parentNode.insertBefore(exportBtn, tabela);

    function exportToCSV() {
      const tabela = document.getElementById("tabelaBoletos");
      if (!tabela) {
        Swal.fire("Erro", "Tabela de boletos não encontrada", "error");
        return;
      }
      ...
      linhas.forEach(row => {
        ...
        // Ignora linhas de total e subtotais ou colunas de ações
        const textoLinha = row.innerText.toLowerCase();
        console.log(" row", row)
        if (textoLinha.includes("subtotal") || textoLinha.includes("total")) return;
        if (colunas.length === 0) return;

        const linhaCSV = Array.from(colunas)
          .filter(col => !col.innerHTML.includes("deletar.gif")) // ignora botão excluir
          .map(col => `"${col.innerText.trim().replace(/"/g, '""')}"`);

        if (linhaCSV.length >= 5) conteudo.push(linhaCSV.join(";"));
      });

      if (conteudo.length === 0) {
        Swal.fire("Aviso!", "Nenhum dado encontrado para exportar.", "warning");
        return;
      }

      const csvFinal = "\uFEFF" + conteudo.join("\n");
      ...
      a.setAttribute("download", "boletos_pendentes_" + getTimestamp() + ".csv");
    ...
    function getTimestamp() {
      const now = new Date();
      return now.toLocaleDateString('pt-BR').replace(/\//g, '') + '_' +
