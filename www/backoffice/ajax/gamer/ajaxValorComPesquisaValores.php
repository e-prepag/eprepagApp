...
if($rs_oprPins){
	...
	while($rs_oprPins_row = pg_fetch_array($rs_oprPins)){ 
      echo '<option value="'.$rs_oprPins_row['pin_valor'].'"'.(isset($_REQUEST['valor']) && (intval($_REQUEST['valor'])==$rs_oprPins_row['pin_valor'])?" selected":"").'>'.$rs_oprPins_row['pin_valor'] . ',00</option>'; 
	}
	echo '</select>';
}
else {
