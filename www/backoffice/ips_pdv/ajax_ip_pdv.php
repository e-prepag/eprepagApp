...
if (isset($_POST["acao"]) && $_POST["acao"] == "listar") {
	...
	$sql = "SELECT ip.ip_address, ip.ip_range, ip.ip_range_ini, ip.ip_range_end, ug.ug_nome_fantasia, ug.ug_id
				FROM pdv_api_ip ip
	...
	if (count($resultRows) > 0) {
		foreach ($resultRows as $key => $value) {
			...
			$acao = "<a class='btn btn-info' href='/risco_merchants/publisher.php?ug_id=" . $value["ug_id"] . "'
			...
			$dataLine = [
				"ug_id" => $value["ug_id"],
				"ug_nome" => (isset($value["ug_nome_fantasia"]) ? $value["ug_nome_fantasia"] : "Não encontrado"),
				"ip_pdv" => $str_ip,
				"acao" => $acao
			];
	...
	$sql = "INSERT INTO pdv_api_ip VALUES (:ug_id, :ip_address, :ip_range_ini, :ip_range_end, :ip_range)";
	$stmt = $conexao->prepare($sql);
	$stmt->bindParam(':ug_id', $ug_id);
	$stmt->bindParam(':ip_address', $ip_address);
	$stmt->bindParam(':ip_range_ini', $ip_range_ini);
	$stmt->bindParam(':ip_range_end', $ip_range_end);
