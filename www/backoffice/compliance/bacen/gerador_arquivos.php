...
//=========  Data de doze meses anteriores a data M�s Ano considerado => 13 em fun��o de mes anterior (-1) e 12 meses antes
...
echo "<br><b>M�s/Ano do per�odo de apura��o: [<span style='color: red'>".$mesAno."</span>]</b><br><br><br>\n";
...
//Buscando dados de cota��o no Banco de Dados
...
if($rs) {
    ...
    if($testeCotacaoZerada) {
        die("Obrigat�rio o cadastramento de cota��o diferente de Zero para os Publishers relacionados acima.");
    }//end if($testeCotacaoZerada)
}//end if($rs)

//Verificando se existe o cadastro de cota��o do dolar para gera��o dos arquivos BACEN
if(isset($vetorCotacaoUSS) && count((array)$vetorCotacaoUSS) == 0){
    die("Antes de gerar os arquivos deve ser cadastrados as cota��es de d�lar no sistema!");
}//end if(count((array)$vetorCotacaoUSS))

//Totalizando publisher internacionais e verificando se bate os totais de publisher com o cadastro de cota��es.
...
if($totalPublisherInternacionais != count((array)$vetorCotacaoUSS)) {
    die("Existe divergencias entre o total de publisher com movimento no m�s de referencia e o total de publisher cadastrado com cota��es!");
}//end if($totalPublisherInternacionais != count((array)$vetorCotacaoUSS)) 

//Dados Necess�rios
$identificacaoRegisto5816_I = '#A1';    // Identifica��o do registro => 1- REGISTRO DE IDENTIFICA��O DA ADMINISTRADORA/PROCESSADORA
$identificacaoRegisto5816_F = '@1';     // Identifica��o do registro => 4- REGISTRO DE CONTROLE FINAL 
$tipoCartao = 'db';                     // Tipo do cart�o para o documento 5816
$codigoMoeda = '220';                   // C�digo da Moeda fico 220 = Dolar Americano
$cnpjEPP = '19037276000172';            // CNPJ da empresa E-PREPAG ADMINISTRADORA DE CARTOES LTDA
$codigoNossaBandeira = '4';             // C�digo de Nossa Bandeira de 1 (uma) posi��o para o documento 5816
...
if (verificaCPFValido($vetorPublisher, date("t",mktime(0, 0, 0, ($mes*1), 1, $ano)), $rs_dados, $vetorPublisherNovos)) {
    ...
    while($rs_dados_row = pg_fetch_array($rs_dados)) {
        if(!verificaCPF_BACEN($rs_dados_row['ug_cpf'])) {
            $exibicaoDadosProblemas .= " ".$rs_dados_row['tipo']." => ID: ".$rs_dados_row['ug_id']." CPF Inv�lido: ".$rs_dados_row['ug_cpf']."<br>\n";
            $i++;
...
if($testeData < $dataInicioOperacao) {
    die("O m�s ano deve ser obrigat�riamente superior a ".$dataInicioOperacao." (AAAAMM).<br>\n");
...
//================================== Inicio da Gera��o do Arquivo Layout 5816
...
$listaUG_CPF = "";   // Lista contendo os IDs de usu�rios que gastram mais de 10.000 dolares
$vetorTotaisAcimaLimite = array();
if (verificaLimiteDetalhamento(10000,$rsTeste)) {
    echo "<b>[<span style='color: red'>ATEN��O: Existe CPF que ultrapassou os US$ 10.000 no m�s</span>]</b><br><br><br>\n";
...
if (verificaLimiteCOAF(5000,$rsCOAF)) {
    echo "<b>[<span style='color: red'>ATEN��O: Existe CPF que ultrapassou os R$ 5.000,00 no m�s e devem ser completados os dados de cadastro e analisados em rela��o ao COAF.</span>]</b><br><br><br>\n";
...
//=========================================================================================================================
//1- REGISTRO DE IDENTIFICA��O DA ADMINISTRADORA/PROCESSADORA
//=========================================================================================================================
// Cabe�alho
unset($vetorHeader);
$vetorHeader = array (
...
// Inicializando variavel com Total do Relat�rio Mensal
$total_5816_dolares = 0;
// Inicializando variavel com Total de CPFs no Relat�rio Mensal
$total_5816_cpfs = 0;
...
if(!$rs) echo "Erro ao selecionar os Detalhamento para os Publishers (".implode(",", $vetorPublisher).").<br>\n";
    ...
    while($rs_row = pg_fetch_array($rs)) {
        ...
        if($cpf_anterior == str_replace("-","",str_replace(".", "", $rs_row['ug_cpf']))) {
            die("<br>O CPF anterior [".$cpf_anterior."] e o pr�ximo CPF [".$rs_row['ug_cpf']."] est� sendo utilizado com NOMES DIFERENTES!<br>Corrigir antes de gerar o arquivo para o BACEN!<pre>".print_r($rs_row,true)."</pre>");
...
//=========================================================================================================================
//3- REGISTRO DE DADOS ( DETALHAMENTO DAS DESPESAS DA FATURA - OBRIGAT�RIO PARA FATURAS NO VALOR ACIMA DE U$ 10.000,00)
//=========================================================================================================================
if(!empty($listaUG_CPF)) { 
    ...
    if(!$rs) echo "Erro ao selecionar os Detalhamento para os Publishers (".implode(",", $vetorPublisher).").<br>\n";
        ...
        while($rs_row = pg_fetch_array($rs)) {
            ...
            // Busca informa��es do estabelecimento onde foi efetuada a despesa no exterior
...
$vetorLines = array (
                     ...
                     1 => array('name' => ($contFatura+$contDetalhamento+2), //Os dois s�o referentes aos registros de Identifica��o e Controle Final
                                'size' => 8
                                ),
                     2 => array('name' => ' ',
                                'size' => 135
...
//================================== Fim da Gera��o do Arquivo Layout 5816





//==================================  In�cio do trecho compactando arquivos Semestrais para serem enviados ao BACEN
...
//Publishers J� em Opera��o constantes em arquivos BACEN anteriores CONCATENANDO com Nacionais
...
// Atualizando a variavel para verifica��o de novos Publishers Nacionais (CONCATENADO com os Internacionais).
$verificadorPublishersNovos = implode(",", $vetorPublisherNovos);
...
//echo "String contendo novos PUblisher ap�s concatenados: [".$verificadorPublishersNovos."]<br>";

//Dados necess�rios para os arquivos TRIMESTRAIS e MENSAIS
$codigoSegmento = '001';
$ano = $ano;
$trimestre = $mes;
$oitoPrimeiros = '19037276';     // 8 primeiros digitos do CNPJ E-PREPAG ADMINISTRADORA DE CARTOES
$funcao = 'E';          // Fun��o D�bito Fixa -- Alterado para E (pr�-pago) em 15/01/2018
$bandeira = 6;          // Bandeira Fixo 6 - Bandeira pr�pria
$formaCaptura = 4;      // Forma de Captura Fixa 4 - N�o presencial
$numeroParcelas = 1;    // Numero de Parcelas Fixo 1 - Somente pagamento � vista
$produto = 20;          // Produto fixo 20 - Outros
$modalidade = 'P';      // Modalidade fixo P - Cart�o emitido com bandeira de cr�dito sem associa��o com outra marca comercial, industrial ou sem fins lucrativos e cart�es com fun��o d�bito.
...
$custoTarifaIntercambio     = 0; // Custo da tarifa de interc�mbio
$custoTaxaAcessoBandeira    = 0; // Custo das taxas de acesso �s bandeiras
$ISPB = '19037276';              // C�digo ISPB cadastrado
$nomeEmissor = 'E-PREPAG ADMINISTRADORA DE CARTOES LTDA'; // Nome da institui��o emissora de cart�es de pagamento pertencente ao conglomerado financeiro.
$receitaEmissaoeFaturamento = 0;// Receitas obtidas junto aos credenciadores, provenientes de incentivos � emiss�o de cart�es de pagamento
$receitaFinanceira = 0;         // Receitas originadas pelo cr�dito rotativo bem como aquelas geradas por ganhos financeiros decorrentes de inadimpl�ncia (multas, juros, etc.)
$custoProgramaRecompensa = 0;   // Custos advindos das vantagens que o emissor oferece ao portador do cart�o, tais como descontos na tarifa de anuidade, programas de recompensa, seguros, etc.
$nomeArquivosTrimestrais = array(); //Lista de Arquivos a serem considerados na compacta��o de arquivos Trimestrais
$nomeArquivosSemestrais = array();  //Lista de Arquivos a serem considerados na compacta��o de arquivos Semestrais
...
// Teste de verifica��o se � m�s para gera��o dos arquivos Trimestrais
if(isTrimestral($mes)) {

    //================================== In�cio Verificando se os Dados Financeiros est�o devidamente apurados (Congelados na Gera��o de Notas)
    $sql = "
        select 
                fp_publisher, 
                data_sem_notas
    ...
    //================================== Verificando se os Dados Complementares est�o devidamente cadastrados
    $sql = "SELECT  *
            FROM complice 
    ...
    if((($rs_complice) ? pg_num_rows($rs_complice) : 0) == 1) {
        ...
        //================================== Verificando se os Dados Complementares de TODO o TRIMESTRE est�o devidamente cadastrados
        $sql = "SELECT  *
                FROM complice 
        ...
        if((($rs_complice) ? pg_num_rows($rs_complice) : 0) == 3 || $testeData == $dataInicioOperacao) {
            ...
            //==================================  Inicio do trecho a gera��o do arquivo SEGMENTO.TXT
            ...
            $vetorLines = array (
                                ...
                                1 => array('name' => 'Estabelecimentos publicadores de games que comercializam cr�ditos pr�-pagos',
                                           'size' => 250
                                            ),
            ...
            if($file->checkFile()){
               ...
               echo "<hr>Arquivo ".$file->getFileName()." n�o gerado.<br>\n"; 
            }
            //==================================  Fim do trecho a gera��o do arquivo SEGMENTO.TXT




            //==================================  Inicio do trecho a gera��o do arquivo RANKING.TXT
            $nomeArquivo = 'ranking.txt';
            $nomeArquivosTrimestrais[] = $nomeArquivo;
            unset($file);
            $file = new FilePosition($nomeArquivo);

            //Adicionando os publishers j� contabilizados em per�odos anteriores ao total de linhas
            ...
            // Buscando informa��es congeladas
            $sql = "
                select 
                        fp_publisher, 
            ...
            if($file->checkFile()){
               ...
               echo "Arquivo ".$file->getFileName()." n�o gerado.<br>\n"; 
            }
            //==================================  Fim do trecho a gera��o do arquivo RANKING.TXT




            //==================================  Inicio do trecho a gera��o do arquivo DESCONTO.TXT
            ...
            if(!$rs) echo "Erro ao selecionar o Publisher (".implode(",", $vetorPublisher).").<br>\n";
                ...
                if(!$rsDesvio) echo "Erro ao selecionar o Publisher Desvio Padr�o (".implode(",", $vetorPublisher).").<br>\n";
                else { 
            ...
            //==================================  Fim do trecho a gera��o do arquivo DESCONTO.TXT




            //==================================  Inicio do trecho a gera��o do arquivo INTERCAM.TXT
            ...
            //==================================  Fim do trecho a gera��o do arquivo INTERCAM.TXT




            //==================================  Inicio do trecho a gera��o do arquivo LUCRCRED.TXT
            ...
            //==================================  Fim do trecho a gera��o do arquivo LUCRCRED.TXT




            //==================================  Inicio do trecho a gera��o do arquivo CONCCRED.TXT
            ...
            //==================================  Fim do trecho a gera��o do arquivo CONCCRED.TXT




            //==================================  Inicio do trecho a gera��o do arquivo INFRESTA.TXT
            $nomeArquivo = 'infresta.txt';
            $nomeArquivosTrimestrais[] = $nomeArquivo;
            unset($file);
            $file = new FilePosition($nomeArquivo);

            //Criando vetor Auxiliar para elabora��o da query
            if(!empty($verificadorPublishersNovos)) {
            ...
            //==================================  Fim do trecho a gera��o do arquivo INFRESTA.TXT




            //==================================  Inicio do trecho a gera��o do arquivo INFRTERM.TXT
            ...
            //==================================  Fim do trecho a gera��o do arquivo INFRTERM.TXT




            //==================================  Inicio do trecho a gera��o do arquivo CONTATOS.TXT
            $nomeArquivo = 'contatos.txt';
            $nomeArquivosTrimestrais[] = $nomeArquivo;
            unset($file);
            $file = new FilePosition($nomeArquivo);

            $quantidadeLinhas = 4; //Devido a ser fixo somente 4 Contatos diretor respons�vel pela presta��o das informa��es, de dois t�cnicos designados como respons�veis e um contato institucional
            ...
            $vetorLines = array (
                                ...
                                4 => array('name' => 'ANALISTA CONT�BIL FINANCEIRO',
                                           'size' => 50
                                           ),
                                5 => array('name' => '11 3030-9105',
                                           'size' => 50
            ...
            //==================================  Fim do trecho a gera��o do arquivo CONTATOS.TXT

            
            
            
            //==================================  Inicio do trecho a gera��o do arquivo DATABASE.TXT
            ...
            $vetorHeader = array (
                                 ...
                                 3 => array('name' => $ano.$mes, //Data-base dos arquivos enviados (AAAAMM), correspondendo ao �ltimo m�s do trimestre de refer�ncia. Por exemplo, a data-base do quarto trimestre de 2018 � 201812. 
                                            'size' => 6
                                            ),
                            );
            ...
            //==================================  Fim do trecho a gera��o do arquivo DATABASE.TXT

            
            
            
            //==================================  In�cio do trecho compactando arquivos Trimestrais para serem enviados ao BACEN
...
// Teste de verifica��o se � m�s para gera��o dos arquivos Semestrais
if(isSemestral($mes)) {
    ...
    if((($rs_complice) ? pg_num_rows($rs_complice) : 0) == 1) {
        ...
        if((($rs_complice) ? pg_num_rows($rs_complice) : 0) == 6 || $testeData == $dataInicioOperacao) {
            ...
            //==================================  Inicio do trecho a gera��o do arquivo EMISSOR.TXT
            ...
            //==================================  Fim do trecho a gera��o do arquivo EMISSOR.TXT




            //==================================  Inicio do trecho a gera��o do arquivo PORTADOR.TXT
            ...
            // Dados 
            // 1� Trimestre
            //Dados sobre Taxas de Manuten��o Anual
            $sql = "
                SELECT 
            ...
            if(!$rsDadosManutecaoAnual) echo "Erro ao selecionar os Dados sobre Taxas de Manuten��o Anual (".implode(",", $vetorPublisher).").<br>\n";
            else { 
                //Capturando Dados sobre Taxas de Manuten��o Anual
                $rsDadosManutecaoAnual_row = pg_fetch_array($rsDadosManutecaoAnual);
            }//end else do if(!$rsDadosManutecaoAnual)

            //Rela��o de alicotas para o c�lculo de desvio padr�o
            $sql = "
                SELECT 
            ...
            if(!$rsRelacaoAlicota) echo "Erro ao selecionar a Rela��o de Alicotas dos Dados sobre Taxas de Manuten��o Anual (".implode(",", $vetorPublisher).").<br>\n";
            else { 
                    // C�lculo do DESVIO PADR�O
                    $desvio_padrao = 0;
            ...
            //echo "Desvio Padr�o:".$desvio_padrao." Desvio padr�o Formatado: ".number_format(($desvio_padrao), 2, ',', '.'); die();
            ...
            $vetorLines = array (
                                ...
                                6 => array('name' => number_format(($rsDadosManutecaoAnual_row['minimo']*100), 0, '', ''),     //Tarifa de anuidade m�nima
                                           'size' => 6
                                           ),
                                7 => array('name' => number_format(($rsDadosManutecaoAnual_row['media_simples']*100), 0, '', ''),     //Tarifa de anuidade m�dia
                                           'size' => 6
                                           ),
                                8 => array('name' => number_format(($rsDadosManutecaoAnual_row['maximo']*100), 0, '', ''),     //Tarifa de anuidade m�xima
                                           'size' => 6
                                           ),
                                9 => array('name' => number_format(($desvio_padrao*100), 0, '', ''),     //Desvio padr�o da tarifa de anuidade
                                           'size' => 6
                                           ),
                                10 => array('name' => 0,    //Estoque de pontos acumulados nas contas dos portadores
                                           'size' => 12
                                           ),
                                11 => array('name' => 0,    //Quantidade de pontos adquiridos no �mbito dos programas de recompensa do emissor
                                           'size' => 12
                                           ),
            ...
            //==================================  Fim do trecho a gera��o do arquivo PORTADOR.TXT




            //==================================  Inicio do trecho a gera��o do arquivo LUCREMIS.TXT
            ...
            //Total de taxas de manuten��o anual no trimestre
            $sql = "
            ...
            if(!$rsTotalManutecaoAnual) echo "Erro ao selecionar o Total de Taxas de Manuten��o Anual (".implode(",", $vetorPublisher).").<br>\n";
            else { 
                //Capturando Total de taxas de manuten��o anual no trimestre
            ...
            //==================================  Fim do trecho a gera��o do arquivo LUCREMIS.TXT



            //==================================  Inicio do trecho a gera��o do arquivo CONCEMIS.TXT
            ...
            // Dados 
            // 1� Trimestre
            // Capturando a quantidade total de cart�es emitidos
            $sql = "
                   select 
            ...
            if(!$rsInfoEmitidos) echo "Erro ao selecionar o Cart�es Emitidos.<br>\n";
            else { 
                //Capturando Dados Cart�es Emitidos
                $rsInfoEmitidos_row = pg_fetch_array($rsInfoEmitidos);
            }//end else do if(!$rsDesvio)
            
            // Capturando a quantidade total de cart�es ativos
            $sql = "
            select   
            ...
            if(!empty($verificadorPublishersNovos)) {
                foreach ($vetorPublisherNovos as $key => $value) {
                    ...
                    //Levantando data para verifica��o se deve ser incluido a parte da querie no select principal
                    ...
                    if(!$rsDataIncioOperacao) die("Erro ao selecionar a Data Ini�cio da Opera��o para o Publisher (".$value.").<br>\n");
                    else { 
                        //Capturando Dados da In�cio da Opera��o
                    ...
                    if($rsDataIncioOperacao_row['data_inicio'] <= getEndDateTrimestral(($mes-3),$ano)) {
                        //Parte que ir� unir a query principal
                        $sql .= "
            ...
            if(!$rsInfoAtivos) echo "Erro ao selecionar o Cart�es Ativos.<br>\n";
            else { 
                //Capturando Dados Cart�es Emitidos
            ...
            //==================================  Fim do trecho a gera��o do arquivo CONCEMIS.TXT
            ...
            $quantidadeLinhas = 8; //Devido a ser fixo somente 3 Contatos diretor respons�vel pela presta��o das informa��es, de dois t�cnicos designados como respons�veis
...
//==================================  In�cio do trecho da altera��o para j� em arquivo do BACEN 
if(!empty($verificadorPublishersNovos)) {
    alteracaoPublisherNovosJaArquivoBACEN($vetorPublisherNovos);
}//end if(!empty($verificadorPublishersNovos))
//==================================  Fim do trecho da altera��o para j� em arquivo do BACEN 
