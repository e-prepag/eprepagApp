...
if (isset($_POST["BtnConcluir"]) && !empty($_POST["BtnConcluir"])) {
    ...
    if ($zip->extractTo($caminho) != true) {
        ...
        // Se houver um erro de sistema, usa ele. Se não, usa o do Zip.
        if (isset($erroSistema['message'])) {
            $msg = "Erro de sistema ao extrair: " . $erroSistema['message'];
        } else {
            $msg = "Erro interno do Zip: " . $zip->getStatusString();
        }
    }
