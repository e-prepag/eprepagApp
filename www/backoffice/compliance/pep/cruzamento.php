    ...
    <table class="table table-bordered top20">
        ...
        ?>
                ...
                while ($rsRow = pg_fetch_array($rs)) {
                    ...
                    // Eliminando mascara e convertendo para inteiro
                    // O * 1 força a conversão para número, o que ajuda na segurança do SQL abaixo
                    $cpf_limpo = str_replace("-", "", str_replace(".", "", $rsRow['ug_cpf']));
                    $cpf_aux = $cpf_limpo * 1;

                    // Verificando conteudo válido para pesquisa
                    if ($cpf_aux > 0) {
                        // Buscando incidência na lista da PEP
                        // Nota: Usei $cpf_aux diretamente pois ele já está limpo
                        $sql = "SELECT * FROM pep WHERE cpf = " . $cpf_aux . ";";
                ...
                ?>
                                <tbody title="Nome Encontrado">
                                    <tr class="trListagem">
                                        ...
                                        <td><?php echo $rsEncontradoRow['descricao_funcao']; ?></td>
                                    </tr>
                                </tbody>
                            <?php
                            } // fim while rsEncontradoRow

                        } else {
                            // CASO 2: NÃO ENCONTROU NA PEP (Adicionado agora)
                            ?>
                            <tbody title="Não Encontrado">
                                <tr class="trListagem">
                                    <td><?php echo $rsRow['tipo']; ?></td>
                                    <td><?php echo $rsRow['ug_cpf']; ?></td>
                                    <td><?php echo $rsRow['ug_nome']; ?></td>
                                    <td style="color: red; font-style: italic;">Não encontrado na PEP</td>
                                    <td>-</td>
                                </tr>
                            </tbody>
                <?php
...
<link href="<?php echo $server_url_ep; ?>/prepag2/js/jqueryui/css/custom-theme/jquery-ui-1.9.2.custom.min.css" rel="stylesheet">
<script src="<?php echo $server_url_ep; ?>/prepag2/js/jqueryui/js/jquery-ui-1.9.2.custom.min.js"></script>
