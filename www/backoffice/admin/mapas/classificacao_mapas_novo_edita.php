        ...
        if(isset($_GET["id"]) && $_GET["id"] != ""){
            ...
            if($rsPublishers = SQLexecuteQuery($select)){
                if(pg_num_rows($rsPublishers) > 0){
                    ...
                    echo "<script>alert('Publisher não encontrado.'); location.href = '" . $raiz_do_projeto . ":$server_port/admin/mapas/classificacao_mapas.php';</script>";
                }
            }

        }else
            echo "<script>alert('Posicao para edição, não especificada'); location.href = '" . $raiz_do_projeto . ":$server_port/admin/mapas/classificacao_mapas.php';</script>";
        
        $method = "editaPosicao";
    }else
        header("Location: " . $raiz_do_projeto . ":$server_port/admin/banners/banners_posicoes.php");
...
<div class="col-md-12">
    <ol class="breadcrumb top10">
        ...
        <li class="active"><a href="<?php echo $sistema->item->getLink() ; ?>"><?php echo $_GET["acao"] == "novo" ? "Nova" : "Edição de"?> classificação de Mapa</a></li>
    </ol>
</div>
