...
if($testeSubmit=="Publicar") {
    //Inicia transacao
                ...
                for ($i=0;$i<count($ids_temp);$i++) {
                        if($msg == ""){
                                list($codlote,$codopr,$opr_codigo_aux) = explode("|",$ids_temp[$i]);
                                //echo "lote: ".$codlote."<br>";
                                //echo "codopr: ".$codopr."<br>";
                                // Passa para o novo estado
                                $sql  = "update pins_card set pin_status='".intval($PINS_STORE_STATUS_VALUES['P'])."' where pin_lote_codigo=".intval($codlote)." and pin_status='".intval($PINS_STORE_STATUS_VALUES['D'])."' and distributor_codigo=".intval($codopr)." and opr_codigo=".intval($opr_codigo_aux).";";
                                //echo $sql."<br>";//die();
                                $rs_pins_save = SQLexecuteQuery($sql);
                                if($rs_pins_save ) {
                                                $msg_pin .= "<span class='txt-verde'>Lote ($codlote) da Distribuidora (".$operacao_array[$codopr].") e Publisher (".$publisher_array[$opr_codigo_aux].") atualizado com sucesso.</span><br>";
                                } else {
                                                $msg = "Erro ao atualizar o Lote ($codlote) da Distribuidora (".$operacao_array[$codopr].") e Publisher (".$publisher_array[$opr_codigo_aux].")<br>$sql<br>";
                                }
                        }//end if($msg == "")
                } //end for
                if (count($ids_temp) == 0) {
...
if($msg == ""){
        $sql  = " 
                select opr_codigo, distributor_codigo, pin_lote_codigo, to_char(pin_dataentrada,'DD/MM/YYYY') as data, count(ps.pin_codinterno) as total, pin_formato, pin_valor 
                from pins_card ps 
                where pin_status='".intval($PINS_STORE_STATUS_VALUES['D'])."' ";
        ...
        $sql .= " group by opr_codigo,distributor_codigo, pin_lote_codigo, data, pin_formato, pin_valor ";
...
<div class="col-md-12">
     <ol class="breadcrumb top10">
         ...
         <li class="active">Publicar PINs Cart�es por LOTE</li>
     </ol>
 </div>
...
?>
<form name="form1" method="post" action="pins_card_publicar.php" onsubmit="javascript:return reload();">
		...
		<?php	
			...
			?>
    	        <tr<?php echo $bgcolor?> valign="top">
    	          <td class="texto" align="center">&nbsp;<input name="chkPIN[]" id="chkPIN" type="checkbox" value="<?php echo $rs_pins_row['pin_lote_codigo']."|".$rs_pins_row['distributor_codigo']."|".$rs_pins_row['opr_codigo'];?>"/>&nbsp;</td>
				  ...
				  <td class="texto" align="center"><nobr>&nbsp;<?php 
					$sql_total = "select count(ps.pin_codinterno) as total from pins_card ps where distributor_codigo = ".$rs_pins_row['distributor_codigo']."and to_char(pin_dataentrada,'DD/MM/YYYY') = '".$rs_pins_row['data']."' and pin_lote_codigo = ".$rs_pins_row['pin_lote_codigo'];
