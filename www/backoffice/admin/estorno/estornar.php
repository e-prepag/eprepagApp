...
if(isset($_POST["ug_id"]) && !empty($_POST["ug_id"]) && isset($_POST["valor"]) && !empty($_POST["valor"])){
    ...
    if($valor > MAX_VALOR){
        ...
        if((($ret) ? pg_num_rows($ret) : 0) == 0){
			...
			$sql = "INSERT INTO estorno_pdv(shn_id, shn_login, ug_id, ug_login, ug_saldo_anterior, ug_saldo_atual, est_tipo, est_valor, ug_descricao) VALUES ($1, $2, $3, $4, $5, $6, 0, $7, $8)";
				$ret = SQLexecuteQueryParams($sql, array($_SESSION['iduser_bko'], $_SESSION['userlogin_bko'], $ug_id, $ug_login, $saldo_anterior, $valor_atual, $valor, $_POST["descricao"]));
			if(!$ret){
...
$sql = "SELECT count(*) as total FROM estorno_pdv WHERE est_tipo = 0";
$rs_total = SQLexecuteQuery($sql);
$row = pg_fetch_assoc($rs_total);
if($row) $registros_total = $row["total"];


$sql = "SELECT * FROM estorno_pdv WHERE est_tipo = 0";
    ...
    <div class="col-md-12">
        ...
        <form id="form-estorno" href="#" class="form-inline" method="POST">
            ...
            <div class="form-group left20">
                <label for="exampleInputEmail2">Valor a ser estornado</label>
            ...
            <button id="confirma-estorno" class="btn btn-default left20">Estornar</button>
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12 text-center" style="color: black;">
                        Você está prestes a adicionar o valor de R$<span id="valor-modal"></span> ao PDV <span id="id-modal"></span><br>
                        Deseja continuar ?
                    </div>
                </div>
                <div class="row top20">
