...
if (!empty($BtnGerarArq) && $tf_v_tipo == 3) {
	...
	if (flag_publisher_test($distributor_codigo)) {
		...
		if (!$ret)
			$msg .= "<font color='#FF0000'><b>Erro ao iniciar transa&cceil;&atilde;o." . PHP_EOL . "</b></font><br>";
		...
		if ($msg == "") {
			...
			$ret = SQLexecuteQuery($sql);
			if (!$ret)
				$msg .= "<font color='#FF0000'><b>Erro ao comitar transa&ccedil;&atilde;o." . PHP_EOL . "<br></b></font><br>";
		} else {
			gravaLog_Depurador("ROLLBACK SQLs!" . PHP_EOL);
			$sql = "ROLLBACK TRANSACTION ";
			$ret = SQLexecuteQuery($sql);
			if (!$ret)
				$msg .= "<font color='#FF0000'><b>Erro ao dar rollback na transa&ccedil;&atilde;o." . PHP_EOL . "<br></b></font><br>";
			echo $msg;
		}
