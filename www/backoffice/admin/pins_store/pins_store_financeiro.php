...
<!--
	function reload() {
		document.form1.action = "pins_store_financeiro.php";
...
<?php
	...
	if($msg == "" && $btPesquisar=='Pesquisar'){
		$sql  = "select 
					ps.pin_formato, 
					ps.distributor_codigo, 
					psra.psra_nome, 
					to_char(psra.psra_dataentrada,'DD/MM/YYYY HH24:MI:SS') as data_envio,	
					ps.pin_lote_codigo, 
					ps.pin_valor,
					psra.psra_codinterno
				from pins_store ps, 
			...
			if(strlen($rs_pins_row['psra_codinterno'])) {
				$sql_total = "
					select 
						...
						count(case when ((ps.pin_arq_gerado is not null) AND (ps.pin_status != '".intval($PINS_STORE_STATUS_VALUES['U'])."') AND (ps.pin_status != '".intval($PINS_STORE_STATUS_VALUES['C'])."') AND (ps.pin_status != '".intval($PINS_STORE_STATUS_VALUES['B'])."')) then 1 end) as circulante,
						count(case when ((ps.pin_arq_gerado is not null) AND (ps.pin_status = '".intval($PINS_STORE_STATUS_VALUES['U'])."')) then 1 end) as utilizados
					from pins_store ps
...
//}
				...
				$sql_total = "
					...
					from pins_store ps 
						left outer join pins_store_apl_historico psah ON (ps.pin_codinterno = psah.psah_pin_id)
					where ps.pin_lote_codigo = ".$rs_pins_row['pin_lote_codigo']." and
						  ps.distributor_codigo = ".$rs_pins_row['distributor_codigo']." and
						  ps.pin_arq_gerado = ".$rs_pins_row['psra_codinterno'];
...
// fim totalizador geral
		...
		$sql .= " order by distributor_codigo,psra.psra_dataentrada DESC, pin_valor ";	
...
<div class="col-md-12">
    <ol class="breadcrumb top10">
        ...
        <li class="active">Relat&oacute;rio Financeiro de PINs E-PREPAG</li>
    </ol>
</div>  
...
?>
<form name="form1" method="post" action="pins_store_financeiro.php" onsubmit="javascript:return reload();">
    <table class="table txt-preto fontsize-pp">
	<?php
	if(isset($msg_pin) && $msg_pin) echo "<tr><td>".$msg_pin."</td></tr>";
	?>
			...
			<table class="table txt-preto fontsize-pp">
    	        ...
    	        <tr bgcolor="F5F5FB">
    	          <td class="texto" align="center"><nobr>&nbsp;
                      <select id='tf_v_formato' name='tf_v_formato'>
		...
		<?php
			...
			?>
    	        <tr<?php echo $bgcolor?> valign="top">
    	          ...
    	          <td class="texto" align="center"><nobr>&nbsp;<?php echo $rs_pins_row['psra_nome']?>&nbsp;</nobr></td>
