...
<div class="container-titulo">
	<?php if ($msg != "") { ?>
		<div style="color: <?php echo (isset($msg) && $msg["tipo"] == "exito") ? "green" : "red"; ?>;" class="alerta">
			<span>&#9888;</span> <?php echo ($msg != "") ? $msg["mensagem"] : ""; ?></div>
	<?php } ?>
	<div class="borda"></div>
	...
	<form class="dados" action="https://<?php echo $server_url_complete; ?>/pdv/keys/key.php" method="POST">
		...
		<input type="number" name="idPdv" value="<?php echo isset($_POST["idPdv"]) ? $_POST["idPdv"] : ""; ?>"
			class="inpId" placeholder="Digite c&#243;digo do pdv">
		<input type="text" name="acesso" value="<?php echo isset($_POST["acesso"]) ? $_POST["acesso"] : ""; ?>"
			class="inpId" placeholder="Digite o acesso do pdv">
...
<div class="container-titulo">
	...
	<form action="https://<?php echo $server_url_complete; ?>/pdv/keys/key.php" class="dados" method="POST">
		<input type="number" value="<?php echo isset($_POST["selecaoPdv"]) ? $_POST["selecaoPdv"] : ""; ?>"
			name="selecaoPdv" class="inpId" placeholder="Digite c&#243;digo do pdv">
					...
					if ($resultadoSelecao != false) {
						...
						foreach ($resultadoSelecao as $key => $value) {
							...
							<tr>
								...
								<td class="colt">
									<?php echo (!is_numeric($situacao)) ? $situacao : (($situacao == 1) ? "Ativo" : "Inativo"); ?></td>
								<td class="colt">
									<?php echo (!is_numeric($situacao)) ? $situacao : (($situacao == 1) ? '<a href="https://' . $server_url_complete . '/pdv/keys/key.php?cod=' . $value["user_id"] . '&action=1">&#10060;</a>'
		...
		<div class="container-rodape">
			...
			<div class="container-botoes-paginacao">
				<?php if ($resultadoTotal["numtotal"] > $limiteporpagina) { ?>
					<span class="rodape btn-rodape"><a
							href="https://<?php echo $server_url_complete; ?>/pdv/keys/key.php?action=ante&ini=<?php echo ($inicial <= 0) ? 0 : $inicial - $limiteporpagina; ?>">anterior</a></span>
					<span class="rodape btn-rodape"><a
							href="https://<?php echo $server_url_complete; ?>/pdv/keys/key.php?action=pro&ini=<?php echo ($inicial >= $resultadoTotal["numtotal"]) ? $inicial : $inicial + $limiteporpagina; ?>">proxima</a></span>
				<?php } ?>
			</div>
		</div>
	<?php } ?>
