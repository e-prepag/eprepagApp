...
$bret = $usuarioGames->b_IsLogin_pagamento_gold(1, $usuarios_pagamento_online_gold_id);
$total_table = (is_countable($usuarios_pagamento_online_gold_id) ? count($usuarios_pagamento_online_gold_id) : 0);
$reg_ate = $total_table;

$s_ug_id_list = "";
foreach($usuarios_pagamento_online_gold_id as $key => $val) {
...
<table class="table txt-preto fontsize-pp">
  <tr> 
    <td> 
        <form name="form1" method="post" action="com_pesquisa_usuarios_gold.php">
					...
					<?php
						...
						if(((($rs) ? pg_num_rows($rs) : 0) != 0) && ($rs)) {
							while ($pgrs = pg_fetch_array ($rs)) {
								{
                                                                        if($pgrs['vg_valor'] + $pgrs['vg_valor_inc'] == 0){
