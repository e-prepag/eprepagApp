...
if (isset($_POST["acao"]) && $_POST["acao"] == "listar") {
	...
	if (count($resultRows) > 0) {
		foreach ($resultRows as $key => $value) {
			...
			$acao = '';
			if ($value["pin_status"] != 8 && $value["pin_status"] != 9 && ($value["opr_pin_epp_formato"] != null || in_array($value["opr_codigo"], [49, 53]))) {
				$acao = '<button class="btn btn-danger btn-negar right10 bottom10" data-codigo="' . $value["pin_codinterno"] . '" data-idpdv="' . $value["ug_id"] . '" data-atual="' . $_POST["reload"] . '">Cancelar</button>';
			}
	...
	$selectRow->execute();
	// Pegamos os pins atualizados
	$updatedPins = $selectRow->fetchAll(PDO::FETCH_COLUMN);

	// Verifica se há pins para atualizar na segunda query
	if (!empty($updatedPins)) {
		// Converte o array de pins em uma string para a query
		$pinsList = implode(",", array_map(function ($pin) {
			return "'$pin'";
		}, $updatedPins));

		// 3. Verifica se há conexão com pins_store
		$queryCheckPinsStore = "SELECT DISTINCT pins_store_pin_codinterno 
			FROM tb_pins_store_pins 
			WHERE pins_pin_codinterno IN ($pinsList);";

		$checkStmt = $conexao->prepare($queryCheckPinsStore);
		$checkStmt->execute();
		$connectedPins = $checkStmt->fetchAll(PDO::FETCH_COLUMN);

		// 4. Se houver conexões, faz o UPDATE em pins_store
		if (!empty($connectedPins)) {
			$connectedPinsList = implode(",", array_map(function ($pin) {
				return "'$pin'";
			}, $connectedPins));

			$queryUpdateStore = "UPDATE pins_store
				SET pin_status = -1
				WHERE pin_codinterno IN ($connectedPinsList);";

			$updateStoreStmt = $conexao->prepare($queryUpdateStore);
			$updateStoreStmt->execute();

			if (!($updateStoreStmt->rowCount() > 0)) {
				$conexao->rollBack();
	...
	if ($selectRow->rowCount() > 0) {
		...
		// 3. Verifica se esse pin tem ligação com pins_store
		$queryCheckPinsStore = "SELECT pins_store_pin_codinterno 
        FROM tb_pins_store_pins 
        WHERE pins_pin_codinterno = :CODIGO;";

		$checkStmt = $conexao->prepare($queryCheckPinsStore);
		$checkStmt->bindValue(":CODIGO", $_POST["pin"]);
		$checkStmt->execute();

		$connectedPin = $checkStmt->fetchColumn();

		// 4. Se houver ligação, atualiza também na pins_store
		if ($connectedPin) {
			$queryUpdateStore = "UPDATE pins_store
            SET pin_status = -1
            WHERE pin_codinterno = :STORE_CODIGO;";

			$updateStoreStmt = $conexao->prepare($queryUpdateStore);
			$updateStoreStmt->bindValue(":STORE_CODIGO", $connectedPin);
			$updateStoreStmt->execute();
