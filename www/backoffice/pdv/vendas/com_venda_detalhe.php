...
if(!function_exists('isVendaDeposito')) {
	...
	function isVendaDeposito($venda_id){
		...
		if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada (em isvendaDeposito($venda_id)).\n";
...
if($msg == ""){
        ...
        if(!empty($vgm_id)) {
            ...
            else $sql .="vgm_id = ".$vgm_id." ;";
            //die($sql);
            $rs_cpf = SQLexecuteQuery($sql);
            if(pg_affected_rows($rs_cpf) < 1) $msg = "Erro ao atualizar os Dados de CPF!";
            unset($vgm_nome_cpf);
            unset($vgm_cpf);
                ...
                if($msgConcilia == ""){
                        ...
                        if(!$rs || pg_num_rows($rs)==0) {
                                ...
                                if($vg_deposito_em_saldo==1) {
                                        ...
                                        if($msgConcilia == "") $BtnProcessaEmail = 1;
                                }
                        }				
                }
        }
        ...
        if($BtnProcessaEmail){
                ...
                if($msgConcilia == ""){
                        ...
                        if($msgConcilia == "") $msgConciliaUsuario .= "Envio de email: Enviado com sucesso.\n";
...
if($msg == ""){
        ...
        if(!$rs_venda_modelos || pg_num_rows($rs_venda_modelos) == 0) $msg = "Nenhum produto encontrado (ABCD).\n";
        else {
...
if($bDebug) echo "SQL historico: ".$sql."<br>";
	...
	<?php  }?>
        ...
        <table class="table txt-preto">
          ...
          <tr bgcolor="#F5F5FB"> 
            ...
            <td><?php 
				echo $vgm_pin_codinterno_tmp." (".((trim($vgm_pin_codinterno_tmp)=="")?"<font color='#0000FF'>Sem PINs vendidos</font>":"<font color='#FF0000'>ATENÇÃO: Já tem PINs vendidos</font>").")";
				?></td>
          </tr>
          <tr bgcolor="#F5F5FB"> 
            <td><b>PINs</b></td>
...
<?php                        
        ...
        <table  class="table">
		  ...
		  <tr bgcolor="#F5F5FB"> 
			<td width="150"><b>C&oacute;digo</b></td>
			<td><a style="text-decoration:none" href="/pdv/usuarios/com_usuario_detalhe.php?usuario_id=<?php  echo $vg_ug_id ?>"><?php  echo $vg_ug_id ?></a></td>
		  </tr>
		  <?php if($_SESSION["visualiza_dados"] == "S"){ ?>
			  <tr bgcolor="#F5F5FB"> 
				<td width="150"><b>Login</b></td>
				<td><a style="text-decoration:none" href="/pdv/usuarios/com_usuario_detalhe.php?usuario_id=<?php  echo $vg_ug_id ?>"><?php  echo $ug_login ?></a></td>
			  </tr>
		  <?php } ?>
...
?>
		...
		<tr bgcolor="#F5F5FB">
			...
			<td><?php  for($j = 0; $j < count($arquivos); $j++){ ?><a style="text-decoration:none" target="_blank" href="/pdv/pagamentos/com_pagto_compr_down.php?venda=<?php echo $venda_id?>&arquivo=<?php echo $arquivos[$j]?>">Comprovante <?php echo ($j+1)?></a><br><?php  } ?></td>
		</tr>
		<?php  	} ?>
        </table>
        <table class="table txt-preto">
            <tr bgcolor="#FFFFFF"> 
              <td colspan="2" bgcolor="#ECE9D8">Dados do CPF Informado para este Pedido</font></td>
            </tr>
            <tr>
                <td>
                ...
                <form name="form5" id="form5" method="post" action="">
                    ...
                    <table class="table">
                        ...
                        <tr bgcolor="F0F0F0" class="texto">
                          <td colspan="" align="left"><input type="checkbox" name="todos" id="todos" value="1" onclick="javascript:yesnoCheck();" /><b>Utilizar o mesmo CPF em todos os modelos na alteração.</b></td>
                          <td align="left"><input type="text" name="vgm_cpf[1]" id="vgm_cpf[1]" value="" maxlength="14" size="14" class="texto cpf"/></td>
                          <td align="left"></td>
                          <td align="left"><input type="text" name="vgm_cpf_data_nascimento[1]" id="vgm_cpf_data_nascimento[1]" value="" maxlength="10" size="10" class="texto data_nasci"/></td>
                          <td align="right"><span name="botao[1]" id="botao[1]" class="glyphicon glyphicon-save-file graphycon-big" aria-hidden="true" style="font-size: 22px;top:0px !important;cursor:pointer;cursor:hand;" alt="Salvar" title="Salvar" onclick="salvaDadosCPF(1);"></span></td>
...
?>
                        <tr class="texto" bgcolor="#F5F5FB">
                            ...
                            <td align="left"><input type="text" name="vgm_cpf[<? echo $key;?>]" id="vgm_cpf[<? echo $key;?>]" value="<?php echo $value;?>" maxlength="14" size="14" class="texto cpf"/></td>
                            <td align="left"><?php echo $vgm_nome_cpf[$key];?></td>
                            <td align="left"><input type="text" name="vgm_cpf_data_nascimento[<? echo $key;?>]" id="vgm_cpf_data_nascimento[<? echo $key;?>]" value="<?php echo formata_data_ts($vgm_cpf_data_nascimento[$key], 0, false, false); ?>" maxlength="10" size="10" class="texto data_nasci"/></td>
                            <td align="right"><span class="glyphicon glyphicon-save-file graphycon-big" aria-hidden="true" style="font-size: 22px;top:0px !important;cursor:pointer;cursor:hand;" alt="Salvar" title="Salvar" onclick="salvaDadosCPF(<?php echo $key;?>);"></span></td>
...
?>
                ...
                <?php } else echo "Este pedido possui produtos que não exigem CPF."; ?>
                </td>
            </tr>
        </table>
...
<?php  }
       ...
       if($vg_ultimo_status == $GLOBALS['STATUS_VENDA']['PEDIDO_EFETUADO'] || $vg_ultimo_status == $GLOBALS['STATUS_VENDA']['PEDIDO_EM_STANDBY'] || $vg_ultimo_status == $GLOBALS['STATUS_VENDA']['AGUARDANDO_PROCESSAMENTO']) {?>
			...
			<table  class="table">
			  ...
			  <tr bgcolor="#F5F5FB"> 
				<td align="center" width="50%"><input type="submit" name="BtnProcessa" value="Processar" class="botao_search"></td>
				<td align="center" width="50%"><input type="submit" name="BtnProcessaAgendamento" value="Agendar Processamento" class="botao_search"></td>
			  </tr>
			</table>
			</form>
<?php  }
       ...
       if($vg_ultimo_status == $GLOBALS['STATUS_VENDA']['PROCESSAMENTO_REALIZADO'] || $vg_ultimo_status == $GLOBALS['STATUS_VENDA']['VENDA_REALIZADA']) {?>
			...
			<table  class="table">
			  ...
			  <tr bgcolor="#F5F5FB"> 
				<td colspan="2" align="center"><input type="submit" name="BtnProcessaEmail" value="Processar Email" class="botao_search"></td>
			  </tr>
			</table>
			</form>
<?php  }
       ...
       if($vg_ultimo_status == $GLOBALS['STATUS_VENDA']['PEDIDO_EFETUADO'] || $vg_ultimo_status == $GLOBALS['STATUS_VENDA']['PEDIDO_EM_STANDBY'] || $vg_ultimo_status == $GLOBALS['STATUS_VENDA']['AGUARDANDO_PROCESSAMENTO']) {?>
			...
			<table  class="table">
			  ...
			  <tr bgcolor="#F5F5FB">
				<td align="center" colspan="2"><input type="submit" name="BtnCancelar" value="Cancelar Venda" class="botao_search" onClick="return confirm('Deseja realmente cancelar esta venda ?');"></td>
			  </tr>
			</table>
			</form>
...
?>
			<table  class="table">
			  <tr bgcolor="#F5F5FB"> 
				<td colspan="2" align="center">
					<input disabled type="button" name="BtnAnterior" value="Anterior não conciliado" class="botao_search" onClick="window.location='com_fila_vendas.php?venda_id=<?php  echo $venda_id?>&fila_ncamp=<?php  echo $fila_ncamp?>&fila_ordem=1<?php echo $varsel?>';">
					&nbsp;&nbsp;&nbsp;
					<input type="button" name="BtnVoltar" value="Voltar" class="botao_search" onClick="window.location='index.php'">
					&nbsp;&nbsp;&nbsp;
					<input disabled type="button" name="BtnProximo" value="Próximo não conciliado" class="botao_search" onClick="window.location='com_fila_vendas.php?venda_id=<?php  echo $venda_id?>&fila_ncamp=<?php  echo $fila_ncamp?>&fila_ordem=2<?php echo $varsel?>';">
				</td>
			  </tr>
			</table>
		...
		while($rs_mod_row = pg_fetch_array($rs_mod)) {;
                    ...
                    if(!$rs_pin || pg_num_rows($rs_pin) == 0) {
                            ...
                            while($rs_pin_row = pg_fetch_array($rs_pin)) {
                                    if($vgm_pin_codinterno) $vgm_pin_codinterno .= ", ";
