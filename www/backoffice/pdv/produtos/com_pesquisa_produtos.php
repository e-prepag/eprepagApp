...
$tf_ogp_mostra_integracao_gamer = $_POST['tf_ogp_mostra_integracao_gamer'] ?? null;
...
$varsel = "&BtnSearch=1&tf_ogp_id=$tf_ogp_id&tf_ogp_ativo=$tf_ogp_ativo&tf_ogp_mostra_integracao_gamer=$tf_ogp_mostra_integracao_gamer";
...
if(isset($BtnSearch)){
	...
	if($msg == ""){
		...
		if(!is_null($tf_ogp_mostra_integracao_gamer) && ($tf_ogp_mostra_integracao_gamer!=""))	{
			$filtro['ogp_mostra_integracao_gamer']= $tf_ogp_mostra_integracao_gamer;
		}
		...
		if($ret != "") $msg = $ret;
			...
			if($total_table == 0) {
				...
				$orderBy .= " offset ".$inicial;
                                $instProduto = new Produto();
				$ret = $instProduto->obterMelhorado($filtro, $orderBy, $rs_produtos);
				if($ret != "") $msg = $ret;
				else {
...
<div class="col-md-12">
    <a href="com_produto_detalhe_insere.php" class="btn btn-sm btn-info">Inserir novo produto</a>
</div>
<div class="col-md-12 top10">
<table class="txt-preto table fontsize-p">
  <tr> 
    <td width="891" valign="top"> 
        <form name="form1" method="post" action="com_pesquisa_produtos.php">
          ...
          <tr bgcolor="#F5F5FB"> 
			...
			<td>
				<select name="tf_ogp_mostra_integracao_gamer" class="form2">
					<option value="" <?php if($tf_ogp_mostra_integracao_gamer == "") echo "selected" ?>>Selecione</option>
					<option value="0" <?php if ($tf_ogp_mostra_integracao_gamer == "0") echo "selected";?>>NÃO</option>
					<option value="1" <?php if ($tf_ogp_mostra_integracao_gamer == "1") echo "selected";?>>SIM</option>
				</select>
			</td>
          </tr>
          <tr bgcolor="#FFFFFF"> 
                ...
                <tr bgcolor="#00008C"> 
                  <td height="11" colspan="3" bgcolor="#FFFFFF"> 
                      ...
                      <tr  bgcolor="#ECE9D8"> 
                        ...
                        <td align="center"><strong><a href="<?php echo $default_add."?ordem=".$ordem."&ncamp=ogp_mostra_integracao_gamer&inicial=".$inicial.$varsel ?>" class="link_branco"><font class="texto">Exibido na<br>Integração</font></a>
                          <?php if($ncamp == 'ogp_mostra_integracao_gamer') echo "<img src=".$img_seta." width='10' height='7' align='absmiddle'>"; ?>
                          </strong></td>
					...
					<?php
						...
						while($rs_produtos_row = pg_fetch_array($rs_produtos)){
							...
							$ogp_mostra_integracao_gamer = ($rs_produtos_row['ogp_mostra_integracao_gamer'] == 1)?"Sim":"Não";
							$opr_status = ($rs_produtos_row['opr_status'] == 1)?"Ativo":"Inativo";
					?>
                      <tr class="trListagem"> 
                        <td class="texto" align="center"><a style="text-decoration:none" href="com_produto_detalhe.php?produto_id=<?php echo $rs_produtos_row['ogp_id'] ?>"><?php echo $rs_produtos_row['ogp_id'] ?></a></td>
                        ...
                        <td class="texto" align="center"><?php echo $ogp_mostra_integracao_gamer ?></td>
