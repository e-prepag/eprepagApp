...
if($Submit){
	...
	if($fopr){			
		$resopr_val = SQLexecuteQueryParams("select opr_codigo from operadoras where opr_codigo=$1", [(int)$fopr]);

                $pgopr_val = pg_fetch_array($resopr_val);
			$sql_resval = "select pin_valor from pins where opr_codigo=$1";
			$params_resval = [(int)$pgopr_val['opr_codigo']];
			if($fcanal=='s' || $fcanal=='p' || $fcanal=='r'){
				$sql_resval .= " and pin_canal=$2";
				$params_resval[] = $fcanal;
			}
			$sql_resval .= " group by pin_valor order by pin_valor";
			$resval = SQLexecuteQueryParams($sql_resval, $params_resval);
		...
		$paramsEstat = [];
		if($fopr){
			$idxOpr = count($paramsEstat) + 1;
			$paramsEstat[] = (int)$fopr;
			$sql .= "and (t0.opr_codigo=$".$idxOpr.") and (t0.opr_codigo=t1.opr_codigo) ";
		}
		if($fvalor){
			$idxValor = count($paramsEstat) + 1;
			$paramsEstat[] = $fvalor;
			$sql .= "and (t0.pin_valor=$".$idxValor.") ";
		}
		if($fcanal=='s' || $fcanal=='p' || $fcanal=='r'){
			$idxCanal = count($paramsEstat) + 1;
			$paramsEstat[] = $fcanal;
			$sql .= "and (t0.pin_canal=$".$idxCanal.") ";
		}
		...
		$orderCampo = (preg_match('/^[a-zA-Z0-9_.]+$/', $ncamp) ? $ncamp : 'opr_nome');
		$sql .= "order by opr_nome, " . $orderCampo . ", pin_valor, pin_status";
		$resestat = SQLexecuteQueryParams($sql, $paramsEstat);
		...
		$paramsMedia = [];
		if($fopr){
			$idxOprM = count($paramsMedia) + 1;
			$paramsMedia[] = (int)$fopr;
			$sql .= "and (t0.opr_codigo=$".$idxOprM.") and (t0.opr_codigo=t1.opr_codigo) ";
		}
		if($fvalor){
			$idxValorM = count($paramsMedia) + 1;
			$paramsMedia[] = $fvalor;
			$sql .= "and (t0.pin_valor=$".$idxValorM.") ";
		}
		if($fcanal=='s' || $fcanal=='p' || $fcanal=='r'){
			$idxCanalM = count($paramsMedia) + 1;
			$paramsMedia[] = $fcanal;
			$sql .= "and (t0.pin_canal=$".$idxCanalM.") ";
		}
		if(!$ChkTreinamento && $fopr <> 78) $sql .=" and (t0.opr_codigo <> 78) ";
		if(!$fopr && !$fvalor){ $sql .= " and (t0.opr_codigo=t1.opr_codigo) "; }
		$sql .= "group by t1.opr_nome, t0.pin_valor, t0.opr_codigo, t1.opr_pedido_estoque_prazo ";
		$sql .= "order by " . $orderCampo . ", pin_valor";

		$sqlMedia = $sql;
		$rs_Media = SQLexecuteQueryParams($sqlMedia, $paramsMedia);
...
*/	
	$varsel = "&fopr=$fopr&fvalor=$fvalor";
		...
		if($fopr) {
			$resec=SQLexecuteQueryParams("select ec_uf,ec_codigo from estab_comissao where ec_opr_codigo = $1 order by ec_uf asc", [(int)$fopr]);
		}
/*	
...
*/
		...
		$paramsEsgotados = [];
		if($fcanal=='s' || $fcanal=='p' || $fcanal=='r'){
			$idxCanalEsg = count($paramsEsgotados) + 1;
			$paramsEsgotados[] = $fcanal;
			$sql .= "and (pins.pin_canal=$".$idxCanalEsg.") ";
		}
		if($Submit &&($fopr!="")){
			$idxOprEsg = count($paramsEsgotados) + 1;
			$paramsEsgotados[] = (int)$fopr;
			$sql .= "and operadoras.opr_codigo=$".$idxOprEsg." ";
		}
		$sql .= "and (not (operadoras.opr_codigo=17 and pins.pin_valor=26)) ";
		$sql .= "except ";
		$sql .= "select distinct operadoras.opr_nome, pins.opr_codigo, pins.pin_valor from pins inner join operadoras ";
		$sql .= "on pins.opr_codigo = operadoras.opr_codigo ";
		$sql .= "where operadoras.opr_status='1' and operadoras.opr_pin_online = 0 and pins.pin_status = '1' ";
		if($fcanal=='s' || $fcanal=='p' || $fcanal=='r'){
			$idxCanalEsg2 = count($paramsEsgotados) + 1;
			$paramsEsgotados[] = $fcanal;
			$sql .= "and (pins.pin_canal=$".$idxCanalEsg2.") ";
		}
		if($Submit &&($fopr!="")){
			$idxOprEsg2 = count($paramsEsgotados) + 1;
			$paramsEsgotados[] = (int)$fopr;
			$sql .= "and operadoras.opr_codigo=$".$idxOprEsg2." ";
		}
	        $sql .= " order by opr_nome, pin_valor; ";
		$rs_esgotados = SQLexecuteQueryParams($sql, $paramsEsgotados);
}
...
<?php
                        ...
                        while ($pgest = pg_fetch_array($rs_esgotados))
                        {
                            ...
                            $rs_count_lan = SQLexecuteQueryParams($sql, [(int)$pgest['opr_codigo'], $pgest['pin_valor']]);
                            ...
                            $rs_count_gamer = SQLexecuteQueryParams($sql, [(int)$pgest['opr_codigo'], $pgest['pin_valor']]);
