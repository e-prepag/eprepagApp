	...
	function processaLote_HabboHotel($fileSource, $nomeArq, $opr_codigo, $loteValor, $fcanal){
		...
		if ($msg == "") {
			...
			if (!$rs) {
				...
				while ($row = pg_fetch_assoc($rs)) {
					$duplicatedPins[] = $row['pin_caracter'];
				}
	
				if (!empty($duplicatedPins)) {
					$msg = "Os seguintes PINs já existem: " . implode(", ", $duplicatedPins) . "\n";
				}
			}
		}
		...
		if ($msg == "") {
			...
			foreach (array_chunk($cargaAr, $batchSize) as $batch) {
				...
				$sql = "
					INSERT INTO pins (
						opr_codigo, pin_status, pin_serial, pin_codigo,
						pin_local, pin_valor, pin_lote_codigo, pin_dataentrada,
