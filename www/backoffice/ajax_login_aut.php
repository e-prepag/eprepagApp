...
try {
    ...
    if (Util::isAjaxRequest()) {
        ...
        if ($stmt && $stmt->rowCount() > 0) {
            ...
            echo "<script>alert('Usu�rio ou senha inv�lidos aaaa');</script>";
        }
    }
...
function modal_criar_token($fetch)
{
    ...
    $btn_recusar = true;
    if ($diasRestantes > 0) {
        $mensagemAuth = "Ative a autentica��o de dois fatores, voc� tem <strong>{$diasRestantes} dias</strong> antes que se torne obrigat�ria.";
    } else {
        $mensagemAuth = "O prazo para ativar a autentica��o de dois fatores expirou, � necess�rio configur�-la.";
        $btn_recusar = false;
    }
                    ...
                    <?php if ($btn_recusar) { ?>
                        <div class="dislineblock">
                            <form id="redir" method="POST" action="/index2.php">
                                ...
                                <button style="font-weight: bold; font-style: italic;" class="pull-right btn btn-info" type="submit">N�o</button>
                            </form>
                        </div>
                    <?php } ?>
                </div>
                <div class="modal-footer">
                    <span class="decoration-none txt-cinza"><em>Problemas com a autentica��o?</em></span>
        ...
        <div class="modal-dialog modal-sm">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title text-left">Digite o token dispon�vel no seu app de autentica��o</h4>
                </div>
                <div class="modal-body espacamento">
