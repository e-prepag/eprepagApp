<?php
require ENDERECO_BASE_CIELO_PAG_INCLUDES."classLogger.php";
...
class Pedido
{
        ...
        private function XMLDadosPedido()
        {
                ...
                if($this->softDescriptor != null && $this->softDescriptor != "")
                {
                        $msg .= '<soft-descriptor>'
                                . $this->softDescriptor .
                                '</soft-descriptor>' . "\n      ";
        ...
        public function Enviar($vmPost, $transacao) {
                ...
                $vmResposta = cielo_httprequest(ENDERECO, "mensagem=" . $vmPost);
                $this->logger->logWrite("RESPOSTA: " . $vmResposta, $transacao);
                gravaLog_CIELO_TMP("Em Enviar(3): transacao= '$transacao'\n vmPost: ".print_r($vmPost, true)."\n vmResposta ".print_r($vmResposta, true)."\n");
                VerificaErro($vmPost, $vmResposta);
                gravaLog_CIELO_TMP("\nEm Enviar(4) - vmResposta: '".$vmResposta."'\n");
                $pattern = "/error:/";
                if(preg_match($pattern, $vmResposta)){
                   return NULL;
                } 
                else{
        ...
        {
                $msg = $this->XMLHeader() . "\n" .
                           '<requisicao-transacao id="' . $this->dadosPedidoNumero . '" versao="' . VERSAO . '">' . "\n   "
                                        . $this->XMLDadosEc() . "\n   "; //md5(date("YmdHisu"))
        ...
        public function RequisicaoConsulta()
        {
                if($this->tid != "") {
                    ...
                    $this->logger->logWrite("TID VAZIO", "Consulta");
                    return null;
                }
        }
