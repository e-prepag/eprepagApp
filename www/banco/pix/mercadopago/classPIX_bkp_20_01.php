...
class classPIX
{
    ...
    public function callSonda($params, &$reposta_consulta)
    {
        ...
        if (!$status) {
            ...
            if ($status == PIX_SONDA_PAGO_OK) {
                $cpf = $data['results'][0]['identification']['number'] ? $data['results'][0]['payer']['identification']['number'] : 'N/A';
                $name = $data['results'][0]['first_name'] ? $data['results'][0]['payer']['first_name'] : 'N/A';
    ...
    private function sendJSON($nome, $cpf, $valor, $vendaId, $email = "")
    {
        ...
        $dateOfExpiration = (new DateTime('+1 day'))->format('Y-m-d\TH:i:s.000P');
