...
class classPIX {
	...
	public function __construct() {
            ...
            if($retorno->access_token == PIX_ERRO) {
                echo ("<br><br>ERRO na Comunica��o com o Banco!<br>Por favor, entre em
                 contado com o suporte da E-Prepag e informe o erro de c�digo PIX790954 - Casa do cr�dito.<br>Obrigado.");
            }
            else {
	...
	public function callService($params) {
			...
			$params["url"] = "http://10.204.132.17/s2/oauth/jwtqr.php";	
           ...
           // $resposta = str_replace(["\t","\n","\r","\\"], [""], $resposta);
			...
			//felipe
//echo "N� do deposito: ".$params["venda_id"];
			if ($ff) {
				$timestamp = date("Y-m-d H:i:s");
            ...
            if(isset($resposta->codigo) && $resposta->codigo == PIX_ERRO) {
				$htmlErro ="
						<div class='col-md-12' style='border: 1px solid black; padding: 5px; margin-top:3px; text-align: center; clear: both;'>
                            <b>ERRO na Comunica��o com o Banco!<br>Por favor, entre em contato com o suporte da E-Prepag e informe o erro de c�digo PIX985235 ou tente novamente mais tarde.<br>Obrigado.</b>
                        </div>";
                return $htmlErro;
                ...
                // $html ="
						...
						// <span style='display: none'>felipe ".$GLOBALS["_SESSION"]["QRCODE"]."---</span>
                            // <button id='btn-copy' title='Copiar c�digo' data-clipboard-text='".$GLOBALS["_SESSION"]["QRCODE"]."' class='top20 btn btn-success'>Copiar c�digo</button>
                        // </div>
                        ...
                        // </div>
						// <div class='col-md-12' style='border: 1px solid black; padding: 5px; margin-top:3px; text-align: center; clear: both;'>
                            // <b>Aten��o o QRcode tem validade de 1 hora.</b>
                        // </div>
                        ...
                        // <script>
                            // $(document).ready(function(){
                                ...
                                // clipboard.on('success', function(e){
                                    // $('#btn-copy').attr('title', 'C�digo copiado');
	...
	public function callSonda($params, &$reposta_consulta) {
            ...
            $params["url"] = "http://10.204.132.17/s5/oauth/PIX/decodificaQRB";
...
// TESTE EXEMPLO PARA IMPLEMENTAR CONCILIA��O
/*            $resposta = '{
    "calendario": {
...
';*/
            ...
            if($resposta->codigo == PIX_ERRO) {
                echo ("<br><br>ERRO na Comunica��o com o Banco!<br>Por favor, entre em contado com o suporte da E-Prepag e informe o erro de c�digo PIX985235.<br>Obrigado.");
            }
            else {
                ...
                if($resposta->status == PIX_SONDA_PAGO_OK) {
                    ...
                    else $this->logEvents("J� existe registro de dados do pagador para o pagamento ".substr($params['idpedido'],2,17).PHP_EOL);
	...
	private function logEvents($msg) {
        ...
        private function sendJSONAuthentication() {
            ...
            //   CURLOPT_URL => "http://10.204.132.17/s5/oauth/seguranca/connect/token",
              ...
              CURLOPT_POSTFIELDS => 'client_id=aBcDeFgHiJ&client_secret=RFGT6547@#&grant_type=client_credentials&scope=cobranca', 
...
//fun��o SONDA para checagem da situa��o do PIX
	...
	if(!$rs_sonda) {
		...
		$valor	= $rs_sonda_row['total'];
                ...
                elseif($tipo == "M") {
                    ...
                    echo "<font color='#FF0000'><b>N�o consta Tipo de Pedido na Tabela de Pagamento (".$numero.").".PHP_EOL."</b></font><br>";
                    return false;   
                }
