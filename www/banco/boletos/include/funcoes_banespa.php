...
{ 
	...
	//Exemplo: 033 9 8 5881 0000001050 9 6377980 2000003008030 0 102
	...
	// 4. Quarto Grupo - composto pelo fator de vencimento (4 dígitos) e valor do título (10 dígitos)
	$campo4  = substr($codigo,4,14);
	// 5. Quinto Grupo - composto pelo Fixo ?9?
	$campo5  = substr($codigo,18,1);
	// 6. Sexto Grupo - composto pelo número do PSK(Código do Cliente)
	$campo6  = substr($codigo,19,7);
	// 7. Sétimo Grupo - composto por Nosso Número
	$campo7  = substr($codigo,26,13);
	// 8. Oitavo Grupo - composto pelo IOF ?Seguradoras - Demais clientes= zero
	$campo8  = substr($codigo,39,1);
	// 9. Nono Grupo - composto pelo 102- Cobrança simples ? SEM Registro
	$campo9  = substr($codigo,40,3);
...
*/
function monta_linha_digitavel_wagner($linha){
	// 1º. Campo: Banco, Código da moeda, Fixo ?9?, Código do Cedente padrão Santander(4 primeiros dígitos) e dígito verificador (módulo 10) deste campo;
        // 2º. Campo: Restante do código do cedente(3 ultimos digitos) padrão Santander, 7 primeiros campos do N/N e dígito verificador (módulo 10) deste campo;
        // 3º. Campo: Restante do Nosso Número(6 ultimos nuúmeros),IOF, Carteira e dígito verificador (módulo 10) deste campo;
        // 4º. Campo: Dígito Verificador do Código de Barras (dígito de autoconferência);
        // 5º. Campo: Composto pelo Fator de  Vencimento (anexo 7) e o Valor Nominal do documento, com a inclusão  de  zeros entre eles até compor as 14 posições do campo,  e sem edição de ponto e vírgula. 
        //            Quando se tratar de bloqueto sem discriminação de valor no código de barras a representação deverá ser com zeros. 
	//Exemplo: 033 9 8 5881 0000001050 9 6377980 2000003008030 0 102 - 03398588100000010509637798020000030080300102
	...
	$linha = substr($campo1,0,5).".".substr($campo1,5).$campo1Digito." ".substr($campo2,0,5).".".substr($campo2,5).$campo2Digito." ".substr($campo3,0,5).".".substr($campo3,5).$campo3Digito." ".$campo4." ".$campo5;
	return $linha;
}//end function monta_linha_digitavel_wagner($linha)
