...
if (!function_exists('writePdvLogSafe')) {
    function writePdvLogSafe($handle, $mensagem)
    {
...
if (!function_exists('closePdvLogSafe')) {
    function closePdvLogSafe($handle)
    {
        if (is_resource($handle)) {
            fclose($handle);
        }
...
if (!function_exists('pdvArrayValueSafe')) {
    function pdvArrayValueSafe($array, $key, $default = null)
    {
...
function verificaEstoque($venda_id)
{
    ...
    if ($ff) {
    writePdvLogSafe($ff, "Chegou no Verifica Estoque " . $venda_id . "\n");
    }
    ...
    writePdvLogSafe($ff, "mensagem: " . $msg . "\n");
    closePdvLogSafe($ff);
...
{
    $pin_ignorados = []; // Lista para armazenar PINs j� processados

    do {
        // Monta a cl�usula para ignorar os PINs j� verificados
        $ignorar_clausula = "";
        if (!empty($pin_ignorados)) {
            $ignorar_clausula = " AND pin_codigo NOT IN ('" . implode("','", $pin_ignorados) . "')";
        }

        // Consulta para buscar um PIN dispon�vel
        ...
        if ($pgpins["pin_codigo"] != '0000000000000000' && $pgpins["pin_codigo"] != '1111111111111111' && $pgpins["pin_codigo"] != null && $pgpins["pin_codigo"] != "") {
            ...
            // Verifica se o PIN j� foi vendido
            ...
            if ($pin_test_duplic && pgNumRowsSafe($pin_test_duplic) > 0 && $vgm_opr_codigo != 16) {
                if ($bDebug) {
                    echo "PIN j� vendido, procurando o pr�ximo..." . PHP_EOL;
                }

                // Adiciona o PIN na lista de ignorados
                $pin_ignorados[] = $pgpins["pin_codigo"];
            } else {
                // Retorna o PIN v�lido
                return [
                    'success' => true,
                    'pin' => $pgpins
                ];
...
function processaVendaGames($venda_id, $EstabCod, &$parametros)
{
            ...
            if (
                ...
                if (
                    ...
                    $parametros["agendamento_acao_obs"] =
                        "Venda j� foi processada." . PHP_EOL;
                } elseif (
                    $vg_ultimo_status ==
        ...
        while ($rs_venda_modelos_row = pgFetchArraySafe($rs_venda_modelos)) {
            ...
            for ($i = 0; $i < $vgm_qtde; $i++) {
                ...
                if ($vgm_ogp_id == 488) {
                    ...
                    if ($ff) {
                    writePdvLogSafe($ff, "Chegou no if" . $vgm_ogp_id . " venda: " . $venda_id . "\n");
                    ...
                    try {
                        ...
                        if ($ff) {
                        writePdvLogSafe($ff, "PIN GERADO NESSE CARAI: " . $pin_codinterno . "\n");
                        writePdvLogSafe($ff, "VGM_ID: " . $vgm_id . "\n");
                        closePdvLogSafe($ff);
                        }
                        ...
                        elseif (pgAffectedRowsSafe($ret) > 1)
                            $msg = "Erro ao atualizar mais de um pin na tabela de pins." . PHP_EOL;
                        elseif (pgAffectedRowsSafe($ret) == 0) {
                            $i--;
                            continue;
                        }
                    } catch (Throwable $err) {
                    ...
                    //sleep(1);
                    //FALA S�RIO GENTE: Eu N�O Tive que comentar isso!!!
                    //usleep(0.1*1000000);
                    ...
                    if (
                        ...
                        $ff = @fopen("/www/arquivos_gerados/logs/livrodjx.txt", "a+");
                        if ($ff) {
                        writePdvLogSafe($ff, "Entra aqui ??? " . $vgm_ogp_id);
                        closePdvLogSafe($ff);
                        }
                    ...
                    if ($bDebug) {
                        echo " AAA+++ - Captura pin v�lido (iter: $i) (" .
                            number_format(
                                getmicrotime() - $time_start_stats,
                                2,
                                ".",
                    ...
                    if ($vgm_pin_request > 0) {
                        ...
                        if ($vgm_pin_request == 1) {
                            ...
                            $rs_bhn_variavel_row = pgFetchArraySafe(
                                $rs_bhn_variavel
                            );
    ...
    if ($msg == "") {
        ...
        $ret = SQLexecuteQuery($sql);
        if (!$ret) {
            $msg = "Erro ao atualizar saldo do usu�rio." . PHP_EOL;
        } elseif (
            ...
            if (!$retSubStatus) {
                ...
                enviaEmail(
                    ...
                    (!checkIP() ? "[PROD]" : "[DEV-HOMOLOG]") .
                        " PDV - Promovido Autom�ticamente para PDV Aprovado",
                    "PDV de ID [" .
                        $vg_ug_id .
                        "] foi promovido autom�ticamente para SubStatus PDV Aprovado ap�s a concilia��o autom�tica de sua primeira compra. ID do Pedido [" .
                        $venda_id .
                        "]."
                );
        ...
        $msgEmailLista .= "
                                <table cellspacing='0' cellpadding='5' width='100%' style='font: normal 13px arial, sans-serif;'>
                                        <tr bgcolor='#CCCCCC'>
                                                ...
                                                <td align='right'><b>Comiss�o</b></td>
                                                <td width='5'>&nbsp;</td>
                                        </tr>";
...
// Est� usando este em processaAgendamentos.bat
function processaAgendamentos($lista = null)
{
    ...
    if ($msg == "") {
        ...
        if (!$rs_agendamentos || pgNumRowsSafe($rs_agendamentos) == 0) {
    ...
    if ($msg == "") {
        // � errado fazer um loop pelos registros da tabela tb_dist_agendamento_execucao e fazer update em tb_dist_agendamento_execucao
        //	dentro do loop -> termina em deadlock
        //		passar a usar pg_fetch_all()

        while ($rs_agendamentos_row = pgFetchArraySafe($rs_agendamentos)) {
                        ...
                        if ($msgConcilia == "") {
                            ...
                            if ($msgConcilia == "") {
                                $msgConciliaUsuario .=
                                    ...
                                    ", R$" .
                                    number_format(
                                        pdvArrayValueSafe($parametros, "total_venda", 0),
                                        2,
                                        ",",
                                        "."
                                    ) .
                                ...
                                $smonitorprocessamentos .=
                                    ...
                                    ")</a> " .
                                    pdvArrayValueSafe($parametros, "s_info_venda", "") .
                            ...
                            if ($msgConcilia == "") {
                                ...
                                //reagendamento
                                $agendamento_acao =
                                    pdvArrayValueSafe($parametros, "agendamento_acao", "");
                                $agendamento_acao_obs =
                                    pdvArrayValueSafe($parametros, "agendamento_acao_obs", "");
                                $agendamento_acao_repetir_em =
                                    pdvArrayValueSafe($parametros, "agendamento_acao_repetir_em", "");
                                if ($agendamento_acao) {
...
function conciliacaoAutomaticaPagtoOnlineExpressMoneyLH($id_venda = null)
{
    ...
    echo $cReturn .
        "========================================================================" .
        $cReturn;
    echo "Prepara concilia��o de pagamentos online PDV pr� (registros com idvenda>0 e n�o processados nos �ltimos " .
        $nminutes .
        " minutos, desde " .
    ...
    if (!$rs_transacoes || pgNumRowsSafe($rs_transacoes) == 0) {
        $msg = "Nenhuma transa��o encontrada (132 PDV)." . $cReturn;
    }

    $irows = 0;
    if ($rs_transacoes) {
        echo "NRegs: " . pgNumRowsSafe($rs_transacoes) . $cReturn;
        while ($rs_transacoes_row = pgFetchArraySafe($rs_transacoes)) {
            ...
            if ($rs_transacoes_row["idvenda"] > 0) {
                ...
                if ($rs_transacoes_row["status"] == 3) {
                    ...
                    // get return values
                    $s_sonda = pdvArrayValueSafe($areturn, "s_sonda", "");
                    $sBanco = pdvArrayValueSafe($areturn, "sBanco", "");
                    $dataconfirma = pdvArrayValueSafe($areturn, "dataconfirma", "");
                    if ($rs_transacoes_row["iforma"] == 9) {
                        echo "===> Data do BB :" . $dataconfirma . PHP_EOL;
                    }
                    $prefix_1 = pdvArrayValueSafe($areturn, "prefix_1", "");
                    $s_sync = pdvArrayValueSafe($areturn, "s_sync", "");
                    $vg_pagto_tipo = pdvArrayValueSafe($areturn, "vg_pagto_tipo", "");
                    ...
                    if ($s_sync == "NO SYNC") {
                        ...
                        // Marca registro como status=3, j� que se chegou aqui quer dizer que n�o passou por confirmaBanco.php
                        $sql =
                        ...
                        if ($msg == "") {
                            ...
                            $ret = SQLexecuteQuery($sql);
                            if (!$ret) {
                                $msg = "Erro ao comitar transa��o." . $cReturn;
                            }
                            ...
                            if (!$ret) {
                                $msg =
                                    "Erro ao dar rollback na transa��o." .
                                    $cReturn;
                            }
                        ...
                        echo "N�o Processado por sonda: forma:" .
                            $rs_transacoes_row["iforma"] .
                            ", numcompra: " .
                            $rs_transacoes_row["numcompra"] .
                            " - IDVenda: " .
                    ...
                    echo "N�o processado: status!=3 e Sonda=false." .
                        $cReturn .
                        "numcompra: " .
                        $rs_transacoes_row["numcompra"] .
                        " - IDVenda: " .
    ...
    if ($msg == "") {
        ...
        echo "Encontrados registros - pgNumRowsSafe(rs_venda): " .
            pgNumRowsSafe($rs_venda) .
            $cReturn;
    }
    ...
    if ($msg == "") {
        while ($rs_venda_row = pgFetchArraySafe($rs_venda)) {
            ...
            if ($rs_venda_dist && pgNumRowsSafe($rs_venda_dist) > 0) {
                while ($rs_venda_dist_row = pgFetchArraySafe($rs_venda_dist)) {
                    ...
                    if ($rs_bol && pgNumRowsSafe($rs_bol) > 0) {
                        $rs_bol_row = pgFetchArraySafe($rs_bol);
                        ...
                        // Marca registro como processado s� aqui, depois de conciliar a venda
...
function conciliacaoAutomaticaPagtoPIXemPDV($id_venda = null)
{
    ...
    echo $cReturn .
        "========================================================================" .
        $cReturn;
    echo "Prepara concilia��o de pagamentos PIX para PDV pr� (n�o processados nos �ltimos " .
        $minutes .
        " minutos, desde " .
            ...
            if ($rs_transacoes_row["idvenda"] > 0) {
                ...
                if ($s_sync == "NO SYNC") {
                    ...
                    if ($msg == "") {
                        ...
                        if (!$ret) {
                            $msg =
                                "Erro ao iniciar transa��o (PDV)." . $cReturn;
                        }
                    }
                    //Arames Monster
                    if (strlen(str_replace("'", "", $dataconfirma)) == 17) {
                    ...
                    if ($msg == "") {
                        ...
                        if (!$ret) {
                            $msg =
                                "Erro ao dar rollback na transa��o." . $cReturn;
                        }
...
/*
    // => o correto � conciliar a partir de tb_pag_compras com status=3
*/
function conciliaExpressMoneyLH_PagtoOnline($venda_id, $usuario_id, $parametros)
{
    ...
    //Valida usuario_id
    if (!$usuario_id) {
        $msg = "C�digo do usu�rio PDV n�o fornecido." . PHP_EOL;
    } elseif (!is_numeric($usuario_id)) {
        $msg = "C�digo do usu�rio PDV inv�lido." . PHP_EOL;
    }
    ...
    if ($msg == "") {
        ...
        if (!$rs_pagto || pgNumRowsSafe($rs_pagto) == 0) {
            ...
            echo "pag_data: '$pag_data', pag_valor: $pag_valor, pag_banco: $pag_banco, pag_iforma: '$pag_iforma'" . PHP_EOL;
        }
    }
    ...
    //Credita valor do boleto no usu�rio LH
    if ($msg == "") {
        $sql =
        ...
        $ret = SQLexecuteQuery($sql);
        if (!$ret) {
            $msg = "Erro ao creditar valor do boleto no usu�rio PDV." . PHP_EOL;
        }
    }
    ...
    if ($msg == "") {
        $sql = "COMMIT TRANSACTION ";
        $ret = SQLexecuteQuery($sql);
        //echo "Coment�rios em COMMIT TRANSACTION  ============================================<br>";
        if (!$ret) {
            $msg = "Erro ao comitar transa��o." . PHP_EOL;
        }
    } else {
        $sql = "ROLLBACK TRANSACTION ";
        //echo "Coment�rios em ROLLBACK TRANSACTION   ============================================<br>";
        $ret = SQLexecuteQuery($sql);
        if (!$ret) {
            $msg = "Erro ao dar rollback na transa��o." . PHP_EOL;
        }
...
function verificaSomenteDebito($venda_id)
{
    ...
    if ($msg == "") {
        //Verifica cada item de cada produro
        if ($rs_venda_row = pgFetchArraySafe($rs_venda)) {
...
function conciliacaoAutomaticaBoletoExpressMoneyLH($vg_id = null)
{
    ...
    if ($msg == "") {
        ...
        echo "Encontrados " . pgNumRowsSafe($rs_venda) . " registros" . PHP_EOL;
        $n = pgNumRowsSafe($rs_venda);
    }
    ...
    if ($msg == "") {
        while ($rs_venda_row = pgFetchArraySafe($rs_venda)) {
            ...
            if ($rs_venda_dist && pgNumRowsSafe($rs_venda_dist) > 0) {
                //echo "  Encontrados (2) ".pgNumRowsSafe($rs_venda_dist)." registros dist".PHP_EOL;
...
function processaExpressMoneyLH($venda_id, $usuario_id, $parametros)
{
    ...
    if (!empty($parametros["showProgress"])) {
        $blShowProgress = true;
    }
    ...
    if ($msg == "") {
        ...
        if (!$rs_venda || pgNumRowsSafe($rs_venda) == 0) {
            ...
            if (
                ...
                $msg = "Venda n�o esta no seu status inicial." . PHP_EOL;
            }
        }
    }
    if ($blDebugMT) {
    ...
    //Enquanto nao tem deposito e boleto
    $vg_pagto_banco = $rs_venda_row["vg_pagto_banco"] ?? null;
