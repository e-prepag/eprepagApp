...
function displayEstoque_POS($distributor_codigo, $pin_valor)
{
	...
	if ($rs_pins) {
		...
		while ($rs_pins_row = pg_fetch_array($rs_pins)) {
			if ($distributor_codigo_last != $rs_pins_row['distributor_codigo']) {
				if (!empty($distributor_codigo_last)) {
					...
					$msg .= "<td colspan='2' align='right'><b>" . number_format((float)$total_aux_parcial, 2, ',', '.') . "</b></td>\n";
					$msg .= "</tr>\n";
			...
			$msg .= "<td align='right'>" . number_format((float)$rs_pins_row['pin_valor'], 2, ',', '.') . "</td>";
		...
		$msg .= "<p><font color='#FF0000'>N�o foram encontrados pins para canal 'w'</font></p><br>\n";
	}
	return $msg;
}
