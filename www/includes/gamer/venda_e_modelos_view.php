        ...
        foreach($arr_venda_modelos as $ind => $rs_venda_modelos_row){
            ...
            if(isset($rs_venda_modelos_row['vgm_ogpm_id']) && !empty($rs_venda_modelos_row['vgm_ogpm_id']) || $rs_venda_modelos_row['vg_deposito_em_saldo'] != 0){
                ...
                if($rs_venda_modelos_row['vg_deposito_em_saldo'] == 0) {
                    ...
                    $iof = $instProduto->buscaIOF($rs_venda_modelos_row['vgm_ogpm_id']) ? "Incluso" : "";
...
if($rs_total_row['total'] == 0) {
        $sql_update_total = "update tb_pag_compras set total = ".(($total_geral+$rs_total_row['taxas'])*100)." where tipo_cliente = 'M' and idvenda = $1;";
        $rs_update_total = SQLexecuteQueryParams($sql_update_total, [$GLOBALS['_SESSION']['venda']]);
}
