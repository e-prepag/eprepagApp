...
function conciliaVendaGames_deposito($venda_id, $dep_id, $EstabCod, $parametros){
        ...
        //Valida venda id
        if(!$venda_id) $msg = "Código da venda não fornecido.".PHP_EOL;
        elseif(!is_numeric($venda_id)) $msg = "Código da venda inválido.".PHP_EOL;
        ...
        //Valida EstabCod
        if(!$EstabCod) $msg = "Código do estabelecimento não fornecido.".PHP_EOL;
        elseif(!is_numeric($EstabCod)) $msg = "Código do estabelecimento inválido.".PHP_EOL;
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO']) $msg = "Dados do Pagamento ainda não recebidos (A1) (vg_ultimo_status: '$vg_ultimo_status').".PHP_EOL;
                }
        }
        ...
        if($msg == ""){
                ...
                if(!$rs_dep || pg_num_rows($rs_dep) == 0) $msg = "Nenhum depósito encontrado.".PHP_EOL;
                else {
        ...
        if($msg == ""){
                ...
                $sql = "update depositos_pendentes set dep_aprovado = 1, dep_aprovado_data = CURRENT_DATE, dep_venda_games_id = " . $venda_id . "
                                where dep_codigo = " . $dep_id;
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao conciliar deposito.".PHP_EOL;
        }
...
function conciliaVendaGames_boleto($venda_id, $boleto_id, $EstabCod, $parametros){
        ...
        //Valida boleto id
        if(!$boleto_id) $msg = "Código do boleto não fornecido.".PHP_EOL;
        elseif(!is_numeric($boleto_id)) $msg = "Código do boleto inválido.".PHP_EOL;
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO']) $msg = "Dados do Pagamento ainda não recebidos (tem '".$vg_ultimo_status ."' e devia ser '". $GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO']."')(A2).".PHP_EOL;
                }
        }
        if($bDebug) {
        ...
        if($msg == ""){
                if($b_isVendaCampeonato) {
                        ...
                        if(!$concilia_cod_sel) $concilia_cod_sel = $GLOBALS['_REQUEST']['concilia_cod_sel'];
                        $sql  = "select * 
                        ...
                        if(!$rs_vendas || pg_num_rows($rs_vendas) == 0) {
                                ...
                                while($rs_vendas_row = pg_fetch_array($rs_vendas)) {
                                        $msg = "";
                                        if($bDebug) echo str_repeat("-", 80).PHP_EOL;
                                        ...
                                        if($vg_pagto_tipo==2) {
                                                ...
                                                if($boleto_id>0) {
                                                        ...
                                                        if($bDebug) echo "Valores Boleto: [(".$bol_valor."+".$GLOBALS['BOLETO_MONEY_ITAU_TAXA_CUSTO_BANCO_2'].") ? (".$bbg_valor_sem_taxa ."+". $GLOBALS['BOLETO_MONEY_ITAU_TAXA_ADICIONAL'].")]".PHP_EOL;

                                                        if(($bol_valor+$GLOBALS['BOLETO_MONEY_ITAU_TAXA_CUSTO_BANCO_2']) ==($bbg_valor_sem_taxa + $GLOBALS['BOLETO_MONEY_ITAU_TAXA_ADICIONAL'])) {
                                                                if($bDebug) echo "Vai conciliar Boleto (bol: $bol_codigo, vg: $venda_id, ug: $usuario_id)".PHP_EOL;
                                                                $parametros['valor'] = $bbg_valor_sem_taxa;

{	/////////	==================   BLOCKED
                                                                $ret = conciliaMoneyDepositoSaldo_boleto($bol_codigo, $venda_id, $usuario_id, $parametros);
                                                                if($ret!="") echo $ret;
                                                                else echo "Depósito por boleto conciliado com sucesso e saldo depositado".PHP_EOL;
}	/////////	==================   BLOCKED
                                                        } else {
                                                            if($bDebug) echo "NÃO concilia Boleto ($boleto_id, $venda_id, $usuario_id)".PHP_EOL;
                                                        }
                                                } else {
                                                    if($bDebug) echo "Nenhum Boleto encontrado ($venda_id, $usuario_id)".PHP_EOL;
                                                }
        ...
        if($msg == ""){
                ...
                if(!$rs_boleto || pg_num_rows($rs_boleto) == 0) $msg = "Nenhum boleto encontrado.".PHP_EOL;
                else {
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao atualizar boleto bancário.".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao conciliar boleto.".PHP_EOL;
        }
        ...
        if($parametros['PROCESS_AUTOM'] == '1') $iduser_bko = $GLOBALS['PROCESS_AUTOM_IDUSER_BKO'];
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao conciliar venda A1 (vg_id=$venda_id).".PHP_EOL;
        }
...
function conciliaVendaGames_PagamentoOnline($venda_id, $pagamento_id, $EstabCod, $parametros, $webhook = false){
        ...
        //Valida boleto id
        if(!$pagamento_id) $msg = "Código do pagamento não fornecido (1).".PHP_EOL;
        elseif(!is_numeric($pagamento_id)) $msg = "Código do pagamento inválido.".PHP_EOL;
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        if(pg_num_rows($rs_venda) == 0){
                              //valida status 
                              if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO'] && $webhook == false) $msg = "Dados do Pagamento ainda não recebidos(A3) (vg_ultimo_status: '$vg_ultimo_status', debia ser ".$GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO'].").".PHP_EOL;
                        }
        ...
        if($msg == ""){
                ...
                if(!$rs_pagamento || pg_num_rows($rs_pagamento) == 0) $msg = "Nenhum pagamento encontrado (B).".PHP_EOL;
                else {
                        $rs_pagamento_row = pg_fetch_array($rs_pagamento);
                        $pag_data = $rs_pagamento_row['dataconfirma'];
                                // Testa a existencia de data cadastrada
                                if(!trim($pag_data)) $pag_data = date("Y-m-d H:i:s");
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao conciliar venda (32)".PHP_EOL.$sql.PHP_EOL;
...
function conciliaVendaGames_Integracao($venda_id, $pagamento_id, $EstabCod, $parametros){
        ...
        //Valida boleto id
        if(!$pagamento_id) $msg = "Código do pagamento não fornecido (2).".PHP_EOL;
        elseif(!is_numeric($pagamento_id)) $msg = "Código do pagamento inválido.".PHP_EOL;
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO']) $msg = "Dados do Pagamento ainda não recebidos(A3a) (vg_ultimo_status: '$vg_ultimo_status', != '".$GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO']."').".PHP_EOL;
                }
        }
...
function conciliaVendaGames_redecard($venda_id, $redecard_id, $EstabCod, $parametros){
        ...
        //Valida redecard id
        if(!$redecard_id) $msg = "Código do redecard não fornecido.".PHP_EOL;
        elseif(!is_numeric($redecard_id)) $msg = "Código do redecard inválido.".PHP_EOL;
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['DADOS_PAGTO_RECEBIDO']) $msg = "Dados do Pagamento ainda não recebidos(A4).".PHP_EOL;
                }
        }
        ...
        if($msg == ""){
                ...
                if(!$rs_redecard || pg_num_rows($rs_redecard) == 0) $msg = "Nenhum redecard encontrado.".PHP_EOL;
                else {
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao aprovar redecard.".PHP_EOL;
        }
...
function verificaEstoque($venda_id){
        ...
        if($msg == ""){
                $sql  = "select * from tb_venda_games vg 
                                ...
                                where vg.vg_id = " . $venda_id;
                $rs_venda_modelos = SQLexecuteQuery($sql);
                if(!$rs_venda_modelos || pg_num_rows($rs_venda_modelos) == 0) $msg = "Nenhum produto encontrado (VE_1).".PHP_EOL;
        }
                ...
                while($rs_venda_modelos_row = pg_fetch_array($rs_venda_modelos)){
                        ...
                        if($vgm_pin_request == 0) {
                                ...
                                if($produto_operadora != 560){
									...
									if(!$rs_pins || pg_num_rows($rs_pins) == 0) $msg .= "Não há pin de " . number_format($vgm_pin_valor, 2, ',', '.') . " da operadora " . $opr_nome . " em estoque (A).".PHP_EOL;
									else {
											$rs_pins_row = pg_fetch_array($rs_pins);
											$pins_qtde = $rs_pins_row['pins_qtde'];
											if($pins_qtde < $vgm_qtde) $msg .= "Não há suficientes pins de " . number_format($vgm_pin_valor, 2, ',', '.') . " da operadora " . $opr_nome . " em estoque (B).".PHP_EOL;
									}
                                }
...
function processaVendaGamesIntegracao($venda_id, $EstabCod, $parametros){
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['PAGTO_CONFIRMADO']) $msg = "Pagamento ainda não esta confirmado (B, status: '$vg_ultimo_status', deve ser '".$GLOBALS['STATUS_VENDA']['PAGTO_CONFIRMADO']."').".PHP_EOL;
                }
        }
        ...
        if($msg == ""){
                $sql  = "select * from tb_venda_games vg " .
                                ...
                                "where vg.vg_id = " . $venda_id;
                $rs_venda_modelos = SQLexecuteQuery($sql);
                if(!$rs_venda_modelos || pg_num_rows($rs_venda_modelos) == 0) $msg = "Nenhuma produto encontrado (I).".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                if($vg_pagto_tipo == $GLOBALS['FORMAS_PAGAMENTO']['DEP_DOC_TRANSF']){
                        ...
                        if(!$rs_deposito || pg_num_rows($rs_deposito) == 0) $msg = "Nenhum deposito encontrado (I).".PHP_EOL;
                        else {
                ...
                //Boleto
                        ...
                        $rs_boleto = SQLexecuteQuery($sql);
                        if(!$rs_boleto || pg_num_rows($rs_boleto) == 0) $msg = "Nenhum boleto encontrado (I).".PHP_EOL;
                        else {
                                $rs_boleto_row = pg_fetch_array($rs_boleto);
                                $bol_documento = $rs_boleto_row['bol_documento'];
                ...
                // Pagamentos Online Bradesco: Débito/Transferência - Banco do Brasil
                        ...
                        */
                        ...
                        $rs_pagamento = SQLexecuteQuery($sql);
                        if(!$rs_pagamento || pg_num_rows($rs_pagamento) == 0) $msg = "Nenhum pagamento encontrado (A2).".PHP_EOL;
                        else {
                                $rs_pagamento_row = pg_fetch_array($rs_pagamento);
        ...
        if($msg == ""){
                if (	($vg_pagto_tipo == $GLOBALS['FORMAS_PAGAMENTO']['BOLETO_BANCARIO'])  
                        ...
                        $ret = SQLexecuteQuery($sql);
                        if(!$ret) $msg_not_relevant = "Erro ao conciliar venda (status_processed not set to '1').".PHP_EOL;
                }
        }
...
function processaVendaGames_pagto_online_banco_epp($venda_id, $EstabCod, $parametros){
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['PAGTO_CONFIRMADO']) $msg = "Pagamento ainda não esta confirmado (C).".PHP_EOL;
                }
        }
        ...
        if($msg == ""){
                $sql  = "select * from tb_venda_games vg " .
                                ...
                                "where vg.vg_id = " . $venda_id;
                $rs_venda_modelos = SQLexecuteQuery($sql);
                if(!$rs_venda_modelos || pg_num_rows($rs_venda_modelos) == 0) $msg = "Nenhum produto encontrado (PVG_EPP_1).".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                if ( $vg_pagto_tipo == $GLOBALS['PAGAMENTO_BANCO_EPP_ONLINE_NUMERIC'] ) {	
                        ...
                        $rs_pagamento = SQLexecuteQuery($sql);
                        if(!$rs_pagamento || pg_num_rows($rs_pagamento) == 0) $msg = "Nenhum pagamento encontrado (A3).".PHP_EOL;
                        else {
                                $rs_pagamento_row = pg_fetch_array($rs_pagamento);
...
function processaVendaGames($venda_id, $EstabCod, $parametros){
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['PAGTO_CONFIRMADO']) $msg = "Pagamento ainda não esta confirmado (A).".PHP_EOL;
                }
        }
        ...
        if($msg == ""){
                $sql  = "select * from tb_venda_games vg " .
                                ...
                                "where vg.vg_id = " . $venda_id;
                $rs_venda_modelos = SQLexecuteQuery($sql);
                if(!$rs_venda_modelos || pg_num_rows($rs_venda_modelos) == 0) $msg = "Nenhum produto encontrado (PVG_1).".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                if($vg_pagto_tipo == $GLOBALS['FORMAS_PAGAMENTO']['DEP_DOC_TRANSF']){
                        ...
                        $sql  = "select * from depositos_pendentes dep " .
                                        "where dep.dep_codigo = " . $vg_dep_codigo;
                        $rs_deposito = SQLexecuteQuery($sql);
                        if(!$rs_deposito || pg_num_rows($rs_deposito) == 0) $msg = "Nenhum deposito encontrado.".PHP_EOL;
                        else {
                                $rs_deposito_row = pg_fetch_array($rs_deposito);
                                $dep_documento = $rs_deposito_row['dep_documento'];
                                $ped_cod_doc_equiv = $dep_documento;
                ...
                // Pagamentos Online Bradesco: Débito/Transferência - Banco do Brasil
                        ...
                        $sql  = "select * from tb_pag_compras pag where pag.numcompra = '" . substr($vg_pagto_num_docto,5)."'";	
                        $rs_pagamento = SQLexecuteQuery($sql);
                        if(!$rs_pagamento || pg_num_rows($rs_pagamento) == 0) $msg = "Nenhum pagamento encontrado (A1).".PHP_EOL;
                        else {
                                $rs_pagamento_row = pg_fetch_array($rs_pagamento);
                                        ...
                                        for($i=0; $i < $vgm_qtde; $i++){
												...
												if($vgm_opr_codigo == 49 && $produto_operadora == 560){	
													...
													$sql = "update tb_venda_games_modelo set 
																	...
																	where vgm_id = '" . $vgm_id . "'";
													$ret = SQLexecuteQuery($sql);
													if(!$ret) $msg = "Erro ao atualizar pin no modelo vendido.".PHP_EOL;
													
													
													$sql = "insert into tb_venda_games_modelo_pins (vgmp_vgm_id, vgmp_pin_codinterno) values (" . $vgm_id . "," . $pin_gerado . ")";
													$ret = SQLexecuteQuery($sql);
													if(!$ret) $msg = "Erro ao associar pin no modelo vendido.".PHP_EOL;
												}else{
													...
													if($msg == ""){
															...
															$rs_pins = SQLexecuteQuery($sql);
															if(!$rs_pins || pg_num_rows($rs_pins) == 0) $msg = "Nenhum pin encontrado.".PHP_EOL;
															else {
													...
													if($msg == ""){
															...
															$ret = SQLexecuteQuery($sql);
															if(!$ret) $msg = "Erro ao atualizar tabela de pins (3212).".PHP_EOL;
													}
...
function processaEmailVendaGames($venda_id, $parametros){ 
    ...
    if($msg == ""){
            ...
            $rs_venda_modelos = SQLexecuteQuery($sql);
            if(!$rs_venda_modelos || pg_num_rows($rs_venda_modelos) == 0) $msg = "Nenhum produto encontrado (PEVG_1).".PHP_EOL;
    }
                            ...
                            if($vgm_pin_request > 0) {
                                ...
                                if($vgm_pin_request == 1) {
									    ...
									    if($vgm_opr_codigo == 159){
											...
											if(is_null($rs_bhn_variavel_row['ogp_valor_minimo']) && is_null($rs_bhn_variavel_row['ogp_valor_maximo'])) $sql .=" and ogpm_valor = ".$vgm_valor_pin.";";
                            ...
                            for($i=0; $i < count($vgm_pin_codinternoAr); $i++){
                                    ...
                                    if($msg == ""){
                                            ...
                                            if(!$rs_pin || pg_num_rows($rs_pin) == 0) {
                                                    ...
                                                    $labSenha = "PIN";
                                                    if($opr_codigo == 16) $labSenha = "Habbo Crédito";	// Habbo
                                                    if($opr_codigo == 38) $labSenha = "Senha (Código de Presente)";	// Stardoll
                                                    if($opr_codigo == 37) $labSenha = "Cód de segurança";	// Softnyx
                                                    if($opr_codigo == 31) $labSenha = "";	// GPotato
                                                    if($opr_codigo == 31) $labSenhaExtra = " (Nro Série + Senha)";	// GPotato
                                                    if($opr_codigo == 33) $labSenha = "PIN Password";	// NDoors
                                                    if($opr_codigo == 44) $labSenha = "Serial";	// Axeso5

                                                    // Formatação de título do No de Serie
                                                    $labSerie = "No Série";
                                                    if($opr_codigo == 37) $labSerie = "No do cartão";	// Softnyx
                                                    if($opr_codigo == 44) $labSerie = "Pin";	// Axeso5
                                                    if($opr_codigo == 34) $labSerie = "Código";	// Webzen
                                                    if($opr_codigo == 74 || $opr_codigo == 75 ||$opr_codigo == 76 || $opr_codigo == 77) $labSerie = "Código"; //Eletronic Arts

                                                    // Formatação do Serial
                                                    if($opr_codigo == 13) $case_serial = wordwrap($case_serial, 4, " ", true);	// Ongame
                                                    if($opr_codigo == 31) $labSerie = "Pin Eletrônico";	// GPotato
                                                    if($opr_codigo == 33) $labSerie = "PIN Code";	// NDoors
                                                    if($opr_codigo == 125) $labSenha = "Habbo Crédito";	// Habbo 2
                                            }
                                    }
            ...
            if($b_isVendaCampeonato) {
                    // Vendas de tipo Campeonato usam um template específico 
                            ...
                            while ($rs_venda_modelos_row = pg_fetch_array($rs_venda_modelos)){
									...
									if($rs_venda_modelos_row['vgm_opr_codigo']=="49") $opr_codigo_contains_EPP_Cash = true;	// marca a venda como contendo EPP Cash
									$vgm_nome_produto = $rs_venda_modelos_row['vgm_nome_produto'];
									$vgm_ogp_id = $rs_venda_modelos_row['vgm_ogp_id'];
                            ...
                            if(substr($prom_opr_codigos, 0, 1)==",") $prom_opr_codigos = substr($prom_opr_codigos, 1);
    ...
    if($msg == ""){
                ...
                $sql .= " vg_ultimo_status = " . SQLaddFields($GLOBALS['STATUS_VENDA']['VENDA_REALIZADA'], "") . "
                                where vg_id = " . $venda_id;
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao atualizar venda (ultimo_status).".PHP_EOL;
                if($msg == ""){
                        if(isset($parametros['PROCESS_TEST']) && ($parametros['PROCESS_TEST']!=1)) {
                                ...
                                $sql = "update tb_venda_games set 
                                                ...
                                                where vg_id = " . $venda_id;
                                $ret = SQLexecuteQuery($sql);
                                if(!$ret) $msg = "Erro ao atualizar venda (data_concilia).".PHP_EOL;
                        }
                }
...
function cancelaVendaGames($venda_id, $parametros){
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao cancelar venda.".PHP_EOL;
                else {
                        ...
                        $ret = SQLexecuteQuery($sql);
                        if(!$ret) $msg = "Erro ao cancelar pagamentos de venda (venda foi cancelada).".PHP_EOL;
...
function descancelaVendaGames($venda_id, $parametros){
        ...
        if($msg == ""){
                ...
                if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada.".PHP_EOL;
                        ...
                        //valida status
                        if($vg_ultimo_status != $GLOBALS['STATUS_VENDA']['VENDA_CANCELADA']) $msg = "Venda não esta cancelada (status: '$vg_ultimo_status').".PHP_EOL;
                }
        }
        ...
        if($msg == ""){
                ...
                if(!$rs_venda_historico || pg_num_rows($rs_venda_historico) == 0) $msg = "Nenhum histórico da venda encontrado.".PHP_EOL;
                else {
        ...
        if($msg == ""){
                $sql = "update tb_venda_games set 
                                ...
                                where vg_id = " . $venda_id;
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao descancelar venda.".PHP_EOL;
        }
...
function cancelaVendasBoletoVencido(){
        ...
        if(!$rs_venda_bol_venc || pg_num_rows($rs_venda_bol_venc) == 0) $msg = "Nenhuma venda encontrada com boleto vencido ha mais de " . $GLOBALS['PROCESS_AUTOM_BOLETO_CANCELAMENTO_DIAS_VENCIDO'] . " dias.".PHP_EOL;
        else {
...
function cancelaVendasEmPedidoEfetuado(){
        ...
        $ret = SQLexecuteQuery($sql);
        if(!$ret) $msg .= "Erro ao cancelar pagamentos sem venda e atrasados ($sql).".PHP_EOL;
...
function conciliacaoAutomaticaBoleto(){
        ...
        if($bDebug) echo $sql.$cReturn;
        if($bDebug) echo "Elapsed time (Procura boleto ".$n_boletos."): ".number_format(getmicrotime() - $time_start_stats, 2, '.', '.').$cReturn;

        $rs_bol = SQLexecuteQuery($sql);
        if($bDebug) echo "pg_num_rows(rs_bol): ".pg_num_rows($rs_bol).$cReturn.$cReturn;
        ...
        if($rs_bol && (pg_num_rows($rs_bol) > 0)){
                ...
                while($rs_bol_row = pg_fetch_array($rs_bol)){
                        ...
                        if($msg == ""){
                                ...
                                if(strlen($bol_documento)==12) { $istart = 5; } elseif(strlen($bol_documento)==11) { $istart = 4; } elseif(strlen($bol_documento)==8) { $istart = 1; } 
                        ...
                        if($bDebug) echo "  >> pg_num_rows(rs_venda): ".pg_num_rows($rs_venda).$cReturn.PHP_EOL;
                        ...
                        if($msg == ""){
                                ...
                                if($bDebug) echo "SQL RRR: ".$sql.$cReturn;
                                ...
                                if($rs_venda_modelos && pg_num_rows($rs_venda_modelos) > 0){
                                        while ($rs_venda_modelos_row = pg_fetch_array($rs_venda_modelos)){
                                                ...
                                                if($vg_integracao_parceiro_origem_id) {
                                                        ...
                                                        if($bDebug) echo "  >> - qtde: $qtde, valor: $valor".$cReturn;
                                        }
                        ...
                        if($msg == "") {
                                ...
                                if($vg_pagto_banco==$GLOBALS['BOLETO_MONEY_BRADESCO_COD_BANCO']) {
                                        ...
                                        if($bDebug) echo "   === Boleto Bradesco -> vai processar".$cReturn;
                                        ...
                                        if($bDebug) echo "   === Boleto Itaú -> vai processar (vg_id: $vg_id)".$cReturn;
                                        ...
                                        if($bDebug) echo "   === Boleto Santander -> vai processar".$cReturn;
                                        switch(substr($vg_pagto_num_docto,0,1)) {
                                                case '2': 
                                                        if(round($bol_valor,2) == round(($total_geral + $taxas),2)) {
                                                                // OK
                                ...
                                if(trim($vg_integracao_parceiro_origem_id)=='') {
                                        ...
                                        if($msgConcilia == "") {
                                                ...
                                                if($msgConcilia == "") $msg .= "Processamento Integração (Boleto): Processado com sucesso.".PHP_EOL;
                                                else $msg .= "Processamento (6): " . $msgConcilia;
                                        } else {
                                                echo "ERRO Concilia (A3): $msgConcilia".$cReturn;
                                        }

                                        if($bDebug) echo " = Em Processamento para integração - Apenas completa a venda".$cReturn;
                                        ...
                                        if($msgConcilia == "") {
                                                ...
                                                $ret = SQLexecuteQuery($sql);
                                                if(!$ret) $msg = "Erro ao atualizar venda (RESDEW).".PHP_EOL;
                                        } else {
...
//										$sret = substr($sret1,strpos($sret1,"Content-type: text/html")+strlen("Content-type: text/html"));
                        ...
                        if($msg == ""){
                                ...
                                if(trim($vg_ultimo_status_obs) != "") $parametros['ultimo_status_obs'] = $vg_ultimo_status_obs . $cReturn . $parametros['ultimo_status_obs'];
                                ...
                                if($bDebug) echo "Elapsed time (conciliaVendaGames_boleto): ".number_format(getmicrotime() - $time_start_stats, 2, '.', '.').$cReturn;
                                ...
                                if(trim($vg_integracao_parceiro_origem_id)=='') {
                                        ...
                                        if($msgConcilia == "") {
                                                if($bDebug) echo "processaVendaGames($vg_id, 1, parametros) " . date('Y-m-d H:i:s') . "  ============================= 2 ".$cReturn;

                                                $msgConcilia = processaVendaGames($vg_id, 1, $parametros);
                                                if($msgConcilia == "") $msg .= "Processamento (7): Processado com sucesso.".$cReturn;
                                                else {
                                                        $msg .= "Processamento (1): " . $msgConcilia;
                                                }
                                        }
                                        if($bDebug) echo "Elapsed time (processaVendaGames(".$vg_id.")) : ".number_format(getmicrotime() - $time_start_stats, 2, '.', '.').$cReturn;

                                        //envia email para o cliente
                                        if($msgConcilia == "") {
                                                if($bDebug) echo "processaEmailVendaGames($vg_id, parametros) " . date('Y-m-d H:i:s') . "  ============================= 3	".$cReturn;
                                                $msgConcilia = processaEmailVendaGames($vg_id, $parametros);
                                                if($msgConcilia == "") $msg .= "Envio de email: Enviado com sucesso.".$cReturn;
                                                else {
                                ...
                                if($bDebug) echo "Elapsed time (processaEmailVendaGames(".$vg_id.")) : ".number_format(getmicrotime() - $time_start_stats, 2, '.', '.').$cReturn;
                        } else {
                ...
                if($bDebug) echo "Sem boletos ".number_format(getmicrotime() - $time_start_stats, 2, '.', '.')." (boleto ".$n_boletos.")".$cReturn."===============================".$cReturn;
        }


        if($bDebug || true) echo $cReturn.$cReturn.$cReturn."ELAPSED TOTAL TIME: ".number_format(getmicrotime() - $time_start_stats, 2, '.', '.').$cReturn."ELAPSED TIME PER CYCLE: ".number_format(((getmicrotime() - $time_start_stats)/(($n_boletos>0)?$n_boletos:1)), 2, '.', '.').$cReturn;
...
//		$sql .= " and ((pgt.datainicio between '".$date_ini."' and '".$date_end."') or (pgt.datainicio between '2010-01-26 00:00:00' and '2010-01-26 23:59:59'))";	
        ...
        if(!$rs_transacoes || pg_num_rows($rs_transacoes) == 0) $msg = "Nenhuma transação encontrada.".$cReturn;
                        ...
                        if($rs_pag && pg_num_rows($rs_pag) > 0) {
                                ...
                                if(trim($vg_integracao_parceiro_origem_id)=='') {
                                        ...
                                        if($msgConcilia == "") {
                                                ...
                                                if($msgConcilia == "") $msg .= "Conciliacao(C1): Conciliado com sucesso.".PHP_EOL;
                                                else $msg .= "Conciliacao: " . $msgConcilia;
                                        }
                                        if($bDebug) {
                                        ...
                                        if($msgConcilia == "") {
                                                $msgConcilia = processaVendaGames($vg_id, 1, $parametros);
                                                if($msgConcilia == "") $msg .= "Processamento (Pagtos Online): Processado com sucesso.".PHP_EOL;
                                                else $msg .= "Processamento (4): " . $msgConcilia;
                                        }
                                        if($bDebug) {
                                        ...
                                        if($msgConcilia == "") {
                                                $msgConcilia = processaEmailVendaGames($vg_id, $parametros);
                                                if($msgConcilia == "") $msg .= "Envio de email: Enviado com sucesso.".PHP_EOL;
                                                else $msg .= "Envio de email: " . $msgConcilia;
                                        }
                                    ...
                                    if($msgConcilia == "") {
                                            ...
                                            if($msgConcilia == "") $msg .= "Conciliacao(C_I): Conciliado com sucesso.".PHP_EOL;
                                            else $msg .= "Conciliacao: " . $msgConcilia;
                                    }
                                    ...
                                    if($msgConcilia == "") {
                                            ...
                                            $msgConcilia = processaVendaGamesIntegracao($vg_id, 1, $parametros);
                                            if($msgConcilia == "") $msg .= "Processamento Integração (Pagtos Online): Processado com sucesso.".PHP_EOL;
                                            else $msg .= "Processamento (5): " . $msgConcilia;
                                    }
        ...
        if($rs_venda_pendentes && pg_num_rows($rs_venda_pendentes) > 0){
                while ($rs_venda_pendentes_row = pg_fetch_array($rs_venda_pendentes)){
                        ...
                        if($vg_ultimo_status==$GLOBALS['STATUS_VENDA']['PAGTO_CONFIRMADO']) {	// 3 and "not vgm_pin_codinterno=''"	(from query) 
                                ...
                                $ret = SQLexecuteQuery($sql);
                                if(!$ret) $msg = "Erro ao atualizar venda com status pendente (pagamento online)".PHP_EOL."$sql.".PHP_EOL;
                                else {
...
function isVendaCampeonato($venda_id){
        ...
        $sql  = "select * from tb_venda_games vg 
                        ...
                        where vg.vg_id = " . $venda_id;
        $rs_venda_modelos = SQLexecuteQuery($sql);
        if(!$rs_venda_modelos || pg_num_rows($rs_venda_modelos) == 0) $msg = "Nenhum produto encontrado.".PHP_EOL;
...
function isVendaDeposito($venda_id){
        ...
        $rs_venda = SQLexecuteQuery($sql);
        if(!$rs_venda || pg_num_rows($rs_venda) == 0) $msg = "Nenhuma venda encontrada (em isvendaDeposito($venda_id)).".PHP_EOL;
                        ...
                        if($rs_pag && pg_num_rows($rs_pag) > 0) {
                                ...
                                if(trim($vg_integracao_parceiro_origem_id)=='') {
                                        ...
                                        if($msgConcilia == "") {
                                                ...
                                                $msgConcilia = processaVendaGames($vg_id, 1, $parametros);
                                                if($msgConcilia == "") $msg .= "Processamento (Pagtos PIX): Processado com sucesso.".PHP_EOL;
                                                else $msg .= "Processamento (4): " . $msgConcilia;
                                        }
                                        if($bDebug) {
                                            echo $cReturn.$cReturn.$cReturn."ELAPSED TOTAL TIME (C1): ".number_format(getmicrotime() - $time_start_stats, 2, '.', '.').$cReturn;
                                        ...
                                        if($msgConcilia == "") {
                                                ...
                                                $msgConcilia = processaVendaGamesIntegracao($vg_id, 1, $parametros);
                                                if($msgConcilia == "") $msg .= "Processamento Integração (Pagtos PIX): Processado com sucesso.".PHP_EOL;
                                                else $msg .= "Processamento (5): " . $msgConcilia;
                                        }
        ...
        if($rs_venda_pendentes && pg_num_rows($rs_venda_pendentes) > 0){
                while ($rs_venda_pendentes_row = pg_fetch_array($rs_venda_pendentes)){
                        ...
                        if($vg_ultimo_status==$GLOBALS['STATUS_VENDA']['PAGTO_CONFIRMADO']) {	// 3 and "not vgm_pin_codinterno=''"	(from query) 
                                ...
                                $ret = SQLexecuteQuery($sql);
                                if(!$ret) $msg = "Erro ao atualizar venda com status pendente (pagamento PIX)".PHP_EOL."$sql.".PHP_EOL;
                                else {
                                        echo "Venda status pendente vg_id:$vg_id_pendente, status ajustado de PAGTO_CONFIRMADO -> VENDA_REALIZADA (" . date('d/m/Y - H:i:s') . ")".PHP_EOL;
                                }
                        } else {
...
function conciliaMoneyDepositoSaldo_boleto($bol_codigo, $venda_id, $usuario_id, $parametros){
        ...
        //Valida boleto id
        if(!$bol_codigo) $msg = "Código do boleto não fornecido.".PHP_EOL;
        elseif(!is_numeric($bol_codigo)) $msg = "Código do boleto inválido.".PHP_EOL;

        //Valida usuario_id
        if(!$usuario_id) $msg = "Código do usuário Money não fornecido.".PHP_EOL;
        elseif(!is_numeric($usuario_id)) $msg = "Código do usuário Money inválido.".PHP_EOL;

        //Recupera o boleto pendente
        if($msg == ""){
                $sql  = "select * from boletos_pendentes bol
                                where bol.bol_codigo = " . $bol_codigo;
                if($bDebug) echo "sqlA1: $sql".PHP_EOL;
                $rs_boleto = SQLexecuteQuery($sql);
                if(!$rs_boleto || pg_num_rows($rs_boleto) == 0) $msg = "Nenhum boleto encontrado.".PHP_EOL;
                else {
                        $rs_boleto_row = pg_fetch_array($rs_boleto);
                        ...
                        $bol_banco = $rs_boleto_row['bol_banco'];
                        $bol_documento = $rs_boleto_row['bol_documento'];
                        if($bDebug) echo "bol_data: '$bol_data', bol_valor: $bol_valor, bol_banco: $bol_banco, bol_documento: '$bol_documento', ".PHP_EOL;
                }
        }		
        ...
        if($msg == ""){
                ...
                if($bDebug) echo "sqlA1a: $sql".PHP_EOL;
                $rs_saldo = SQLexecuteQuery($sql);
                if(!$rs_saldo || pg_num_rows($rs_saldo) == 0) $msg = "Nenhum usuário encontrado.".PHP_EOL;
                else {
                        $rs_saldo_row = pg_fetch_array($rs_saldo);
                        $ug_perfil_saldo_prev = $rs_saldo_row['ug_perfil_saldo'];
                        if($bDebug) echo " ++++B [".date("Y-m-d H:i:s")."] para ug_id:$ug_id ug_perfil_saldo_prev = '$ug_perfil_saldo_prev', valor = ".$parametros['valor'].PHP_EOL;
                }
        }
        ...
        if($msg == ""){
                ...
                if($bDebug) echo "sqlA2: $sql".PHP_EOL;
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao conciliar boleto.".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                if($bDebug) echo "sqlA3: $sql".PHP_EOL;
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao creditar valor do boleto no usuário Money.".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                if($bDebug) echo "sqlA4: $sql".PHP_EOL;
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao completar venda Money (Boleto).".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                if($bDebug) echo "sqlA5: $sql".PHP_EOL;
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao inserir Saldo FIFO (BOleto).".PHP_EOL;
        }
        ...
        if($msg == ""){
                $sql  = "select ug_perfil_saldo from usuarios_games where ug_id =".$usuario_id ;
                if($bDebug) echo "sqlA1b: $sql".PHP_EOL;
                $rs_saldo = SQLexecuteQuery($sql);
                if(!$rs_saldo || pg_num_rows($rs_saldo) == 0) $msg = "Nenhum usuário encontrado.".PHP_EOL;
                else {
                        $rs_saldo_row = pg_fetch_array($rs_saldo);
                        $ug_perfil_saldo_prev = $rs_saldo_row['ug_perfil_saldo'];
                        if($bDebug) echo " ++++B2 [".date("Y-m-d H:i:s")."] para ug_id:$ug_id ug_perfil_saldo_prev = '$ug_perfil_saldo_prev', valor = ".$parametros['valor'].PHP_EOL;
                }
        }
                ...
                while($rs_vendas_row = pg_fetch_array($rs_vendas)) {
                        ...
                        if($vg_pagto_tipo==2) {
                                ...
                                if($boleto_id>0) {
                                        ...
                                        if($bDebug) echo "Banco: [".$codigo_banco."]".PHP_EOL." Valores Boleto: [(".$bol_valor."+".$GLOBALS['BOLETO_MONEY_ITAU_TAXA_CUSTO_BANCO_2'].") ? (".$bbg_valor_sem_taxa ."+". $GLOBALS['BOLETO_MONEY_ITAU_TAXA_ADICIONAL'].")]".PHP_EOL;
                                        if(($bol_valor+$GLOBALS['BOLETO_MONEY_ITAU_TAXA_CUSTO_BANCO_2']) ==($bbg_valor_sem_taxa + $GLOBALS['BOLETO_MONEY_ITAU_TAXA_ADICIONAL'])) {
                                                if($bDebug) echo "Vai conciliar Boleto (bol: $bol_codigo, vg: $venda_id, ug: $usuario_id)".PHP_EOL;
                                                $parametros['valor'] = $bbg_valor_sem_taxa;
                                                ...
                                                if($bDebug) echo "Vai conciliar Boleto Santander OU Bradesco(bol: $bol_codigo, vg: $venda_id, ug: $usuario_id)".PHP_EOL;
                        ...
                        elseif($vg_pagto_tipo>4) {
                                ...
                                if(!$rs_pagto || pg_num_rows($rs_pagto) == 0) $msg = "Nenhum pagamento online encontrado para conciliação (Saldo Gamer, vg_pagto_tipo: $vg_pagto_tipo).".PHP_EOL;
                                else {
                                        if($bDebug) echo "Encontrados ".pg_num_rows($rs_pagto)." pagtos para ".$rs_vendas_row['vg_id'].PHP_EOL;
                                        ...
                                        $parametros = array();
                                        if($bDebug) echo "Valores PagtoOnline: [".$total_pagto_sem_taxas." ? ".$vg_deposito_em_saldo_valor."]".PHP_EOL;
                                        if(number_format($total_pagto_sem_taxas,2,'.','') == number_format($vg_deposito_em_saldo_valor,2,'.','')) {
                                                if($bDebug) echo "Vai conciliar PagtoOnline ($venda_id, $usuario_id)".PHP_EOL;
                                                $parametros['valor'] = $total_pagto_sem_taxas;

                                                $ret = conciliaMoneyDepositoSaldo_PagtoOnline($venda_id, $usuario_id, $parametros);
                                                if($ret!="") echo $ret;
                                                else echo "Depósito por pagamento online conciliado com sucesso e saldo depositado".PHP_EOL;
                                        } else {
                                                if($bDebug) echo "NÃO concilia PagtoOnline ($venda_id, $usuario_id)".PHP_EOL;
                                        }
                                }
                        } 
                        else {
...
function conciliaAutomaticaMoneyDepositoSaldocomPIX($webhook = false, $venda = 0) {
        ...
        if(!$rs_transacoes || pg_num_rows($rs_transacoes) == 0) $msg = "Nenhuma transacao encontrada.".$cReturn;
                        ...
                        if($rs_transacoes_row['idvenda']>0) {
                                ...
                                if($s_sync=="NO SYNC" || $webhook == true) {			/////   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                                        ...
                                        if($msg == ""){
                                                $sql = "COMMIT TRANSACTION ";
                                                $ret = SQLexecuteQuery($sql);
                                                if(!$ret) $msg = "Erro ao comitar transacao.".PHP_EOL;
                                                ...
                                                $sql = "ROLLBACK TRANSACTION ";
                                                $ret = SQLexecuteQuery($sql);
                                                if(!$ret) $msg = "Erro ao dar rollback na transacao.".PHP_EOL;
        ...
        if(!$rs_vendas || pg_num_rows($rs_vendas) == 0) {
                ...
                while($rs_vendas_row = pg_fetch_array($rs_vendas)) {
                        ...
                        echo "sqlB2: $sql".PHP_EOL;
                        $rs_pagto = SQLexecuteQuery($sql);
                        if(!$rs_pagto || pg_num_rows($rs_pagto) == 0) $msg = "Nenhum pagamento online encontrado para conciliacao (Saldo Gamer, vg_pagto_tipo: $vg_pagto_tipo).".PHP_EOL;
                        else {
                                if($bDebug) echo "Encontrados ".pg_num_rows($rs_pagto)." pagtos para ".$rs_vendas_row['vg_id'].PHP_EOL;
                                $rs_pagto_row = pg_fetch_array($rs_pagto);
                                // conciliaMoneyDepositoSaldo_PagtoOnline($venda_id, $usuario_id, $parametros)
                                ...
                                if(number_format($total_pagto_sem_taxas,2,'.','') == number_format($vg_deposito_em_saldo_valor,2,'.','')) {
                                        ...
                                        $ret = conciliaMoneyDepositoSaldo_PagtoOnline($venda_id, $usuario_id, $parametros);
                                        if($ret!="") echo $ret;
                                        else echo "Deposito por pagamento online conciliado com sucesso e saldo depositado".PHP_EOL;
                                } else {
                                        if($bDebug) echo "NAO concilia PagtoOnline ($venda_id, $usuario_id)".PHP_EOL;
                                }
                        }
...
function conciliaMoneyDepositoSaldo_PagtoOnline($venda_id, $usuario_id, $parametros){
        ...
        //Valida usuario_id
        if(!$usuario_id) $msg = "Código do usuário Gamer não fornecido.".PHP_EOL;
        elseif(!is_numeric($usuario_id)) $msg = "Código do usuário Gamer inválido.".PHP_EOL;
        ...
        if($msg == ""){
                ...
                $rs_pagto = SQLexecuteQuery($sql);
                if(!$rs_pagto || pg_num_rows($rs_pagto) == 0) $msg = "Nenhum pagamento encontrado.".PHP_EOL;
                else {
                        $rs_pagto_row = pg_fetch_array($rs_pagto);
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao creditar valor do boleto no usuário Gamer.".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao completar venda Money (PagtoOnline).".PHP_EOL;
        }
        ...
        if($msg == ""){
                ...
                $ret = SQLexecuteQuery($sql);
                if(!$ret) $msg = "Erro ao inserir Saldo FIFO (PagtoOnline).".PHP_EOL;
        }
