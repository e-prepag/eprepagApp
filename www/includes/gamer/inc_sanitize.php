			   ...
			   foreach($params as $key => $val) {
					   ...
					   switch($val['1']) {
							   ...
							   case 'D': 
											 ...
											 break;
							   case 'F': 
											 $val_mod = sanitize_float_array($val['0']);
											 break;
					   }
...
//protected 
function sanitize_float_array($inval){
//			   $inval = intval($inval);
			   $outval = (filter_var($inval, FILTER_VALIDATE_FLOAT) === false)?"000":$inval;
			   return $outval;
}
//protected 
...
function sanitize_general_array($strval){
			   $outval = $strval;
			   if (strtoupper($outval) != str_replace("DROP ", "d_r_o_p_", strtoupper($outval)))
					$outval = str_replace("DROP ", "d_r_o_p_", strtoupper($outval));
			   if (strtoupper($outval) != str_replace("CREATE ", "c_r_e_a_t_e_", strtoupper($outval)))
					$outval = str_replace("CREATE ", "c_r_e_a_t_e_", strtoupper($outval));
			   if (strtoupper($outval) != str_replace("INSERT ", "i_n_s_e_r_t_", strtoupper($outval)))
					$outval = str_replace("INSERT ", "i_n_s_e_r_t_", strtoupper($outval));
			   if (strtoupper($outval) != str_replace("DELETE ", "d_e_l_e_t_e_", strtoupper($outval)))
					$outval = str_replace("DELETE ", "d_e_l_e_t_e_", strtoupper($outval));
			   if (strtoupper($outval) != str_replace("SELECT ", "s_e_l_e_c_t_", strtoupper($outval)))
					$outval = str_replace("SELECT ", "s_e_l_e_c_t_", strtoupper($outval));
			   if (strtoupper($outval) != str_replace("UPDATE ", "u_p_d_a_t_e_", strtoupper($outval)))
					$outval = str_replace("UPDATE ", "u_p_d_a_t_e_", strtoupper($outval));
			   if (strtoupper($outval) != str_replace("ALTER ", "a_l_t_e_r_", strtoupper($outval)))
