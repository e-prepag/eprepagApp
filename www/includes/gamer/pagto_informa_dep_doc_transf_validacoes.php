...
if ($msg == "") {
    ...
    WHERE vg.vg_ug_id = $1
        AND vg_pagto_banco = $2
        AND vg_pagto_local = $3
        AND vg_pagto_num_docto = $4
        AND vg_pagto_valor_pago = $5
        AND vg_pagto_data = $6
";
	...
	$params = [
		...
		$pagto_banco,
		$pagto_local,
		implode("|", $pagto_num_docto),
		moeda2numeric($pagto_valor_pago),
		monta_data_gravacao($pagto_data_data) . " " . $pagto_data_horas . ":" . $pagto_data_minutos,
	];

	$rs_venda_pagto = SQLexecuteQueryParams($sql, $params);
