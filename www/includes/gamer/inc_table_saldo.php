...
function get_venda_pagto_com_pinsepp_composicao($venda_id)
{
    ...
    SELECT *
    FROM pins_store
        INNER JOIN pins_store_pag_epp_pin 
            ON (ps_pin_codinterno = pin_codinterno)
...
";
	...
	$rs_pins_eppcash = SQLexecuteQueryParams($sql, $params);
...
function get_venda_pagto_com_saldo_composicao($venda_id)
{
	...
	$sql = "
    SELECT 
        scf.vg_id AS vg_id_deposito,
        (
            SELECT distributor_codigo
            FROM pins_store ps
                INNER JOIN pins_store_pag_epp_pin pspep 
                    ON (ps_pin_codinterno = pin_codinterno)
            WHERE pin_codinterno IN (
                SELECT ps_pin_codinterno
...
";
	...
	$rs_saldo_utilizado = SQLexecuteQueryParams($sql, $params);
