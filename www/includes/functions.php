...
function SQLexecuteQuery($sql)
{
	...
	if (preg_match('/^\s*(INSERT|UPDATE|DELETE|USUARIOS)/i', $sql)) {
		...
		$log .= " | URL from: " . (isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '') . PHP_EOL;
...
/*
DESCRI��O:
Fun��o que faz a pagina��o de uma query

ENTRADA:
$inicial: Indica em qual linha da query vai come�ar a mostrar
$total_table: Quantidade de registros que a query retornou
$max: Quantidade de registros por p�gina na tela
$qtde_colunas: Quantidade de colunas que tem a linha da tabela do HTML
$img_anterior: Caminho da imagem Anterior
$img_proxima: Caminho da imagem Pr�xima
$default_add: Nome do arquivo da p�gina
$range: N�mero do range inicial
$range_qtde: Quantidade de paginas mostradas na pagina��o
$ncamp: Campo que est� ordenando o SQL
$varsel: Vari�veis de pesquisa
...
function paginacao_query($inicial, $total_table, $max, $qtde_colunas, $img_anterior, $img_proxima, $default_add, $range, $range_qtde, $ncamp, $varsel)
{
	if ($total_table > $max) // S� mostra a numera��o das p�ginas se a quantidade de registros for maior do que o m�ximo permitido na tela
	{
		...
		$qtde_pg = ceil($total_table / $max); // Quantidade de p�ginas que a query vai ter

		if (($range + ($range_qtde - 1)) > $qtde_pg) // Retorna a quantidade de p�ginas de um range
			$limite = $qtde_pg;
		else
			$limite = ($range + $range_qtde - 1);

		for ($s = $range; $s <= $limite; $s++) // Monta a pagina��o propriamente dita
		{
			$esta_pag = ceil($inicial / $max) + 1;
			$indice = (($s * $max) - ($max - 1)) - 1;
			if ($esta_pag != $s) echo "<a href=" . $default_add . "?inicial=" . $indice . "&range=" . $range . "&ncamp=" . $ncamp . $varsel . "&btPesquisar=Pesquisar" . " class='link_azul'><font color='#00008C' size='1' face='Arial, Helvetica, sans-serif'>";
			else echo "<b>";
...
function data_mais_um($data)
{
	...
	if ($mes_num == 31) // Janeiro - Mar�o - Maio - Julho - Agosto - Outubro - Dezembro 
	{
		if ($dia <= 30) {
			$dia++;
		} else {
...
function data_menos_um($data)
{
	...
	if ($dia == 1) {
		...
		if ($mes_anterior == 2 && $bissexto == 0) {
			$mes_num = 28;
		}
		if ($mes_anterior == 4 || $mes_anterior == 6 || $mes_anterior == 9 || $mes_anterior == 11) {
			$mes_num = 30;
		}
		if ($mes_anterior == 1 || $mes_anterior == 3 || $mes_anterior == 5 || $mes_anterior == 7 || $mes_anterior == 8 || $mes_anterior == 10 || $mes_anterior == 12) {
			$mes_num = 31;
		}
...
function monta_ddd_string($campoddd)
{
	...
	for ($x = 1; $x <= strlen($recebeDDD); $x += 1) {
		...
		if (ord($ch) >= 48 && ord($ch) <= 57) {
			$alerta = 1;
			break;
		} // Se a string tiver n�mero 
		else {
			$alerta = 0;
		} // Se a string N�O tiver n�mero
	}

	if ($alerta == 1) // Se a string tiver n�mero
	{
		$aux_tr = 0;
		$aux_pv = 0;
		$traco = "";
		for ($y = 1; $y <= strlen($recebeDDD); $y += 1) {
			...
			if (ord($cha) == 45)   // Verifica se a string tem tra�o      
			{
				$traco = $aux_tr;
				$pos = $traco - 2;
				$inicial = substr($recebeDDD, $pos, 2);          // Indica o DDD inicial
				$final = substr($recebeDDD, $traco + 1, 2);      // Indica o DDD final
				for ($s = $inicial; $s <= $final; $s++) {      // Monta a sequ�ncia dos DDD�s entre inicial e final
					$var .= $s . ";";
				}
			}
			$aux_tr += 1;
...
function moeda_char($string)
{
	...
	if ($alerta == 1) {
		...
		if (strlen($dep_virg) == 1) {
			$depois_format = $dep_virg . "0";
		}
		if (strlen($dep_virg) == 2) {
			$depois_format = $dep_virg;
		}
		if (strlen($dep_virg) > 2) {
...
# Esta fun��o recebe um valor sem casa decimal, apenas n�meros
...
function insere_EstabelecimentoMovimentacao(
	...
	//							# ser� transformado para positivo.
		...
		<table border="0" cellspacing="0" cellpadding="0" align="center">
			...
			<tr align="center">
				<td>
					...
					<input name="btnSeuBanco" type="button" class="<?php if ($GLOBALS['_SESSION']['is_integration'] == true) echo "cleanLeftMargin botao_simples int-btn1 grad1";
																	else echo "botao_amex"; ?>" value="Ir para seu Banco" onClick="fcnSeuBanco(document.formSeuBanco);">
				</td>
			</tr>
		</table>
	</form>
...
function get_day_of_week($date1)
{
	...
	switch ($dia_ingles) {
		case "1":
			$dia_semana = LANG_SITE_DAY_OF_WEEK_MONDAY;
			break;
		case "2":
			$dia_semana = LANG_SITE_DAY_OF_WEEK_TUESDAY;
			break;
		case "3":
			$dia_semana = LANG_SITE_DAY_OF_WEEK_WEDNESDAY;
			break;
		case "4":
			$dia_semana = LANG_SITE_DAY_OF_WEEK_THURSDAY;
			break;
		case "5":
			$dia_semana = LANG_SITE_DAY_OF_WEEK_FRIDAY;
			break;
		case "6":
			$dia_semana = LANG_SITE_DAY_OF_WEEK_SATURDAY;
			break;
		case "0":
			$dia_semana = LANG_SITE_DAY_OF_WEEK_SUNDAY;
			break;
	}
	return $dia_semana;
}
